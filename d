// N5 Mock Test Data - v18.8.10
if (typeof MOCK_DATA === 'undefined') { var MOCK_DATA = {}; }
MOCK_DATA['N5'] = {
  "info": {
    "level": "N5",
    "totalSets": 20,
    "questionsPerSet": 76,
    "timeLimit": 90
  },
  "sets": {
    "1": [
      {
        "type": "漢字読み",
        "text": "<u>空く</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "やま",
          "ひだり",
          "せんげつ",
          "すく"
        ],
        "answer": 3,
        "id": "N5-01-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>左</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "すく",
          "でる",
          "なまえ",
          "ひだり"
        ],
        "answer": 3,
        "id": "N5-01-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>毎日</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "おしえる",
          "まいとし",
          "おんな",
          "まいにち"
        ],
        "answer": 3,
        "id": "N5-01-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>入学</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "おしえる",
          "ひとり",
          "こんしゅう",
          "にゅうがく"
        ],
        "answer": 3,
        "id": "N5-01-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>教室</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "くるま",
          "はなす",
          "きょうしつ",
          "はんぶん"
        ],
        "answer": 2,
        "id": "N5-01-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>来年</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "すいようび",
          "まいとし",
          "らいねん",
          "はは"
        ],
        "answer": 2,
        "id": "N5-01-006"
      },
      {
        "type": "表記",
        "text": "<u>き</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "木",
          "見る",
          "雪",
          "上"
        ],
        "answer": 0,
        "id": "N5-01-007"
      },
      {
        "type": "表記",
        "text": "<u>あとで</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "出口",
          "鳥",
          "毎週",
          "後で"
        ],
        "answer": 3,
        "id": "N5-01-008"
      },
      {
        "type": "表記",
        "text": "<u>いろいろ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "色々",
          "天気",
          "力",
          "女"
        ],
        "answer": 0,
        "id": "N5-01-009"
      },
      {
        "type": "表記",
        "text": "<u>げつようび</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "車",
          "先週",
          "日曜日",
          "月曜日"
        ],
        "answer": 3,
        "id": "N5-01-010"
      },
      {
        "type": "表記",
        "text": "<u>きく</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "先月",
          "聞く",
          "今週",
          "駅"
        ],
        "answer": 1,
        "id": "N5-01-011"
      },
      {
        "type": "表記",
        "text": "<u>もくようび</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "正しい",
          "社会",
          "木曜日",
          "聞こえる"
        ],
        "answer": 2,
        "id": "N5-01-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みかん",
          "くるま",
          "かさ",
          "バス"
        ],
        "answer": 0,
        "id": "N5-01-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ズボン",
          "ジュース",
          "めがね",
          "えんぴつ"
        ],
        "answer": 1,
        "id": "N5-01-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "えき",
          "つくえ",
          "そば",
          "びょういん"
        ],
        "answer": 2,
        "id": "N5-01-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "びょういん",
          "すし",
          "でんしゃ",
          "えき"
        ],
        "answer": 1,
        "id": "N5-01-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みかん",
          "りんご",
          "パン",
          "がっこう"
        ],
        "answer": 3,
        "id": "N5-01-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ほん",
          "テレビ",
          "みず",
          "としょかん"
        ],
        "answer": 3,
        "id": "N5-01-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "コンビニ",
          "えんぴつ",
          "ほん",
          "みず"
        ],
        "answer": 0,
        "id": "N5-01-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みかん",
          "びじゅつかん",
          "おちゃ",
          "みず"
        ],
        "answer": 1,
        "id": "N5-01-020"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>日曜日<rt>にちようび</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "to do",
          "backpack",
          "Sunday",
          "boy"
        ],
        "answer": 2,
        "id": "N5-01-021"
      },
      {
        "type": "言い換え",
        "text": "「スポーツ」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "grandmother (polite)",
          "road",
          "sports",
          "sometimes"
        ],
        "answer": 2,
        "id": "N5-01-022"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>出口<rt>でぐち</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "as expected",
          "more",
          "exit",
          "that (far)"
        ],
        "answer": 2,
        "id": "N5-01-023"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>一つ<rt>ひとつ</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "one thing",
          "sky",
          "toilet",
          "to speak"
        ],
        "answer": 0,
        "id": "N5-01-024"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>電話<rt>でんわ</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "bright",
          "telephone",
          "how many",
          "thank you"
        ],
        "answer": 1,
        "id": "N5-01-025"
      },
      {
        "type": "文法形式",
        "text": "わたしは　がくせい（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "です",
          "ます",
          "ある",
          "だ"
        ],
        "answer": 0,
        "id": "N5-01-026"
      },
      {
        "type": "文法形式",
        "text": "がっこう（　）いきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "を",
          "で",
          "が"
        ],
        "answer": 0,
        "id": "N5-01-027"
      },
      {
        "type": "文法形式",
        "text": "ほん（　）よみます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "で",
          "に",
          "が"
        ],
        "answer": 0,
        "id": "N5-01-028"
      },
      {
        "type": "文法形式",
        "text": "うみ（　）およぎます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "を",
          "に",
          "が"
        ],
        "answer": 0,
        "id": "N5-01-029"
      },
      {
        "type": "文法形式",
        "text": "ねこ（　）います。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "は",
          "に"
        ],
        "answer": 0,
        "id": "N5-01-030"
      },
      {
        "type": "文法形式",
        "text": "りんご（　）みかんを　かいました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "と",
          "や",
          "も",
          "が"
        ],
        "answer": 0,
        "id": "N5-01-031"
      },
      {
        "type": "文法形式",
        "text": "9じ（　）しごとを　はじめます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "から",
          "に",
          "で",
          "まで"
        ],
        "answer": 0,
        "id": "N5-01-032"
      },
      {
        "type": "文法形式",
        "text": "これは　わたし（　）かばんです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "の",
          "は",
          "を",
          "が"
        ],
        "answer": 0,
        "id": "N5-01-033"
      },
      {
        "type": "文法形式",
        "text": "あした　ともだちに　（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "あいます",
          "あって",
          "あう",
          "あいました"
        ],
        "answer": 0,
        "id": "N5-01-034"
      },
      {
        "type": "文法形式",
        "text": "きのう　えいがを　（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "みました",
          "みて",
          "みます",
          "みる"
        ],
        "answer": 0,
        "id": "N5-01-035"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "がくせい",
          "です",
          "は",
          "わたし"
        ],
        "answer": 0,
        "id": "N5-01-036"
      },
      {
        "type": "文の組み立て",
        "text": "これは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "ほん",
          "です",
          "は",
          "これ"
        ],
        "answer": 0,
        "id": "N5-01-037"
      },
      {
        "type": "文の組み立て",
        "text": "あれは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "えき",
          "です",
          "は",
          "あれ"
        ],
        "answer": 0,
        "id": "N5-01-038"
      },
      {
        "type": "文の組み立て",
        "text": "きょうは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "あつい",
          "です",
          "は",
          "きょう"
        ],
        "answer": 0,
        "id": "N5-01-039"
      },
      {
        "type": "文の組み立て",
        "text": "りんごは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "おいしい",
          "です",
          "は",
          "りんご"
        ],
        "answer": 0,
        "id": "N5-01-040"
      },
      {
        "type": "文章の文法",
        "text": "わたしは まいあさ 6じに おきます。かおを あらって、あさごはんを（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "たべます",
          "たべて",
          "たべた",
          "たべる"
        ],
        "answer": 0,
        "id": "N5-01-041"
      },
      {
        "type": "文章の文法",
        "text": "きのうは にちようびでした。ともだちと こうえんで（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "あそびました",
          "あそびます",
          "あそんで",
          "あそぶ"
        ],
        "answer": 0,
        "id": "N5-01-042"
      },
      {
        "type": "文章の文法",
        "text": "このりょうりは とても おいしいです。もっと（　）ください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "たべて",
          "たべます",
          "たべた",
          "たべる"
        ],
        "answer": 0,
        "id": "N5-01-043"
      },
      {
        "type": "文章の文法",
        "text": "あしたは テストが あります。きょうは たくさん（　）つもりです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "べんきょうする",
          "べんきょうして",
          "べんきょうした",
          "べんきょうします"
        ],
        "answer": 0,
        "id": "N5-01-044"
      },
      {
        "type": "文章の文法",
        "text": "わたしの へやは ちいさいですが、（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "きれい",
          "きれいな",
          "きれいに",
          "きれいで"
        ],
        "answer": 0,
        "id": "N5-01-045"
      },
      {
        "type": "読解（短文）",
        "text": "たなかさんは　まいあさ　7じに　おきます。あさごはんを　たべて、8じに　いえを　でます。",
        "question": "たなかさんは　なんじに　おきますか。",
        "options": [
          "6じ",
          "7じ",
          "8じ",
          "9じ"
        ],
        "answer": 1,
        "id": "N5-01-046"
      },
      {
        "type": "読解（短文）",
        "text": "わたしは　まいにち　6じかん　ねます。11じに　ねて、5じに　おきます。",
        "question": "わたしは　なんじに　おきますか。",
        "options": [
          "4じ",
          "5じ",
          "6じ",
          "7じ"
        ],
        "answer": 1,
        "id": "N5-01-047"
      },
      {
        "type": "読解（短文）",
        "text": "このみせは　10じに　あきます。6じに　しまります。",
        "question": "このみせは　なんじに　あきますか。",
        "options": [
          "9じ",
          "10じ",
          "11じ",
          "12じ"
        ],
        "answer": 1,
        "id": "N5-01-048"
      },
      {
        "type": "読解（中文）",
        "text": "わたしは　まいにち　7じに　おきます。シャワーを　あびて、あさごはんを　たべます。8じに　いえを　でて、がっこうに　いきます。がっこうまで　でんしゃで　30ぷんです。",
        "question": "わたしは　なにで　がっこうに　いきますか。",
        "options": [
          "バス",
          "でんしゃ",
          "じてんしゃ",
          "あるいて"
        ],
        "answer": 1,
        "id": "N5-01-049"
      },
      {
        "type": "読解（中文）",
        "text": "わたしの　いえには　いぬが　1ぴきと　ねこが　2ひき　います。いぬの　なまえは　ポチです。ねこの　なまえは　タマと　ミケです。まいにち　さんぽを　します。",
        "question": "ねこは　なんびき　いますか。",
        "options": [
          "1ぴき",
          "2ひき",
          "3びき",
          "4ひき"
        ],
        "answer": 1,
        "id": "N5-01-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-01-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "小学校",
          "ジム",
          "クリニック",
          "歯医者"
        ],
        "answer": 0,
        "id": "N5-01-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "中学校",
          "小学校",
          "ジム",
          "クリニック"
        ],
        "answer": 2,
        "id": "N5-01-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "高校",
          "中学校",
          "小学校",
          "ジム"
        ],
        "answer": 0,
        "id": "N5-01-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "保育園",
          "高校",
          "中学校",
          "小学校"
        ],
        "answer": 2,
        "id": "N5-01-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "幼稚園",
          "保育園",
          "高校",
          "中学校"
        ],
        "answer": 0,
        "id": "N5-01-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "大学",
          "幼稚園",
          "保育園",
          "高校"
        ],
        "answer": 2,
        "id": "N5-01-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "1時",
          "1時15分",
          "1時半",
          "1時45分"
        ],
        "answer": 0,
        "id": "N5-01-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "2時",
          "2時15分",
          "2時半",
          "2時45分"
        ],
        "answer": 1,
        "id": "N5-01-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "3時",
          "3時15分",
          "3時半",
          "3時45分"
        ],
        "answer": 0,
        "id": "N5-01-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "4時",
          "4時15分",
          "4時半",
          "4時45分"
        ],
        "answer": 1,
        "id": "N5-01-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "5時",
          "5時15分",
          "5時半",
          "5時45分"
        ],
        "answer": 0,
        "id": "N5-01-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "6時",
          "6時15分",
          "6時半",
          "6時45分"
        ],
        "answer": 1,
        "id": "N5-01-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "おはようございます",
          "こんにちは",
          "こんばんは",
          "おやすみなさい"
        ],
        "answer": 0,
        "id": "N5-01-064",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "おはよう",
          "こんにちは",
          "こんばんは",
          "じゃあね"
        ],
        "answer": 0,
        "id": "N5-01-065",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "行ってきます",
          "行ってらっしゃい",
          "ただいま",
          "おかえりなさい"
        ],
        "answer": 0,
        "id": "N5-01-066",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "いただきます",
          "ごちそうさまでした",
          "おいしかったです",
          "もう少し"
        ],
        "answer": 0,
        "id": "N5-01-067",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "初めまして",
          "よろしくお願いします",
          "こちらこそ",
          "どうぞよろしく"
        ],
        "answer": 0,
        "id": "N5-01-068",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "田中です",
          "山田です",
          "鈴木です",
          "佐藤です"
        ],
        "answer": 0,
        "id": "N5-01-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "山田です",
          "鈴木です",
          "佐藤です",
          "高橋です"
        ],
        "answer": 0,
        "id": "N5-01-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "鈴木です",
          "佐藤です",
          "高橋です",
          "伊藤です"
        ],
        "answer": 0,
        "id": "N5-01-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "佐藤です",
          "高橋です",
          "伊藤です",
          "渡辺です"
        ],
        "answer": 0,
        "id": "N5-01-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "高橋です",
          "伊藤です",
          "渡辺です",
          "山本です"
        ],
        "answer": 0,
        "id": "N5-01-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "伊藤です",
          "渡辺です",
          "山本です",
          "中村です"
        ],
        "answer": 0,
        "id": "N5-01-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "渡辺です",
          "山本です",
          "中村です",
          "小林です"
        ],
        "answer": 0,
        "id": "N5-01-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "山本です",
          "中村です",
          "小林です",
          "加藤です"
        ],
        "answer": 0,
        "id": "N5-01-076",
        "script": "なんさいですか。"
      }
    ],
    "2": [
      {
        "type": "漢字読み",
        "text": "<u>上手</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "ただしい",
          "じょうず",
          "みず",
          "せんげつ"
        ],
        "answer": 1,
        "id": "N5-02-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>飲む</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "みち",
          "のむ",
          "げつようび",
          "おみず"
        ],
        "answer": 1,
        "id": "N5-02-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>音</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "おと",
          "まいつき",
          "かいけい",
          "つくる"
        ],
        "answer": 0,
        "id": "N5-02-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>手</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "おちゃ",
          "おと",
          "いま",
          "て"
        ],
        "answer": 3,
        "id": "N5-02-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>十分</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "うまれる",
          "みえる",
          "さがる",
          "じゅうぶん"
        ],
        "answer": 3,
        "id": "N5-02-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>足りる</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "かいけい",
          "てんき",
          "あした",
          "たりる"
        ],
        "answer": 3,
        "id": "N5-02-006"
      },
      {
        "type": "表記",
        "text": "<u>ひゃく</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "今日",
          "百",
          "見える",
          "分かる"
        ],
        "answer": 1,
        "id": "N5-02-007"
      },
      {
        "type": "表記",
        "text": "<u>こんしゅう</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "今週",
          "見つける",
          "二つ",
          "肉"
        ],
        "answer": 0,
        "id": "N5-02-008"
      },
      {
        "type": "表記",
        "text": "<u>あめ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "電気",
          "雨",
          "午後",
          "社長"
        ],
        "answer": 1,
        "id": "N5-02-009"
      },
      {
        "type": "表記",
        "text": "<u>みぎ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "毎週",
          "右",
          "入力",
          "花"
        ],
        "answer": 1,
        "id": "N5-02-010"
      },
      {
        "type": "表記",
        "text": "<u>でんき</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "休み",
          "電気",
          "絵",
          "生まれる"
        ],
        "answer": 1,
        "id": "N5-02-011"
      },
      {
        "type": "表記",
        "text": "<u>わかる</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "時間",
          "分かる",
          "黒い",
          "作る"
        ],
        "answer": 1,
        "id": "N5-02-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "テレビ",
          "かさ",
          "りんご",
          "びょういん"
        ],
        "answer": 2,
        "id": "N5-02-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "かぎ",
          "おちゃ",
          "ズボン",
          "くつ"
        ],
        "answer": 1,
        "id": "N5-02-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "くるま",
          "うどん",
          "バス",
          "でんわ"
        ],
        "answer": 1,
        "id": "N5-02-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "でんわ",
          "いす",
          "すし",
          "かさ"
        ],
        "answer": 2,
        "id": "N5-02-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みかん",
          "えんぴつ",
          "おちゃ",
          "えき"
        ],
        "answer": 3,
        "id": "N5-02-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "へや",
          "みず",
          "テレビ",
          "パン"
        ],
        "answer": 0,
        "id": "N5-02-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "テレビ",
          "パン",
          "デパート",
          "ほん"
        ],
        "answer": 2,
        "id": "N5-02-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みかん",
          "こうえん",
          "りんご",
          "おちゃ"
        ],
        "answer": 1,
        "id": "N5-02-020"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>形<rt>かたち</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "you",
          "supermarket",
          "shape",
          "to be born"
        ],
        "answer": 2,
        "id": "N5-02-021"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>黒い<rt>くろい</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "where",
          "immediately",
          "sweater",
          "black"
        ],
        "answer": 3,
        "id": "N5-02-022"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>聞こえる<rt>きこえる</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "television",
          "notebook",
          "between",
          "be audible"
        ],
        "answer": 3,
        "id": "N5-02-023"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>男<rt>おとこ</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "between",
          "man",
          "therefore",
          "coat"
        ],
        "answer": 1,
        "id": "N5-02-024"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>社会<rt>しゃかい</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "society",
          "why",
          "to speak",
          "to be born"
        ],
        "answer": 0,
        "id": "N5-02-025"
      },
      {
        "type": "文法形式",
        "text": "わたしは　にくを　（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "たべません",
          "たべない",
          "たべます",
          "たべる"
        ],
        "answer": 0,
        "id": "N5-02-026"
      },
      {
        "type": "文法形式",
        "text": "てを　あらっ（　）ごはんを　たべます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "て",
          "に",
          "から",
          "で"
        ],
        "answer": 0,
        "id": "N5-02-027"
      },
      {
        "type": "文法形式",
        "text": "いま　ごはんを　（　）います。",
        "question": "（　）に　はいるのは？",
        "options": [
          "たべて",
          "たべた",
          "たべる",
          "たべ"
        ],
        "answer": 0,
        "id": "N5-02-028"
      },
      {
        "type": "文法形式",
        "text": "わたしは　にほんに　いき（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "たい",
          "て",
          "る",
          "ます"
        ],
        "answer": 0,
        "id": "N5-02-029"
      },
      {
        "type": "文法形式",
        "text": "きょうは　（　）ですね。",
        "question": "（　）に　はいるのは？",
        "options": [
          "あつい",
          "あつくて",
          "あつく",
          "あつかった"
        ],
        "answer": 0,
        "id": "N5-02-030"
      },
      {
        "type": "文法形式",
        "text": "やまださんは　（　）ひとです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "しんせつな",
          "しんせつに",
          "しんせつ",
          "しんせつで"
        ],
        "answer": 0,
        "id": "N5-02-031"
      },
      {
        "type": "文法形式",
        "text": "えきは　（　）ですか。",
        "question": "（　）に　はいるのは？",
        "options": [
          "どこ",
          "なに",
          "いつ",
          "だれ"
        ],
        "answer": 0,
        "id": "N5-02-032"
      },
      {
        "type": "文法形式",
        "text": "りんごを　（　）ください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "ひとつ",
          "ひとこ",
          "いっこ",
          "いちこ"
        ],
        "answer": 0,
        "id": "N5-02-033"
      },
      {
        "type": "文法形式",
        "text": "あめです（　）、でかけません。",
        "question": "（　）に　はいるのは？",
        "options": [
          "から",
          "ので",
          "のに",
          "が"
        ],
        "answer": 0,
        "id": "N5-02-034"
      },
      {
        "type": "文法形式",
        "text": "じかんが　あっ（　）、てつだいます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "たら",
          "で",
          "と",
          "ら"
        ],
        "answer": 0,
        "id": "N5-02-035"
      },
      {
        "type": "文の組み立て",
        "text": "にほんごは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "むずかしい",
          "です",
          "は",
          "にほんご"
        ],
        "answer": 0,
        "id": "N5-02-036"
      },
      {
        "type": "文の組み立て",
        "text": "このへやは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "ひろい",
          "です",
          "は",
          "へや"
        ],
        "answer": 0,
        "id": "N5-02-037"
      },
      {
        "type": "文の組み立て",
        "text": "あのひとは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "せんせい",
          "です",
          "は",
          "ひと"
        ],
        "answer": 0,
        "id": "N5-02-038"
      },
      {
        "type": "文の組み立て",
        "text": "このみせは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "やすい",
          "です",
          "は",
          "みせ"
        ],
        "answer": 0,
        "id": "N5-02-039"
      },
      {
        "type": "文の組み立て",
        "text": "そのくるまは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "あたらしい",
          "です",
          "は",
          "くるま"
        ],
        "answer": 0,
        "id": "N5-02-040"
      },
      {
        "type": "文章の文法",
        "text": "この えいがは おもしろいです。ぜひ（　）ください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "みて",
          "みます",
          "みた",
          "みる"
        ],
        "answer": 0,
        "id": "N5-02-041"
      },
      {
        "type": "文章の文法",
        "text": "かれは まいにち ジョギングを（　）います。",
        "question": "（　）に　はいるのは？",
        "options": [
          "して",
          "する",
          "した",
          "します"
        ],
        "answer": 0,
        "id": "N5-02-042"
      },
      {
        "type": "文章の文法",
        "text": "きょうは あめが ふって（　）、さんぽに いけません。",
        "question": "（　）に　はいるのは？",
        "options": [
          "いるので",
          "いるから",
          "いて",
          "います"
        ],
        "answer": 0,
        "id": "N5-02-043"
      },
      {
        "type": "文章の文法",
        "text": "この ケーキは あまくて（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "おいしい",
          "おいしく",
          "おいしいな",
          "おいしいに"
        ],
        "answer": 0,
        "id": "N5-02-044"
      },
      {
        "type": "文章の文法",
        "text": "わたしは にほんごを（　）たいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "はなせる",
          "はなし",
          "はなす",
          "はなして"
        ],
        "answer": 0,
        "id": "N5-02-045"
      },
      {
        "type": "読解（短文）",
        "text": "えいがは　2じかんです。3じに　はじまります。",
        "question": "えいがは　なんじに　おわりますか。",
        "options": [
          "4じ",
          "5じ",
          "6じ",
          "7じ"
        ],
        "answer": 1,
        "id": "N5-02-046"
      },
      {
        "type": "読解（短文）",
        "text": "バスは　15ふんごとに　きます。いま　9じです。つぎのバスは　9じ15ふんです。",
        "question": "つぎのバスは　なんじですか。",
        "options": [
          "9じ",
          "9じ15ふん",
          "9じ30ぷん",
          "10じ"
        ],
        "answer": 1,
        "id": "N5-02-047"
      },
      {
        "type": "読解（短文）",
        "text": "わたしの　へやには　つくえと　いすが　あります。まどの　ちかくに　ベッドが　あります。",
        "question": "ベッドは　どこに　ありますか。",
        "options": [
          "つくえの　ちかく",
          "いすの　ちかく",
          "まどの　ちかく",
          "ドアの　ちかく"
        ],
        "answer": 1,
        "id": "N5-02-048"
      },
      {
        "type": "読解（中文）",
        "text": "きのう　ともだちと　えいがを　みました。えいがは　2じかんでした。そのあと　レストランで　ばんごはんを　たべました。とても　たのしかったです。",
        "question": "えいがの　あとで　なにを　しましたか。",
        "options": [
          "かいものを　した",
          "ばんごはんを　たべた",
          "いえに　かえった",
          "こうえんに　いった"
        ],
        "answer": 1,
        "id": "N5-02-049"
      },
      {
        "type": "読解（中文）",
        "text": "わたしの　しゅみは　りょうりです。どようびに　よく　りょうりを　します。きのうは　カレーを　つくりました。かぞくは　おいしいと　いいました。",
        "question": "きのう　なにを　つくりましたか。",
        "options": [
          "すし",
          "ラーメン",
          "カレー",
          "サラダ"
        ],
        "answer": 1,
        "id": "N5-02-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-02-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "教会",
          "大学",
          "幼稚園",
          "保育園"
        ],
        "answer": 0,
        "id": "N5-02-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "お寺",
          "教会",
          "大学",
          "幼稚園"
        ],
        "answer": 2,
        "id": "N5-02-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "神社",
          "お寺",
          "教会",
          "大学"
        ],
        "answer": 0,
        "id": "N5-02-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "遊園地",
          "神社",
          "お寺",
          "教会"
        ],
        "answer": 2,
        "id": "N5-02-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "水族館",
          "遊園地",
          "神社",
          "お寺"
        ],
        "answer": 0,
        "id": "N5-02-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "動物園",
          "水族館",
          "遊園地",
          "神社"
        ],
        "answer": 2,
        "id": "N5-02-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "7時",
          "7時15分",
          "7時半",
          "7時45分"
        ],
        "answer": 0,
        "id": "N5-02-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "8時",
          "8時15分",
          "8時半",
          "8時45分"
        ],
        "answer": 1,
        "id": "N5-02-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "9時",
          "9時15分",
          "9時半",
          "9時45分"
        ],
        "answer": 0,
        "id": "N5-02-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "10時",
          "10時15分",
          "10時半",
          "10時45分"
        ],
        "answer": 1,
        "id": "N5-02-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "11時",
          "11時15分",
          "11時半",
          "11時45分"
        ],
        "answer": 0,
        "id": "N5-02-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "12時",
          "12時15分",
          "12時半",
          "12時45分"
        ],
        "answer": 1,
        "id": "N5-02-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "お久しぶりです",
          "お元気でしたか",
          "お変わりないですか",
          "最近どうですか"
        ],
        "answer": 0,
        "id": "N5-02-064",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "さようなら",
          "またね",
          "お先に",
          "では"
        ],
        "answer": 0,
        "id": "N5-02-065",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "お邪魔します",
          "お邪魔しました",
          "ごめんください",
          "どうぞお上がりください"
        ],
        "answer": 0,
        "id": "N5-02-066",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "いらっしゃいませ",
          "ありがとうございました",
          "またお越しください",
          "お気をつけて"
        ],
        "answer": 0,
        "id": "N5-02-067",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "お疲れ様です",
          "お疲れ様でした",
          "ご苦労様です",
          "お先に失礼します"
        ],
        "answer": 0,
        "id": "N5-02-068",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "中村です",
          "小林です",
          "加藤です",
          "吉田です"
        ],
        "answer": 0,
        "id": "N5-02-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "小林です",
          "加藤です",
          "吉田です",
          "山口です"
        ],
        "answer": 0,
        "id": "N5-02-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "加藤です",
          "吉田です",
          "山口です",
          "松本です"
        ],
        "answer": 0,
        "id": "N5-02-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "吉田です",
          "山口です",
          "松本です",
          "井上です"
        ],
        "answer": 0,
        "id": "N5-02-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "山口です",
          "松本です",
          "井上です",
          "木村です"
        ],
        "answer": 0,
        "id": "N5-02-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "松本です",
          "井上です",
          "木村です",
          "林です"
        ],
        "answer": 0,
        "id": "N5-02-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "井上です",
          "木村です",
          "林です",
          "斎藤です"
        ],
        "answer": 0,
        "id": "N5-02-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "木村です",
          "林です",
          "斎藤です",
          "清水です"
        ],
        "answer": 0,
        "id": "N5-02-076",
        "script": "なんさいですか。"
      }
    ],
    "3": [
      {
        "type": "漢字読み",
        "text": "<u>お金</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "あかるい",
          "おかね",
          "みず",
          "でぐち"
        ],
        "answer": 1,
        "id": "N5-03-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>先生</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "おとな",
          "くるま",
          "せんせい",
          "あいだ"
        ],
        "answer": 2,
        "id": "N5-03-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>何</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "おんな",
          "なに",
          "おなじ",
          "とけい"
        ],
        "answer": 1,
        "id": "N5-03-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>明日</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "なまえ",
          "き",
          "おみず",
          "あした"
        ],
        "answer": 3,
        "id": "N5-03-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>上がる</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "うまれる",
          "あがる",
          "かく",
          "かたち"
        ],
        "answer": 1,
        "id": "N5-03-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>先週</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "え",
          "おちゃ",
          "せんしゅう",
          "いろいろ"
        ],
        "answer": 2,
        "id": "N5-03-006"
      },
      {
        "type": "表記",
        "text": "<u>おと</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "音",
          "魚",
          "鳥",
          "音楽"
        ],
        "answer": 0,
        "id": "N5-03-007"
      },
      {
        "type": "表記",
        "text": "<u>ただしい</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "話す",
          "電気",
          "下手",
          "正しい"
        ],
        "answer": 3,
        "id": "N5-03-008"
      },
      {
        "type": "表記",
        "text": "<u>みみ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "耳",
          "今週",
          "書く",
          "男の子"
        ],
        "answer": 0,
        "id": "N5-03-009"
      },
      {
        "type": "表記",
        "text": "<u>はな</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "生まれる",
          "生きる",
          "音楽",
          "花"
        ],
        "answer": 3,
        "id": "N5-03-010"
      },
      {
        "type": "表記",
        "text": "<u>もり</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "森",
          "土曜日",
          "国",
          "毎日"
        ],
        "answer": 0,
        "id": "N5-03-011"
      },
      {
        "type": "表記",
        "text": "<u>め</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "目",
          "お茶",
          "見える",
          "教室"
        ],
        "answer": 0,
        "id": "N5-03-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "いす",
          "みかん",
          "バス",
          "かさ"
        ],
        "answer": 1,
        "id": "N5-03-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "かぎ",
          "めがね",
          "おちゃ",
          "シャツ"
        ],
        "answer": 2,
        "id": "N5-03-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "いす",
          "テレビ",
          "カレー",
          "びょういん"
        ],
        "answer": 2,
        "id": "N5-03-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "テレビ",
          "すし",
          "くるま",
          "でんしゃ"
        ],
        "answer": 1,
        "id": "N5-03-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "えき",
          "みかん",
          "テレビ",
          "えんぴつ"
        ],
        "answer": 0,
        "id": "N5-03-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "としょかん",
          "テレビ",
          "えんぴつ",
          "おちゃ"
        ],
        "answer": 0,
        "id": "N5-03-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みかん",
          "スーパー",
          "テレビ",
          "えんぴつ"
        ],
        "answer": 1,
        "id": "N5-03-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "びじゅつかん",
          "えんぴつ",
          "みず",
          "テレビ"
        ],
        "answer": 0,
        "id": "N5-03-020"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>社長<rt>しゃちょう</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "that (far)",
          "sky",
          "company president",
          "school"
        ],
        "answer": 2,
        "id": "N5-03-021"
      },
      {
        "type": "言い換え",
        "text": "「おやすみ」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "meat",
          "to buy",
          "power",
          "good night"
        ],
        "answer": 3,
        "id": "N5-03-022"
      },
      {
        "type": "言い換え",
        "text": "「かける」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "telephone",
          "left",
          "holiday",
          "to wear (glasses)"
        ],
        "answer": 3,
        "id": "N5-03-023"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>今日<rt>きょう</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "grandmother (polite)",
          "convenience store",
          "two people",
          "today"
        ],
        "answer": 3,
        "id": "N5-03-024"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>外国<rt>がいこく</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "foreign country",
          "forest",
          "healthy/energetic",
          "country"
        ],
        "answer": 0,
        "id": "N5-03-025"
      },
      {
        "type": "文法形式",
        "text": "ともだち（　）あいました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "が",
          "を",
          "で"
        ],
        "answer": 0,
        "id": "N5-03-026"
      },
      {
        "type": "文法形式",
        "text": "でんしゃ（　）のります。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "が",
          "を",
          "で"
        ],
        "answer": 0,
        "id": "N5-03-027"
      },
      {
        "type": "文法形式",
        "text": "えき（　）おります。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "が",
          "に",
          "を"
        ],
        "answer": 0,
        "id": "N5-03-028"
      },
      {
        "type": "文法形式",
        "text": "にほんご（　）べんきょうします。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "で",
          "が"
        ],
        "answer": 0,
        "id": "N5-03-029"
      },
      {
        "type": "文法形式",
        "text": "へや（　）そうじします。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "で",
          "に",
          "が"
        ],
        "answer": 0,
        "id": "N5-03-030"
      },
      {
        "type": "文法形式",
        "text": "おんがく（　）ききます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "が",
          "で"
        ],
        "answer": 0,
        "id": "N5-03-031"
      },
      {
        "type": "文法形式",
        "text": "えいが（　）みます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "が",
          "で"
        ],
        "answer": 0,
        "id": "N5-03-032"
      },
      {
        "type": "文法形式",
        "text": "こうえん（　）さんぽします。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "で",
          "が"
        ],
        "answer": 0,
        "id": "N5-03-033"
      },
      {
        "type": "文法形式",
        "text": "やま（　）のぼります。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "を",
          "が",
          "で"
        ],
        "answer": 0,
        "id": "N5-03-034"
      },
      {
        "type": "文法形式",
        "text": "レストラン（　）たべます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "が",
          "に",
          "を"
        ],
        "answer": 0,
        "id": "N5-03-035"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "ほんを",
          "よみます",
          "は",
          "わたし"
        ],
        "answer": 1,
        "id": "N5-03-036"
      },
      {
        "type": "文の組み立て",
        "text": "ともだちは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "おんがくを",
          "ききます",
          "は",
          "ともだち"
        ],
        "answer": 1,
        "id": "N5-03-037"
      },
      {
        "type": "文の組み立て",
        "text": "ははは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "りょうりを",
          "つくります",
          "は",
          "はは"
        ],
        "answer": 1,
        "id": "N5-03-038"
      },
      {
        "type": "文の組み立て",
        "text": "ちちは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "しんぶんを",
          "よみます",
          "は",
          "ちち"
        ],
        "answer": 1,
        "id": "N5-03-039"
      },
      {
        "type": "文の組み立て",
        "text": "あには ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "サッカーを",
          "します",
          "は",
          "あに"
        ],
        "answer": 1,
        "id": "N5-03-040"
      },
      {
        "type": "文章の文法",
        "text": "でんしゃが こないので、バス（　）いきましょう。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "に",
          "を",
          "が"
        ],
        "answer": 0,
        "id": "N5-03-041"
      },
      {
        "type": "文章の文法",
        "text": "この ほんは とても（　）です。よんでみてください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "おもしろい",
          "おもしろく",
          "おもしろいな",
          "おもしろいに"
        ],
        "answer": 0,
        "id": "N5-03-042"
      },
      {
        "type": "文章の文法",
        "text": "かのじょは うたが（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "じょうず",
          "じょうずな",
          "じょうずに",
          "じょうずで"
        ],
        "answer": 0,
        "id": "N5-03-043"
      },
      {
        "type": "文章の文法",
        "text": "きょう（　）あつかったです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "は",
          "が",
          "を",
          "に"
        ],
        "answer": 0,
        "id": "N5-03-044"
      },
      {
        "type": "文章の文法",
        "text": "わたしは コーヒー（　）すきです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "は",
          "に"
        ],
        "answer": 0,
        "id": "N5-03-045"
      },
      {
        "type": "読解（短文）",
        "text": "ほんやは　えきの　まえに　あります。ゆうびんきょくは　ほんやの　となりです。",
        "question": "ゆうびんきょくは　どこに　ありますか。",
        "options": [
          "えきの　まえ",
          "ほんやの　となり",
          "えきの　なか",
          "ほんやの　うしろ"
        ],
        "answer": 1,
        "id": "N5-03-046"
      },
      {
        "type": "読解（短文）",
        "text": "スーパーは　がっこうの　みぎに　あります。びょういんは　がっこうの　ひだりに　あります。",
        "question": "びょういんは　どこに　ありますか。",
        "options": [
          "がっこうの　まえ",
          "がっこうの　みぎ",
          "がっこうの　ひだり",
          "がっこうの　うしろ"
        ],
        "answer": 1,
        "id": "N5-03-047"
      },
      {
        "type": "読解（短文）",
        "text": "わたしの　いえは　こうえんの　ちかくに　あります。こうえんまで　あるいて　5ふんです。",
        "question": "こうえんまで　どのくらい　かかりますか。",
        "options": [
          "3ぷん",
          "5ふん",
          "10ぷん",
          "15ふん"
        ],
        "answer": 1,
        "id": "N5-03-048"
      },
      {
        "type": "読解（中文）",
        "text": "たなかさんは　かいしゃいんです。まいにち　9じから　6じまで　はたらきます。どようびと　にちようびは　やすみです。やすみの　ひは　テニスを　します。",
        "question": "たなかさんは　いつ　テニスを　しますか。",
        "options": [
          "げつようび",
          "すいようび",
          "きんようび",
          "どようびか　にちようび"
        ],
        "answer": 1,
        "id": "N5-03-049"
      },
      {
        "type": "読解（中文）",
        "text": "わたしは　にほんごがっこうの　がくせいです。クラスには　15にんの　がくせいが　います。アメリカじんが　5にん、ちゅうごくじんが　6にん、かんこくじんが　4にん　います。",
        "question": "クラスで　いちばん　おおいのは　どこの　ひとですか。",
        "options": [
          "アメリカじん",
          "ちゅうごくじん",
          "かんこくじん",
          "にほんじん"
        ],
        "answer": 1,
        "id": "N5-03-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-03-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "プール",
          "動物園",
          "水族館",
          "遊園地"
        ],
        "answer": 0,
        "id": "N5-03-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "体育館",
          "プール",
          "動物園",
          "水族館"
        ],
        "answer": 2,
        "id": "N5-03-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "市役所",
          "体育館",
          "プール",
          "動物園"
        ],
        "answer": 0,
        "id": "N5-03-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "交番",
          "市役所",
          "体育館",
          "プール"
        ],
        "answer": 2,
        "id": "N5-03-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "駐車場",
          "交番",
          "市役所",
          "体育館"
        ],
        "answer": 0,
        "id": "N5-03-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "本屋",
          "駐車場",
          "交番",
          "市役所"
        ],
        "answer": 2,
        "id": "N5-03-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "午前1時",
          "午前1時半",
          "午後1時",
          "午後1時半"
        ],
        "answer": 0,
        "id": "N5-03-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "午前2時",
          "午前2時半",
          "午後2時",
          "午後2時半"
        ],
        "answer": 1,
        "id": "N5-03-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "午前3時",
          "午前3時半",
          "午後3時",
          "午後3時半"
        ],
        "answer": 0,
        "id": "N5-03-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "午前4時",
          "午前4時半",
          "午後4時",
          "午後4時半"
        ],
        "answer": 1,
        "id": "N5-03-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "午前5時",
          "午前5時半",
          "午後5時",
          "午後5時半"
        ],
        "answer": 0,
        "id": "N5-03-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "午前6時",
          "午前6時半",
          "午後6時",
          "午後6時半"
        ],
        "answer": 1,
        "id": "N5-03-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "おめでとうございます",
          "ありがとうございます",
          "恐れ入ります",
          "恐縮です"
        ],
        "answer": 0,
        "id": "N5-03-064",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "お大事に",
          "お気をつけて",
          "ご無事で",
          "お元気で"
        ],
        "answer": 0,
        "id": "N5-03-065",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "失礼します",
          "失礼しました",
          "すみません",
          "申し訳ありません"
        ],
        "answer": 0,
        "id": "N5-03-066",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "ごめんなさい",
          "申し訳ございません",
          "お詫びします",
          "許してください"
        ],
        "answer": 0,
        "id": "N5-03-067",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "どういたしまして",
          "いいえ",
          "こちらこそ",
          "とんでもないです"
        ],
        "answer": 0,
        "id": "N5-03-068",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "林です",
          "斎藤です",
          "清水です",
          "山崎です"
        ],
        "answer": 0,
        "id": "N5-03-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "斎藤です",
          "清水です",
          "山崎です",
          "池田です"
        ],
        "answer": 0,
        "id": "N5-03-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "清水です",
          "山崎です",
          "池田です",
          "森です"
        ],
        "answer": 0,
        "id": "N5-03-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "山崎です",
          "池田です",
          "森です",
          "阿部です"
        ],
        "answer": 0,
        "id": "N5-03-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "池田です",
          "森です",
          "阿部です",
          "石川です"
        ],
        "answer": 0,
        "id": "N5-03-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "森です",
          "阿部です",
          "石川です",
          "前田です"
        ],
        "answer": 0,
        "id": "N5-03-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "阿部です",
          "石川です",
          "前田です",
          "藤田です"
        ],
        "answer": 0,
        "id": "N5-03-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "石川です",
          "前田です",
          "藤田です",
          "小川です"
        ],
        "answer": 0,
        "id": "N5-03-076",
        "script": "なんさいですか。"
      }
    ],
    "4": [
      {
        "type": "漢字読み",
        "text": "<u>見える</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "みえる",
          "あめ",
          "じょうず",
          "ごぜん"
        ],
        "answer": 0,
        "id": "N5-04-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>目</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "いろ",
          "じゅうぶん",
          "め",
          "がっこう"
        ],
        "answer": 2,
        "id": "N5-04-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>千</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "いく",
          "でぐち",
          "せん",
          "いつつ"
        ],
        "answer": 2,
        "id": "N5-04-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>火曜日</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "はいる",
          "みる",
          "しゅつりょく",
          "かようび"
        ],
        "answer": 3,
        "id": "N5-04-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>力</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "らいしゅう",
          "ちから",
          "いろ",
          "げんき"
        ],
        "answer": 1,
        "id": "N5-04-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>作る</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "つくる",
          "き",
          "きんようび",
          "おしえる"
        ],
        "answer": 0,
        "id": "N5-04-006"
      },
      {
        "type": "表記",
        "text": "<u>らいげつ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "分ける",
          "外国",
          "道",
          "来月"
        ],
        "answer": 3,
        "id": "N5-04-007"
      },
      {
        "type": "表記",
        "text": "<u>まいしゅう</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "毎週",
          "書く",
          "日曜日",
          "一本"
        ],
        "answer": 0,
        "id": "N5-04-008"
      },
      {
        "type": "表記",
        "text": "<u>すく</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "鳥",
          "天気",
          "空く",
          "一本"
        ],
        "answer": 2,
        "id": "N5-04-009"
      },
      {
        "type": "表記",
        "text": "<u>どようび</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "外国",
          "上",
          "今年",
          "土曜日"
        ],
        "answer": 3,
        "id": "N5-04-010"
      },
      {
        "type": "表記",
        "text": "<u>みる</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "午前",
          "見る",
          "午後",
          "聞く"
        ],
        "answer": 1,
        "id": "N5-04-011"
      },
      {
        "type": "表記",
        "text": "<u>なか</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "時計",
          "出る",
          "お母さん",
          "中"
        ],
        "answer": 3,
        "id": "N5-04-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "かばん",
          "くるま",
          "みかん",
          "でんわ"
        ],
        "answer": 2,
        "id": "N5-04-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "おちゃ",
          "ぼうし",
          "くつ",
          "めがね"
        ],
        "answer": 0,
        "id": "N5-04-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "でんわ",
          "いす",
          "そば",
          "テレビ"
        ],
        "answer": 2,
        "id": "N5-04-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "バス",
          "てんぷら",
          "えき",
          "びょういん"
        ],
        "answer": 1,
        "id": "N5-04-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ほん",
          "がっこう",
          "おちゃ",
          "みかん"
        ],
        "answer": 1,
        "id": "N5-04-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "いえ",
          "テレビ",
          "ほん",
          "おちゃ"
        ],
        "answer": 0,
        "id": "N5-04-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みせ",
          "テレビ",
          "ほん",
          "パン"
        ],
        "answer": 0,
        "id": "N5-04-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "テレビ",
          "やま",
          "みず",
          "えんぴつ"
        ],
        "answer": 1,
        "id": "N5-04-020"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>行く<rt>いく</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "shirt",
          "four things",
          "sofa",
          "to go"
        ],
        "answer": 3,
        "id": "N5-04-021"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>一本<rt>いっぽん</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "one (long)",
          "mountain",
          "table",
          "good at"
        ],
        "answer": 0,
        "id": "N5-04-022"
      },
      {
        "type": "言い換え",
        "text": "「カメラ」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "camera",
          "convenience store",
          "telephone",
          "person"
        ],
        "answer": 0,
        "id": "N5-04-023"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>名前<rt>なまえ</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "name",
          "enroll",
          "grandfather (polite)",
          "company"
        ],
        "answer": 0,
        "id": "N5-04-024"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>一人<rt>ひとり</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "one person",
          "that",
          "sorry",
          "music"
        ],
        "answer": 0,
        "id": "N5-04-025"
      },
      {
        "type": "文法形式",
        "text": "カフェ（　）コーヒーを　のみます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "に",
          "が",
          "を"
        ],
        "answer": 0,
        "id": "N5-04-026"
      },
      {
        "type": "文法形式",
        "text": "スーパー（　）かいものします。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "に",
          "が",
          "を"
        ],
        "answer": 0,
        "id": "N5-04-027"
      },
      {
        "type": "文法形式",
        "text": "びょういん（　）いきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "を",
          "が",
          "で"
        ],
        "answer": 0,
        "id": "N5-04-028"
      },
      {
        "type": "文法形式",
        "text": "ゆうびんきょく（　）てがみを　だします。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "を",
          "に",
          "が"
        ],
        "answer": 0,
        "id": "N5-04-029"
      },
      {
        "type": "文法形式",
        "text": "ぎんこう（　）おかねを　おろします。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "が",
          "に",
          "を"
        ],
        "answer": 0,
        "id": "N5-04-030"
      },
      {
        "type": "文法形式",
        "text": "としょかん（　）ほんを　かります。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "に",
          "を",
          "が"
        ],
        "answer": 0,
        "id": "N5-04-031"
      },
      {
        "type": "文法形式",
        "text": "バス（　）まちます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "で",
          "が",
          "に"
        ],
        "answer": 0,
        "id": "N5-04-032"
      },
      {
        "type": "文法形式",
        "text": "みち（　）あるきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "に",
          "で"
        ],
        "answer": 0,
        "id": "N5-04-033"
      },
      {
        "type": "文法形式",
        "text": "はし（　）わたります。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "が",
          "で"
        ],
        "answer": 0,
        "id": "N5-04-034"
      },
      {
        "type": "文法形式",
        "text": "かど（　）まがります。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "で",
          "が"
        ],
        "answer": 0,
        "id": "N5-04-035"
      },
      {
        "type": "文の組み立て",
        "text": "あねは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "えいがを",
          "みます",
          "は",
          "あね"
        ],
        "answer": 1,
        "id": "N5-04-036"
      },
      {
        "type": "文の組み立て",
        "text": "せんせいは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "にほんごを",
          "おしえます",
          "は",
          "せんせい"
        ],
        "answer": 1,
        "id": "N5-04-037"
      },
      {
        "type": "文の組み立て",
        "text": "がくせいは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "べんきょうを",
          "します",
          "は",
          "がくせい"
        ],
        "answer": 1,
        "id": "N5-04-038"
      },
      {
        "type": "文の組み立て",
        "text": "かれは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "テニスを",
          "します",
          "は",
          "かれ"
        ],
        "answer": 1,
        "id": "N5-04-039"
      },
      {
        "type": "文の組み立て",
        "text": "かのじょは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "ピアノを",
          "ひきます",
          "は",
          "かのじょ"
        ],
        "answer": 1,
        "id": "N5-04-040"
      },
      {
        "type": "文章の文法",
        "text": "えきまで あるいて 10ぷん（　）かかります。",
        "question": "（　）に　はいるのは？",
        "options": [
          "ぐらい",
          "だけ",
          "しか",
          "まで"
        ],
        "answer": 0,
        "id": "N5-04-041"
      },
      {
        "type": "文章の文法",
        "text": "この かばんは かる（　）、べんりです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "くて",
          "いで",
          "いと",
          "いから"
        ],
        "answer": 0,
        "id": "N5-04-042"
      },
      {
        "type": "文章の文法",
        "text": "にほんごは むずかしい（　）、おもしろいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "けど",
          "から",
          "ので",
          "のに"
        ],
        "answer": 0,
        "id": "N5-04-043"
      },
      {
        "type": "文章の文法",
        "text": "わたしは りょこうが すき（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "です",
          "ます",
          "だ",
          "な"
        ],
        "answer": 0,
        "id": "N5-04-044"
      },
      {
        "type": "文章の文法",
        "text": "あの たてものは びょういん（　）ホテルです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "では なくて",
          "じゃなくて",
          "ではない",
          "じゃない"
        ],
        "answer": 0,
        "id": "N5-04-045"
      },
      {
        "type": "読解（短文）",
        "text": "えきから　がっこうまで　バスで　20ぷんです。あるくと　1じかんです。",
        "question": "バスで　なんぷん　かかりますか。",
        "options": [
          "10ぷん",
          "20ぷん",
          "30ぷん",
          "1じかん"
        ],
        "answer": 1,
        "id": "N5-04-046"
      },
      {
        "type": "読解（短文）",
        "text": "やまださんには　おにいさんが　1にんと　いもうとが　2にん　います。",
        "question": "やまださんには　きょうだいが　なんにん　いますか。",
        "options": [
          "1にん",
          "2にん",
          "3にん",
          "4にん"
        ],
        "answer": 1,
        "id": "N5-04-047"
      },
      {
        "type": "読解（短文）",
        "text": "わたしの　かぞくは　5にんです。ちちと　ははと　あにと　わたしと　おとうとです。",
        "question": "かぞくは　なんにんですか。",
        "options": [
          "3にん",
          "4にん",
          "5にん",
          "6にん"
        ],
        "answer": 1,
        "id": "N5-04-048"
      },
      {
        "type": "読解（中文）",
        "text": "きょうの　しゅくだいは　かんじを　20こ　かくことです。あしたまでに　ださなければなりません。かんじは　むずかしいですが、がんばります。",
        "question": "しゅくだいは　なんですか。",
        "options": [
          "ほんを　よむ",
          "かんじを　かく",
          "さくぶんを　かく",
          "テストの　べんきょう"
        ],
        "answer": 1,
        "id": "N5-04-049"
      },
      {
        "type": "読解（中文）",
        "text": "わたしの　がっこうには　としょかんが　あります。ほんが　たくさん　あります。まいにち　ひるやすみに　としょかんで　ほんを　よみます。",
        "question": "いつ　としょかんに　いきますか。",
        "options": [
          "あさ",
          "ひるやすみ",
          "よる",
          "しゅうまつ"
        ],
        "answer": 1,
        "id": "N5-04-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-04-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "薬局",
          "本屋",
          "駐車場",
          "交番"
        ],
        "answer": 0,
        "id": "N5-04-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "空港",
          "薬局",
          "本屋",
          "駐車場"
        ],
        "answer": 2,
        "id": "N5-04-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "映画館",
          "空港",
          "薬局",
          "本屋"
        ],
        "answer": 0,
        "id": "N5-04-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "博物館",
          "映画館",
          "空港",
          "薬局"
        ],
        "answer": 2,
        "id": "N5-04-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "美術館",
          "博物館",
          "映画館",
          "空港"
        ],
        "answer": 0,
        "id": "N5-04-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "ホテル",
          "美術館",
          "博物館",
          "映画館"
        ],
        "answer": 2,
        "id": "N5-04-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "午前7時",
          "午前7時半",
          "午後7時",
          "午後7時半"
        ],
        "answer": 0,
        "id": "N5-04-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "午前8時",
          "午前8時半",
          "午後8時",
          "午後8時半"
        ],
        "answer": 1,
        "id": "N5-04-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "午前9時",
          "午前9時半",
          "午後9時",
          "午後9時半"
        ],
        "answer": 0,
        "id": "N5-04-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "午前10時",
          "午前10時半",
          "午後10時",
          "午後10時半"
        ],
        "answer": 1,
        "id": "N5-04-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "午前11時",
          "午前11時半",
          "午後11時",
          "午後11時半"
        ],
        "answer": 0,
        "id": "N5-04-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "午前12時",
          "午前12時半",
          "午後12時",
          "午後12時半"
        ],
        "answer": 1,
        "id": "N5-04-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "お待たせしました",
          "お待ちください",
          "もう少しです",
          "準備中です"
        ],
        "answer": 0,
        "id": "N5-04-064",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "頑張ってください",
          "応援しています",
          "期待しています",
          "楽しみです"
        ],
        "answer": 0,
        "id": "N5-04-065",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "お体に気をつけて",
          "ご自愛ください",
          "お元気で",
          "良い一日を"
        ],
        "answer": 0,
        "id": "N5-04-066",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "よい週末を",
          "よいお年を",
          "よい旅を",
          "よい休日を"
        ],
        "answer": 0,
        "id": "N5-04-067",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "乾杯",
          "お疲れ様",
          "いただきます",
          "ごちそうさま"
        ],
        "answer": 0,
        "id": "N5-04-068",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "前田です",
          "藤田です",
          "小川です",
          "後藤です"
        ],
        "answer": 0,
        "id": "N5-04-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "藤田です",
          "小川です",
          "後藤です",
          "岡田です"
        ],
        "answer": 0,
        "id": "N5-04-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "小川です",
          "後藤です",
          "岡田です",
          "村上です"
        ],
        "answer": 0,
        "id": "N5-04-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "東京です",
          "大阪です",
          "京都です",
          "名古屋です"
        ],
        "answer": 0,
        "id": "N5-04-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "大阪です",
          "京都です",
          "名古屋です",
          "福岡です"
        ],
        "answer": 0,
        "id": "N5-04-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "京都です",
          "名古屋です",
          "福岡です",
          "札幌です"
        ],
        "answer": 0,
        "id": "N5-04-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "名古屋です",
          "福岡です",
          "札幌です",
          "横浜です"
        ],
        "answer": 0,
        "id": "N5-04-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "福岡です",
          "札幌です",
          "横浜です",
          "神戸です"
        ],
        "answer": 0,
        "id": "N5-04-076",
        "script": "なんさいですか。"
      }
    ],
    "5": [
      {
        "type": "漢字読み",
        "text": "<u>今月</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "かいけい",
          "せん",
          "おとうさん",
          "こんげつ"
        ],
        "answer": 3,
        "id": "N5-05-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>男の子</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "せんせい",
          "おちゃ",
          "おとこ",
          "おとこのこ"
        ],
        "answer": 3,
        "id": "N5-05-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>月</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "なか",
          "ごぜん",
          "ちいさい",
          "つき"
        ],
        "answer": 3,
        "id": "N5-05-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>今年</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "くる",
          "おちゃ",
          "ことし",
          "ひとり"
        ],
        "answer": 2,
        "id": "N5-05-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>絵</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "え",
          "みぎ",
          "そと",
          "ひと"
        ],
        "answer": 0,
        "id": "N5-05-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>下がる</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "にゅうりょく",
          "はなす",
          "てんき",
          "さがる"
        ],
        "answer": 3,
        "id": "N5-05-006"
      },
      {
        "type": "表記",
        "text": "<u>しゅつりょく</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "中",
          "音楽",
          "出力",
          "お金"
        ],
        "answer": 2,
        "id": "N5-05-007"
      },
      {
        "type": "表記",
        "text": "<u>やすむ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "お茶",
          "休む",
          "前",
          "元気"
        ],
        "answer": 1,
        "id": "N5-05-008"
      },
      {
        "type": "表記",
        "text": "<u>そら</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "下がる",
          "見つける",
          "空",
          "下手"
        ],
        "answer": 2,
        "id": "N5-05-009"
      },
      {
        "type": "表記",
        "text": "<u>かたち</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "土曜日",
          "同じ",
          "上",
          "形"
        ],
        "answer": 3,
        "id": "N5-05-010"
      },
      {
        "type": "表記",
        "text": "<u>よむ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "後で",
          "読む",
          "間",
          "鳥"
        ],
        "answer": 1,
        "id": "N5-05-011"
      },
      {
        "type": "表記",
        "text": "<u>さかな</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "魚",
          "雪",
          "元気",
          "来週"
        ],
        "answer": 0,
        "id": "N5-05-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "やさい",
          "いす",
          "えき",
          "くるま"
        ],
        "answer": 0,
        "id": "N5-05-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みず",
          "えんぴつ",
          "ぼうし",
          "めがね"
        ],
        "answer": 0,
        "id": "N5-05-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "でんしゃ",
          "かさ",
          "いす",
          "カレー"
        ],
        "answer": 3,
        "id": "N5-05-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "つくえ",
          "やさい",
          "くるま",
          "かさ"
        ],
        "answer": 1,
        "id": "N5-05-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ほん",
          "がっこう",
          "りんご",
          "パン"
        ],
        "answer": 1,
        "id": "N5-05-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みず",
          "テレビ",
          "みかん",
          "へや"
        ],
        "answer": 3,
        "id": "N5-05-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "スーパー",
          "テレビ",
          "パン",
          "みず"
        ],
        "answer": 0,
        "id": "N5-05-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "こうえん",
          "おちゃ",
          "テレビ",
          "えんぴつ"
        ],
        "answer": 0,
        "id": "N5-05-020"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>鳥<rt>とり</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "black",
          "bird",
          "to wear (glasses)",
          "power"
        ],
        "answer": 1,
        "id": "N5-05-021"
      },
      {
        "type": "言い換え",
        "text": "「ズボン」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "thousand",
          "same",
          "pants",
          "mountain"
        ],
        "answer": 2,
        "id": "N5-05-022"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>五つ<rt>いつつ</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "five things",
          "to know",
          "same",
          "to exit/leave"
        ],
        "answer": 0,
        "id": "N5-05-023"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>下がる<rt>さがる</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "that",
          "five things",
          "tomorrow",
          "to go down"
        ],
        "answer": 3,
        "id": "N5-05-024"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>お茶<rt>おちゃ</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "weather",
          "supermarket",
          "tea",
          "to turn on"
        ],
        "answer": 2,
        "id": "N5-05-025"
      },
      {
        "type": "文法形式",
        "text": "いえ（　）かえります。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "で",
          "を",
          "が"
        ],
        "answer": 0,
        "id": "N5-05-026"
      },
      {
        "type": "文法形式",
        "text": "くに（　）かえります。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "で",
          "を",
          "が"
        ],
        "answer": 0,
        "id": "N5-05-027"
      },
      {
        "type": "文法形式",
        "text": "かいしゃ（　）つとめています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "が",
          "で",
          "を"
        ],
        "answer": 0,
        "id": "N5-05-028"
      },
      {
        "type": "文法形式",
        "text": "だいがく（　）かよっています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "を",
          "が",
          "で"
        ],
        "answer": 0,
        "id": "N5-05-029"
      },
      {
        "type": "文法形式",
        "text": "アパート（　）すんでいます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "で",
          "が",
          "を"
        ],
        "answer": 0,
        "id": "N5-05-030"
      },
      {
        "type": "文法形式",
        "text": "わたし（　）にほんじんです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "は",
          "を",
          "に",
          "が"
        ],
        "answer": 0,
        "id": "N5-05-031"
      },
      {
        "type": "文法形式",
        "text": "これ（　）わたしの　かばんです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "は",
          "が",
          "を",
          "に"
        ],
        "answer": 0,
        "id": "N5-05-032"
      },
      {
        "type": "文法形式",
        "text": "あの　ひと（　）だれですか。",
        "question": "（　）に　はいるのは？",
        "options": [
          "は",
          "が",
          "を",
          "に"
        ],
        "answer": 0,
        "id": "N5-05-033"
      },
      {
        "type": "文法形式",
        "text": "きょう（　）あついですね。",
        "question": "（　）に　はいるのは？",
        "options": [
          "は",
          "が",
          "に",
          "を"
        ],
        "answer": 0,
        "id": "N5-05-034"
      },
      {
        "type": "文法形式",
        "text": "にほんご（　）むずかしいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "は",
          "を",
          "に",
          "が"
        ],
        "answer": 0,
        "id": "N5-05-035"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "えきに",
          "いきます",
          "から",
          "うち"
        ],
        "answer": 1,
        "id": "N5-05-036"
      },
      {
        "type": "文の組み立て",
        "text": "でんしゃは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "とうきょうに",
          "つきます",
          "から",
          "おおさか"
        ],
        "answer": 1,
        "id": "N5-05-037"
      },
      {
        "type": "文の組み立て",
        "text": "ひこうきは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "なりたに",
          "つきます",
          "から",
          "ニューヨーク"
        ],
        "answer": 1,
        "id": "N5-05-038"
      },
      {
        "type": "文の組み立て",
        "text": "バスは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "がっこうに",
          "いきます",
          "から",
          "えき"
        ],
        "answer": 1,
        "id": "N5-05-039"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "かいしゃに",
          "いきます",
          "から",
          "うち"
        ],
        "answer": 1,
        "id": "N5-05-040"
      },
      {
        "type": "文章の文法",
        "text": "きのう デパートで くつ（　）かいました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "は",
          "に"
        ],
        "answer": 0,
        "id": "N5-05-041"
      },
      {
        "type": "文章の文法",
        "text": "この ケーキは やす（　）おいしいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "くて",
          "いで",
          "いと",
          "くない"
        ],
        "answer": 0,
        "id": "N5-05-042"
      },
      {
        "type": "文章の文法",
        "text": "あした ともだち（　）あいます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "と",
          "を",
          "が"
        ],
        "answer": 0,
        "id": "N5-05-043"
      },
      {
        "type": "文章の文法",
        "text": "わたしは まいにち 8じ（　）おきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "で",
          "を",
          "が"
        ],
        "answer": 0,
        "id": "N5-05-044"
      },
      {
        "type": "文章の文法",
        "text": "にほんご（　）べんきょうしています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "は",
          "に"
        ],
        "answer": 0,
        "id": "N5-05-045"
      },
      {
        "type": "読解（短文）",
        "text": "たなかさんは　25さいです。おとうとは　20さいです。",
        "question": "おとうとは　なんさいですか。",
        "options": [
          "15さい",
          "20さい",
          "25さい",
          "30さい"
        ],
        "answer": 1,
        "id": "N5-05-046"
      },
      {
        "type": "読解（短文）",
        "text": "わたしの　せんせいは　にほんじんです。えいごと　ちゅうごくごが　できます。",
        "question": "せんせいは　どこの　ひとですか。",
        "options": [
          "アメリカじん",
          "ちゅうごくじん",
          "にほんじん",
          "かんこくじん"
        ],
        "answer": 1,
        "id": "N5-05-047"
      },
      {
        "type": "読解（短文）",
        "text": "すずきさんは　かいしゃいんです。まいにち　でんしゃで　かいしゃに　いきます。",
        "question": "すずきさんは　なにを　していますか。",
        "options": [
          "がくせい",
          "せんせい",
          "かいしゃいん",
          "いしゃ"
        ],
        "answer": 1,
        "id": "N5-05-048"
      },
      {
        "type": "読解（中文）",
        "text": "せんせいは　とても　やさしいです。わからないとき、ていねいに　おしえて　くれます。にほんごの　べんきょうが　すきに　なりました。",
        "question": "せんせいは　どんな　ひとですか。",
        "options": [
          "きびしい",
          "やさしい",
          "こわい",
          "つまらない"
        ],
        "answer": 1,
        "id": "N5-05-049"
      },
      {
        "type": "読解（中文）",
        "text": "らいしゅう　しけんが　あります。にほんごの　しけんです。まいにち　3じかん　べんきょうしています。いい　てんを　とりたいです。",
        "question": "なんの　しけんですか。",
        "options": [
          "えいご",
          "すうがく",
          "にほんご",
          "かがく"
        ],
        "answer": 1,
        "id": "N5-05-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-05-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "カフェ",
          "ホテル",
          "美術館",
          "博物館"
        ],
        "answer": 0,
        "id": "N5-05-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "レストラン",
          "カフェ",
          "ホテル",
          "美術館"
        ],
        "answer": 2,
        "id": "N5-05-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "会社",
          "レストラン",
          "カフェ",
          "ホテル"
        ],
        "answer": 0,
        "id": "N5-05-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "学校",
          "会社",
          "レストラン",
          "カフェ"
        ],
        "answer": 2,
        "id": "N5-05-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "公園",
          "学校",
          "会社",
          "レストラン"
        ],
        "answer": 0,
        "id": "N5-05-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "デパート",
          "公園",
          "学校",
          "会社"
        ],
        "answer": 2,
        "id": "N5-05-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "1:00",
          "1:10",
          "1:20",
          "1:30"
        ],
        "answer": 0,
        "id": "N5-05-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "2:00",
          "2:10",
          "2:20",
          "2:30"
        ],
        "answer": 1,
        "id": "N5-05-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "3:00",
          "3:10",
          "3:20",
          "3:30"
        ],
        "answer": 0,
        "id": "N5-05-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "4:00",
          "4:10",
          "4:20",
          "4:30"
        ],
        "answer": 1,
        "id": "N5-05-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "5:00",
          "5:10",
          "5:20",
          "5:30"
        ],
        "answer": 0,
        "id": "N5-05-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "6:00",
          "6:10",
          "6:20",
          "6:30"
        ],
        "answer": 1,
        "id": "N5-05-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "はい",
          "いいえ",
          "わかりました",
          "わかりません"
        ],
        "answer": 0,
        "id": "N5-05-064",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "そうです",
          "違います",
          "そうですか",
          "ほんとうですか"
        ],
        "answer": 0,
        "id": "N5-05-065",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "いいですよ",
          "だめです",
          "ちょっと...",
          "無理です"
        ],
        "answer": 0,
        "id": "N5-05-066",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "もちろん",
          "当然です",
          "絶対に",
          "きっと"
        ],
        "answer": 0,
        "id": "N5-05-067",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "たぶん",
          "おそらく",
          "かもしれません",
          "でしょう"
        ],
        "answer": 0,
        "id": "N5-05-068",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "札幌です",
          "横浜です",
          "神戸です",
          "仙台です"
        ],
        "answer": 0,
        "id": "N5-05-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "横浜です",
          "神戸です",
          "仙台です",
          "広島です"
        ],
        "answer": 0,
        "id": "N5-05-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "神戸です",
          "仙台です",
          "広島です",
          "北海道です"
        ],
        "answer": 0,
        "id": "N5-05-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "仙台です",
          "広島です",
          "北海道です",
          "沖縄です"
        ],
        "answer": 0,
        "id": "N5-05-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "広島です",
          "北海道です",
          "沖縄です",
          "奈良です"
        ],
        "answer": 0,
        "id": "N5-05-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "北海道です",
          "沖縄です",
          "奈良です",
          "鎌倉です"
        ],
        "answer": 0,
        "id": "N5-05-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "沖縄です",
          "奈良です",
          "鎌倉です",
          "箱根です"
        ],
        "answer": 0,
        "id": "N5-05-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "奈良です",
          "鎌倉です",
          "箱根です",
          "日光です"
        ],
        "answer": 0,
        "id": "N5-05-076",
        "script": "なんさいですか。"
      }
    ],
    "6": [
      {
        "type": "漢字読み",
        "text": "<u>正しい</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "ただしい",
          "わける",
          "あし",
          "おんがく"
        ],
        "answer": 0,
        "id": "N5-06-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>人</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "おみず",
          "ひと",
          "でんわ",
          "はな"
        ],
        "answer": 1,
        "id": "N5-06-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>天気</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "へた",
          "え",
          "なまえ",
          "てんき"
        ],
        "answer": 3,
        "id": "N5-06-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>来月</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "らいげつ",
          "がいこく",
          "えき",
          "にゅうがく"
        ],
        "answer": 0,
        "id": "N5-06-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>一つ</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "まえ",
          "おちゃ",
          "こんげつ",
          "ひとつ"
        ],
        "answer": 3,
        "id": "N5-06-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>木曜日</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "ひとつ",
          "はな",
          "もくようび",
          "のむ"
        ],
        "answer": 2,
        "id": "N5-06-006"
      },
      {
        "type": "表記",
        "text": "<u>じゅうぶん</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "花",
          "車",
          "十分",
          "道"
        ],
        "answer": 2,
        "id": "N5-06-007"
      },
      {
        "type": "表記",
        "text": "<u>だいがく</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "出る",
          "生きる",
          "電話",
          "大学"
        ],
        "answer": 3,
        "id": "N5-06-008"
      },
      {
        "type": "表記",
        "text": "<u>たかい</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "大人",
          "百",
          "間",
          "高い"
        ],
        "answer": 3,
        "id": "N5-06-009"
      },
      {
        "type": "表記",
        "text": "<u>ふん</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "間",
          "口",
          "分",
          "下"
        ],
        "answer": 2,
        "id": "N5-06-010"
      },
      {
        "type": "表記",
        "text": "<u>でんしゃ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "電車",
          "森",
          "毎年",
          "午前"
        ],
        "answer": 0,
        "id": "N5-06-011"
      },
      {
        "type": "表記",
        "text": "<u>でる</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "電車",
          "一つ",
          "口",
          "出る"
        ],
        "answer": 3,
        "id": "N5-06-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "たまご",
          "かばん",
          "でんわ",
          "つくえ"
        ],
        "answer": 0,
        "id": "N5-06-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ズボン",
          "えんぴつ",
          "ジュース",
          "ノート"
        ],
        "answer": 2,
        "id": "N5-06-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "びょういん",
          "ラーメン",
          "かさ",
          "かばん"
        ],
        "answer": 1,
        "id": "N5-06-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "かばん",
          "くるま",
          "テレビ",
          "てんぷら"
        ],
        "answer": 3,
        "id": "N5-06-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "おちゃ",
          "がっこう",
          "みかん",
          "ほん"
        ],
        "answer": 1,
        "id": "N5-06-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "テレビ",
          "きょうしつ",
          "パン",
          "おちゃ"
        ],
        "answer": 1,
        "id": "N5-06-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みかん",
          "えんぴつ",
          "りんご",
          "スーパー"
        ],
        "answer": 3,
        "id": "N5-06-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みず",
          "えいがかん",
          "ほん",
          "みかん"
        ],
        "answer": 1,
        "id": "N5-06-020"
      },
      {
        "type": "言い換え",
        "text": "「くれる」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "to give (to me)",
          "enroll",
          "various",
          "fish"
        ],
        "answer": 0,
        "id": "N5-06-021"
      },
      {
        "type": "言い換え",
        "text": "「あげる」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "every week",
          "to give",
          "that (far)",
          "adult"
        ],
        "answer": 1,
        "id": "N5-06-022"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>毎日<rt>まいにち</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "how much",
          "behind",
          "more",
          "every day"
        ],
        "answer": 3,
        "id": "N5-06-023"
      },
      {
        "type": "言い換え",
        "text": "「マフラー」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "necktie",
          "scarf",
          "to wear (glasses)",
          "backpack"
        ],
        "answer": 1,
        "id": "N5-06-024"
      },
      {
        "type": "言い換え",
        "text": "「もらう」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "to receive",
          "hotel",
          "why",
          "water"
        ],
        "answer": 0,
        "id": "N5-06-025"
      },
      {
        "type": "文法形式",
        "text": "この　りんご（　）おいしいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "は",
          "を",
          "に",
          "が"
        ],
        "answer": 0,
        "id": "N5-06-026"
      },
      {
        "type": "文法形式",
        "text": "あの　えいが（　）おもしろかったです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "は",
          "が",
          "に",
          "を"
        ],
        "answer": 0,
        "id": "N5-06-027"
      },
      {
        "type": "文法形式",
        "text": "きのう（　）あめでした。",
        "question": "（　）に　はいるのは？",
        "options": [
          "は",
          "が",
          "を",
          "に"
        ],
        "answer": 0,
        "id": "N5-06-028"
      },
      {
        "type": "文法形式",
        "text": "らいしゅう（　）テストがあります。",
        "question": "（　）に　はいるのは？",
        "options": [
          "は",
          "に",
          "が",
          "を"
        ],
        "answer": 0,
        "id": "N5-06-029"
      },
      {
        "type": "文法形式",
        "text": "わたし（　）さかなが　すきです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "は",
          "が",
          "を",
          "に"
        ],
        "answer": 0,
        "id": "N5-06-030"
      },
      {
        "type": "文法形式",
        "text": "あめ（　）ふっています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "に",
          "は",
          "を"
        ],
        "answer": 0,
        "id": "N5-06-031"
      },
      {
        "type": "文法形式",
        "text": "つくえの　うえに　ほん（　）あります。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "は",
          "に"
        ],
        "answer": 0,
        "id": "N5-06-032"
      },
      {
        "type": "文法形式",
        "text": "わたしは　コーヒー（　）すきです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "は",
          "を",
          "に"
        ],
        "answer": 0,
        "id": "N5-06-033"
      },
      {
        "type": "文法形式",
        "text": "やまださんは　えいご（　）じょうずです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "に",
          "は"
        ],
        "answer": 0,
        "id": "N5-06-034"
      },
      {
        "type": "文法形式",
        "text": "わたしは　あたま（　）いたいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "は",
          "に"
        ],
        "answer": 0,
        "id": "N5-06-035"
      },
      {
        "type": "文の組み立て",
        "text": "きのう ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "えいがを",
          "みました",
          "は",
          "わたし"
        ],
        "answer": 0,
        "id": "N5-06-036"
      },
      {
        "type": "文の組み立て",
        "text": "せんしゅう ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "りょこうを",
          "しました",
          "は",
          "かぞく"
        ],
        "answer": 0,
        "id": "N5-06-037"
      },
      {
        "type": "文の組み立て",
        "text": "おととい ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "てがみを",
          "かきました",
          "は",
          "わたし"
        ],
        "answer": 0,
        "id": "N5-06-038"
      },
      {
        "type": "文の組み立て",
        "text": "けさ ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "ごはんを",
          "たべました",
          "は",
          "わたし"
        ],
        "answer": 0,
        "id": "N5-06-039"
      },
      {
        "type": "文の組み立て",
        "text": "ゆうべ ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "テレビを",
          "みました",
          "は",
          "かぞく"
        ],
        "answer": 0,
        "id": "N5-06-040"
      },
      {
        "type": "文章の文法",
        "text": "この みせは やすくて、（　）がいいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "あじ",
          "あじが",
          "あじの",
          "あじに"
        ],
        "answer": 0,
        "id": "N5-06-041"
      },
      {
        "type": "文章の文法",
        "text": "かれは せが（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "たかい",
          "たかく",
          "たかいな",
          "たかいに"
        ],
        "answer": 0,
        "id": "N5-06-042"
      },
      {
        "type": "文章の文法",
        "text": "まいあさ 6じに おき（　）、ジョギングをします。",
        "question": "（　）に　はいるのは？",
        "options": [
          "て",
          "ます",
          "た",
          "る"
        ],
        "answer": 0,
        "id": "N5-06-043"
      },
      {
        "type": "文章の文法",
        "text": "この もんだいは（　）ないです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "むずかしく",
          "むずかしい",
          "むずかしくて",
          "むずかしいで"
        ],
        "answer": 0,
        "id": "N5-06-044"
      },
      {
        "type": "文章の文法",
        "text": "きょうは さむい（　）、コートをきました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "ので",
          "から",
          "けど",
          "のに"
        ],
        "answer": 0,
        "id": "N5-06-045"
      },
      {
        "type": "読解（短文）",
        "text": "わたしは　りんごが　すきです。バナナも　すきです。でも　みかんは　きらいです。",
        "question": "わたしは　なにが　きらいですか。",
        "options": [
          "りんご",
          "バナナ",
          "みかん",
          "いちご"
        ],
        "answer": 1,
        "id": "N5-06-046"
      },
      {
        "type": "読解（短文）",
        "text": "やまださんの　しゅみは　どくしょです。まいにち　30ぷん　ほんを　よみます。",
        "question": "やまださんの　しゅみは　なんですか。",
        "options": [
          "りょこう",
          "どくしょ",
          "りょうり",
          "スポーツ"
        ],
        "answer": 1,
        "id": "N5-06-047"
      },
      {
        "type": "読解（短文）",
        "text": "わたしは　サッカーが　すきです。でも　やきゅうは　あまり　すきじゃないです。",
        "question": "わたしは　なにが　すきですか。",
        "options": [
          "やきゅう",
          "サッカー",
          "テニス",
          "すいえい"
        ],
        "answer": 1,
        "id": "N5-06-048"
      },
      {
        "type": "読解（中文）",
        "text": "きのう　かぞくと　うみに　いきました。てんきが　よくて、とても　きれいでした。およいだり、すなで　あそんだり　しました。たのしかったです。",
        "question": "きのう　どこに　いきましたか。",
        "options": [
          "やま",
          "うみ",
          "こうえん",
          "まち"
        ],
        "answer": 1,
        "id": "N5-06-049"
      },
      {
        "type": "読解（中文）",
        "text": "らいげつ　ともだちと　きょうとに　いきます。2はく3にちの　りょこうです。おてらや　じんじゃを　みたいです。",
        "question": "りょこうは　なんにちですか。",
        "options": [
          "1にち",
          "2にち",
          "3にち",
          "4にち"
        ],
        "answer": 1,
        "id": "N5-06-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-06-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "コンビニ",
          "デパート",
          "公園",
          "学校"
        ],
        "answer": 0,
        "id": "N5-06-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "スーパー",
          "コンビニ",
          "デパート",
          "公園"
        ],
        "answer": 2,
        "id": "N5-06-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "図書館",
          "スーパー",
          "コンビニ",
          "デパート"
        ],
        "answer": 0,
        "id": "N5-06-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "郵便局",
          "図書館",
          "スーパー",
          "コンビニ"
        ],
        "answer": 2,
        "id": "N5-06-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "銀行",
          "郵便局",
          "図書館",
          "スーパー"
        ],
        "answer": 0,
        "id": "N5-06-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "病院",
          "銀行",
          "郵便局",
          "図書館"
        ],
        "answer": 2,
        "id": "N5-06-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "7:00",
          "7:10",
          "7:20",
          "7:30"
        ],
        "answer": 0,
        "id": "N5-06-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "8:00",
          "8:10",
          "8:20",
          "8:30"
        ],
        "answer": 1,
        "id": "N5-06-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "9:00",
          "9:10",
          "9:20",
          "9:30"
        ],
        "answer": 0,
        "id": "N5-06-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "10:00",
          "10:10",
          "10:20",
          "10:30"
        ],
        "answer": 1,
        "id": "N5-06-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "11:00",
          "11:10",
          "11:20",
          "11:30"
        ],
        "answer": 0,
        "id": "N5-06-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "12:00",
          "12:10",
          "12:20",
          "12:30"
        ],
        "answer": 1,
        "id": "N5-06-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "そうですね",
          "そうかもしれません",
          "その通りです",
          "確かに"
        ],
        "answer": 0,
        "id": "N5-06-064",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "なるほど",
          "分かりました",
          "了解です",
          "承知しました"
        ],
        "answer": 0,
        "id": "N5-06-065",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "すごいですね",
          "素晴らしい",
          "素敵ですね",
          "良かったですね"
        ],
        "answer": 0,
        "id": "N5-06-066",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "残念ですね",
          "仕方ないですね",
          "困りましたね",
          "大変ですね"
        ],
        "answer": 0,
        "id": "N5-06-067",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "本当ですか",
          "信じられない",
          "びっくりです",
          "驚きました"
        ],
        "answer": 0,
        "id": "N5-06-068",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "鎌倉です",
          "箱根です",
          "日光です",
          "金沢です"
        ],
        "answer": 0,
        "id": "N5-06-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "箱根です",
          "日光です",
          "金沢です",
          "長崎です"
        ],
        "answer": 0,
        "id": "N5-06-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "日光です",
          "金沢です",
          "長崎です",
          "熊本です"
        ],
        "answer": 0,
        "id": "N5-06-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "金沢です",
          "長崎です",
          "熊本です",
          "新潟です"
        ],
        "answer": 0,
        "id": "N5-06-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "長崎です",
          "熊本です",
          "新潟です",
          "静岡です"
        ],
        "answer": 0,
        "id": "N5-06-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "熊本です",
          "新潟です",
          "静岡です",
          "岡山です"
        ],
        "answer": 0,
        "id": "N5-06-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "新潟です",
          "静岡です",
          "岡山です",
          "松山です"
        ],
        "answer": 0,
        "id": "N5-06-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "静岡です",
          "岡山です",
          "松山です",
          "高松です"
        ],
        "answer": 0,
        "id": "N5-06-076",
        "script": "なんさいですか。"
      }
    ],
    "7": [
      {
        "type": "漢字読み",
        "text": "<u>社長</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "しゃちょう",
          "ひゃく",
          "まいつき",
          "あした"
        ],
        "answer": 0,
        "id": "N5-07-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>午前</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "にちようび",
          "ごぜん",
          "うえ",
          "みち"
        ],
        "answer": 1,
        "id": "N5-07-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>黒い</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "あし",
          "みつける",
          "くろい",
          "おなじ"
        ],
        "answer": 2,
        "id": "N5-07-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>見つける</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "らいねん",
          "こんげつ",
          "みつける",
          "いっぽん"
        ],
        "answer": 2,
        "id": "N5-07-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>毎年</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "き",
          "した",
          "まいとし",
          "いきる"
        ],
        "answer": 2,
        "id": "N5-07-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>五つ</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "どようび",
          "いつつ",
          "おかあさん",
          "おおきい"
        ],
        "answer": 1,
        "id": "N5-07-006"
      },
      {
        "type": "表記",
        "text": "<u>きょうしつ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "土曜日",
          "水",
          "行く",
          "教室"
        ],
        "answer": 3,
        "id": "N5-07-007"
      },
      {
        "type": "表記",
        "text": "<u>あし</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "目",
          "毎月",
          "電車",
          "足"
        ],
        "answer": 3,
        "id": "N5-07-008"
      },
      {
        "type": "表記",
        "text": "<u>さがる</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "耳",
          "下がる",
          "先週",
          "大人"
        ],
        "answer": 1,
        "id": "N5-07-009"
      },
      {
        "type": "表記",
        "text": "<u>がいこく</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "毎月",
          "知る",
          "生まれる",
          "外国"
        ],
        "answer": 3,
        "id": "N5-07-010"
      },
      {
        "type": "表記",
        "text": "<u>あがる</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "上がる",
          "水曜日",
          "自分",
          "時々"
        ],
        "answer": 0,
        "id": "N5-07-011"
      },
      {
        "type": "表記",
        "text": "<u>わける</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "音",
          "分ける",
          "知る",
          "高い"
        ],
        "answer": 1,
        "id": "N5-07-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "バス",
          "かばん",
          "くだもの",
          "でんしゃ"
        ],
        "answer": 2,
        "id": "N5-07-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "コーヒー",
          "えんぴつ",
          "ぼうし",
          "ほん"
        ],
        "answer": 0,
        "id": "N5-07-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "そば",
          "バス",
          "くるま",
          "いす"
        ],
        "answer": 0,
        "id": "N5-07-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "かさ",
          "てんぷら",
          "びょういん",
          "でんしゃ"
        ],
        "answer": 1,
        "id": "N5-07-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "としょかん",
          "パン",
          "テレビ",
          "みず"
        ],
        "answer": 0,
        "id": "N5-07-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みかん",
          "ほん",
          "へや",
          "テレビ"
        ],
        "answer": 2,
        "id": "N5-07-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "おちゃ",
          "みせ",
          "みかん",
          "えんぴつ"
        ],
        "answer": 1,
        "id": "N5-07-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "こうえん",
          "ほん",
          "パン",
          "えんぴつ"
        ],
        "answer": 0,
        "id": "N5-07-020"
      },
      {
        "type": "言い換え",
        "text": "「いる」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "sweater",
          "to exist (animate)",
          "time",
          "Sunday"
        ],
        "answer": 1,
        "id": "N5-07-021"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>入力<rt>にゅうりょく</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "input",
          "university",
          "hotel",
          "to do"
        ],
        "answer": 0,
        "id": "N5-07-022"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>中<rt>ちゅう</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "last month",
          "to exit/leave",
          "to eat",
          "inside"
        ],
        "answer": 3,
        "id": "N5-07-023"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>何<rt>なに</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "finally",
          "outside",
          "what",
          "taxi"
        ],
        "answer": 2,
        "id": "N5-07-024"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>十分<rt>じゅうぶん</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "ten thousand",
          "toilet",
          "enough",
          "bus"
        ],
        "answer": 2,
        "id": "N5-07-025"
      },
      {
        "type": "文法形式",
        "text": "のど（　）かわきました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "に",
          "を",
          "は"
        ],
        "answer": 0,
        "id": "N5-07-026"
      },
      {
        "type": "文法形式",
        "text": "おなか（　）すきました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "に",
          "は",
          "を"
        ],
        "answer": 0,
        "id": "N5-07-027"
      },
      {
        "type": "文法形式",
        "text": "じかん（　）ありません。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "に",
          "を",
          "は"
        ],
        "answer": 0,
        "id": "N5-07-028"
      },
      {
        "type": "文法形式",
        "text": "おかね（　）たりません。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "に",
          "は",
          "を"
        ],
        "answer": 0,
        "id": "N5-07-029"
      },
      {
        "type": "文法形式",
        "text": "ともだち（　）えいがを　みました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "と",
          "も",
          "や",
          "が"
        ],
        "answer": 0,
        "id": "N5-07-030"
      },
      {
        "type": "文法形式",
        "text": "パン（　）たまごを　たべました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "と",
          "も",
          "や",
          "が"
        ],
        "answer": 0,
        "id": "N5-07-031"
      },
      {
        "type": "文法形式",
        "text": "すし（　）てんぷらが　すきです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "や",
          "が",
          "と",
          "も"
        ],
        "answer": 0,
        "id": "N5-07-032"
      },
      {
        "type": "文法形式",
        "text": "ほん（　）ざっしを　よみます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "や",
          "も",
          "が",
          "と"
        ],
        "answer": 0,
        "id": "N5-07-033"
      },
      {
        "type": "文法形式",
        "text": "わたし（　）にほんごを　べんきょうしています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "も",
          "が",
          "は",
          "を"
        ],
        "answer": 0,
        "id": "N5-07-034"
      },
      {
        "type": "文法形式",
        "text": "やまださん（　）きます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "も",
          "を",
          "は",
          "が"
        ],
        "answer": 0,
        "id": "N5-07-035"
      },
      {
        "type": "文の組み立て",
        "text": "あした ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "しごとを",
          "します",
          "は",
          "わたし"
        ],
        "answer": 0,
        "id": "N5-07-036"
      },
      {
        "type": "文の組み立て",
        "text": "らいしゅう ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "テストが",
          "あります",
          "は",
          "がっこう"
        ],
        "answer": 0,
        "id": "N5-07-037"
      },
      {
        "type": "文の組み立て",
        "text": "らいげつ ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "りょこうを",
          "します",
          "は",
          "かぞく"
        ],
        "answer": 0,
        "id": "N5-07-038"
      },
      {
        "type": "文の組み立て",
        "text": "らいねん ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "けっこんを",
          "します",
          "は",
          "あね"
        ],
        "answer": 0,
        "id": "N5-07-039"
      },
      {
        "type": "文の組み立て",
        "text": "まいにち ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "うんどうを",
          "します",
          "は",
          "わたし"
        ],
        "answer": 0,
        "id": "N5-07-040"
      },
      {
        "type": "文章の文法",
        "text": "わたしは にほんりょうりが（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "すき",
          "すきな",
          "すきに",
          "すきで"
        ],
        "answer": 0,
        "id": "N5-07-041"
      },
      {
        "type": "文章の文法",
        "text": "この へやは しずか（　）きれいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "に",
          "な",
          "の"
        ],
        "answer": 0,
        "id": "N5-07-042"
      },
      {
        "type": "文章の文法",
        "text": "やまださんは やさしい（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "ひと",
          "ひとな",
          "ひとに",
          "ひとで"
        ],
        "answer": 0,
        "id": "N5-07-043"
      },
      {
        "type": "文章の文法",
        "text": "わたしは あした がっこうに（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "いきます",
          "いって",
          "いった",
          "いく"
        ],
        "answer": 0,
        "id": "N5-07-044"
      },
      {
        "type": "文章の文法",
        "text": "この りんごは あか（　）おいしいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "くて",
          "いで",
          "いと",
          "くない"
        ],
        "answer": 0,
        "id": "N5-07-045"
      },
      {
        "type": "読解（短文）",
        "text": "このレストランは　やさいりょうりが　おいしいです。にくりょうりも　あります。",
        "question": "なにが　おいしいですか。",
        "options": [
          "にくりょうり",
          "さかなりょうり",
          "やさいりょうり",
          "めんりょうり"
        ],
        "answer": 1,
        "id": "N5-07-046"
      },
      {
        "type": "読解（短文）",
        "text": "わたしは　あさ　コーヒーを　のみます。よる　おちゃを　のみます。",
        "question": "あさ　なにを　のみますか。",
        "options": [
          "おちゃ",
          "コーヒー",
          "ジュース",
          "みず"
        ],
        "answer": 1,
        "id": "N5-07-047"
      },
      {
        "type": "読解（短文）",
        "text": "きょうは　はれです。あしたは　あめです。",
        "question": "あしたの　てんきは　どうですか。",
        "options": [
          "はれ",
          "くもり",
          "あめ",
          "ゆき"
        ],
        "answer": 1,
        "id": "N5-07-048"
      },
      {
        "type": "読解（中文）",
        "text": "にほんに　きて　3かげつです。とうきょうと　おおさかに　いきました。きょうとには　まだ　いっていません。らいげつ　いきたいです。",
        "question": "どこに　まだ　いっていませんか。",
        "options": [
          "とうきょう",
          "おおさか",
          "きょうと",
          "なら"
        ],
        "answer": 1,
        "id": "N5-07-049"
      },
      {
        "type": "読解（中文）",
        "text": "きょうは　デパートに　いきました。くつと　かばんを　かいました。くつは　5000えん、かばんは　3000えんでした。",
        "question": "なにを　かいましたか。",
        "options": [
          "ふくと　くつ",
          "くつと　かばん",
          "かばんと　ぼうし",
          "ふくと　かばん"
        ],
        "answer": 1,
        "id": "N5-07-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-07-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "駅",
          "病院",
          "銀行",
          "郵便局"
        ],
        "answer": 0,
        "id": "N5-07-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "1050円",
          "1100円",
          "1150円",
          "1200円"
        ],
        "answer": 2,
        "id": "N5-07-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "1000円",
          "1050円",
          "1100円",
          "1150円"
        ],
        "answer": 0,
        "id": "N5-07-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "950円",
          "1000円",
          "1050円",
          "1100円"
        ],
        "answer": 2,
        "id": "N5-07-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "900円",
          "950円",
          "1000円",
          "1050円"
        ],
        "answer": 0,
        "id": "N5-07-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "850円",
          "900円",
          "950円",
          "1000円"
        ],
        "answer": 2,
        "id": "N5-07-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "1:15",
          "1:30",
          "1:45",
          "2:00"
        ],
        "answer": 0,
        "id": "N5-07-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "2:15",
          "2:30",
          "2:45",
          "3:00"
        ],
        "answer": 1,
        "id": "N5-07-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "3:15",
          "3:30",
          "3:45",
          "4:00"
        ],
        "answer": 0,
        "id": "N5-07-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "4:15",
          "4:30",
          "4:45",
          "5:00"
        ],
        "answer": 1,
        "id": "N5-07-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "5:15",
          "5:30",
          "5:45",
          "6:00"
        ],
        "answer": 0,
        "id": "N5-07-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "6:15",
          "6:30",
          "6:45",
          "7:00"
        ],
        "answer": 1,
        "id": "N5-07-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "賛成です",
          "反対です",
          "どちらとも",
          "検討します"
        ],
        "answer": 0,
        "id": "N5-07-064",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "大丈夫です",
          "問題ありません",
          "心配いりません",
          "気にしないで"
        ],
        "answer": 0,
        "id": "N5-07-065",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "お願いします",
          "けっこうです",
          "どちらでもいいです",
          "考えます"
        ],
        "answer": 0,
        "id": "N5-07-066",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "喜んで",
          "是非",
          "ぜひお願いします",
          "楽しみです"
        ],
        "answer": 0,
        "id": "N5-07-067",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "遠慮します",
          "ご遠慮ください",
          "ちょっと...",
          "今回は"
        ],
        "answer": 0,
        "id": "N5-07-068",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "岡山です",
          "松山です",
          "高松です",
          "那覇です"
        ],
        "answer": 0,
        "id": "N5-07-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "松山です",
          "高松です",
          "那覇です",
          "千葉です"
        ],
        "answer": 0,
        "id": "N5-07-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "高松です",
          "那覇です",
          "千葉です",
          "埼玉です"
        ],
        "answer": 0,
        "id": "N5-07-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "那覇です",
          "千葉です",
          "埼玉です",
          "栃木です"
        ],
        "answer": 0,
        "id": "N5-07-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "千葉です",
          "埼玉です",
          "栃木です",
          "群馬です"
        ],
        "answer": 0,
        "id": "N5-07-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "埼玉です",
          "栃木です",
          "群馬です",
          "茨城です"
        ],
        "answer": 0,
        "id": "N5-07-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "会社員です",
          "学生です",
          "教師です",
          "医者です"
        ],
        "answer": 0,
        "id": "N5-07-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "学生です",
          "教師です",
          "医者です",
          "看護師です"
        ],
        "answer": 0,
        "id": "N5-07-076",
        "script": "なんさいですか。"
      }
    ],
    "8": [
      {
        "type": "漢字読み",
        "text": "<u>四つ</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "みつける",
          "げんき",
          "くる",
          "よっつ"
        ],
        "answer": 3,
        "id": "N5-08-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>今</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "うまれる",
          "いま",
          "かく",
          "らいしゅう"
        ],
        "answer": 1,
        "id": "N5-08-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>時間</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "うえ",
          "みず",
          "なに",
          "じかん"
        ],
        "answer": 3,
        "id": "N5-08-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>出る</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "げんき",
          "おと",
          "でる",
          "きょうしつ"
        ],
        "answer": 2,
        "id": "N5-08-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>前</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "きこえる",
          "まえ",
          "もくようび",
          "はいる"
        ],
        "answer": 1,
        "id": "N5-08-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>休み</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "かく",
          "じゅうぶん",
          "ごぜん",
          "やすみ"
        ],
        "answer": 3,
        "id": "N5-08-006"
      },
      {
        "type": "表記",
        "text": "<u>たべる</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "食べる",
          "見える",
          "下がる",
          "正しい"
        ],
        "answer": 0,
        "id": "N5-08-007"
      },
      {
        "type": "表記",
        "text": "<u>え</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "四つ",
          "人",
          "大学",
          "絵"
        ],
        "answer": 3,
        "id": "N5-08-008"
      },
      {
        "type": "表記",
        "text": "<u>まいとし</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "電車",
          "本",
          "毎年",
          "午前"
        ],
        "answer": 2,
        "id": "N5-08-009"
      },
      {
        "type": "表記",
        "text": "<u>みつける</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "黒い",
          "見つける",
          "出力",
          "色"
        ],
        "answer": 1,
        "id": "N5-08-010"
      },
      {
        "type": "表記",
        "text": "<u>げんき</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "人",
          "外国",
          "出口",
          "元気"
        ],
        "answer": 3,
        "id": "N5-08-011"
      },
      {
        "type": "表記",
        "text": "<u>おかあさん</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "お母さん",
          "魚",
          "森",
          "人"
        ],
        "answer": 0,
        "id": "N5-08-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "びょういん",
          "でんしゃ",
          "りんご",
          "でんわ"
        ],
        "answer": 2,
        "id": "N5-08-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "コーヒー",
          "ぼうし",
          "ズボン",
          "えんぴつ"
        ],
        "answer": 0,
        "id": "N5-08-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "かばん",
          "いす",
          "でんわ",
          "うどん"
        ],
        "answer": 3,
        "id": "N5-08-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "いす",
          "にく",
          "びょういん",
          "くるま"
        ],
        "answer": 1,
        "id": "N5-08-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "えんぴつ",
          "パン",
          "びょういん",
          "ほん"
        ],
        "answer": 2,
        "id": "N5-08-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ほん",
          "えんぴつ",
          "いえ",
          "おちゃ"
        ],
        "answer": 2,
        "id": "N5-08-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みかん",
          "りんご",
          "えんぴつ",
          "デパート"
        ],
        "answer": 3,
        "id": "N5-08-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "りんご",
          "みず",
          "おちゃ",
          "うみ"
        ],
        "answer": 3,
        "id": "N5-08-020"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>千<rt>せん</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "to be enough",
          "above",
          "flower",
          "thousand"
        ],
        "answer": 3,
        "id": "N5-08-021"
      },
      {
        "type": "言い換え",
        "text": "「おばあさん」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "to buy",
          "picture",
          "goodbye",
          "grandmother (polite)"
        ],
        "answer": 3,
        "id": "N5-08-022"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>雪<rt>ゆき</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "snow",
          "bill",
          "camera",
          "every month"
        ],
        "answer": 0,
        "id": "N5-08-023"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>時々<rt>ときどき</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "sometimes",
          "good evening",
          "gradually",
          "oneself"
        ],
        "answer": 0,
        "id": "N5-08-024"
      },
      {
        "type": "言い換え",
        "text": "「あそこ」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "over there",
          "many",
          "good evening",
          "to be born"
        ],
        "answer": 0,
        "id": "N5-08-025"
      },
      {
        "type": "文法形式",
        "text": "5じ（　）しごとを　おわります。",
        "question": "（　）に　はいるのは？",
        "options": [
          "まで",
          "から",
          "に",
          "で"
        ],
        "answer": 0,
        "id": "N5-08-026"
      },
      {
        "type": "文法形式",
        "text": "うち（　）えきまで　あるきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "から",
          "に",
          "で",
          "まで"
        ],
        "answer": 0,
        "id": "N5-08-027"
      },
      {
        "type": "文法形式",
        "text": "にほん（　）きました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "から",
          "に",
          "まで",
          "で"
        ],
        "answer": 0,
        "id": "N5-08-028"
      },
      {
        "type": "文法形式",
        "text": "にほん（　）りょうりは　おいしいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "の",
          "は",
          "が",
          "を"
        ],
        "answer": 0,
        "id": "N5-08-029"
      },
      {
        "type": "文法形式",
        "text": "やまださん（　）いえは　おおきいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "の",
          "は",
          "が",
          "を"
        ],
        "answer": 0,
        "id": "N5-08-030"
      },
      {
        "type": "文法形式",
        "text": "がっこう（　）せんせいは　しんせつです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "の",
          "を",
          "が",
          "は"
        ],
        "answer": 0,
        "id": "N5-08-031"
      },
      {
        "type": "文法形式",
        "text": "まいにち　6じに　（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "おきます",
          "おきました",
          "おきる",
          "おきて"
        ],
        "answer": 0,
        "id": "N5-08-032"
      },
      {
        "type": "文法形式",
        "text": "よる　11じに　（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "ねます",
          "ねて",
          "ねる",
          "ねました"
        ],
        "answer": 0,
        "id": "N5-08-033"
      },
      {
        "type": "文法形式",
        "text": "まいにち　しんぶんを　（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "よみます",
          "よみました",
          "よんで",
          "よむ"
        ],
        "answer": 0,
        "id": "N5-08-034"
      },
      {
        "type": "文法形式",
        "text": "せんしゅう　とうきょうに　（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "いきました",
          "いって",
          "いく",
          "いきます"
        ],
        "answer": 0,
        "id": "N5-08-035"
      },
      {
        "type": "文の組み立て",
        "text": "この ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "ほんは",
          "おもしろい",
          "です",
          "とても"
        ],
        "answer": 0,
        "id": "N5-08-036"
      },
      {
        "type": "文の組み立て",
        "text": "あの ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "レストランは",
          "おいしい",
          "です",
          "とても"
        ],
        "answer": 0,
        "id": "N5-08-037"
      },
      {
        "type": "文の組み立て",
        "text": "この ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "へやは",
          "しずか",
          "です",
          "とても"
        ],
        "answer": 0,
        "id": "N5-08-038"
      },
      {
        "type": "文の組み立て",
        "text": "その ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "えいがは",
          "ゆうめい",
          "です",
          "とても"
        ],
        "answer": 0,
        "id": "N5-08-039"
      },
      {
        "type": "文の組み立て",
        "text": "あの ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "せんせいは",
          "やさしい",
          "です",
          "とても"
        ],
        "answer": 0,
        "id": "N5-08-040"
      },
      {
        "type": "文章の文法",
        "text": "えいごは にほんご（　）むずかしいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "より",
          "ほど",
          "くらい",
          "ぐらい"
        ],
        "answer": 0,
        "id": "N5-08-041"
      },
      {
        "type": "文章の文法",
        "text": "わたしは コーヒー（　）こうちゃが すきです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "より",
          "ほど",
          "から",
          "まで"
        ],
        "answer": 0,
        "id": "N5-08-042"
      },
      {
        "type": "文章の文法",
        "text": "この みちを まっすぐ（　）ください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "いって",
          "いきます",
          "いった",
          "いく"
        ],
        "answer": 0,
        "id": "N5-08-043"
      },
      {
        "type": "文章の文法",
        "text": "きのう 3じかん（　）べんきょうしました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "ぐらい",
          "しか",
          "だけ",
          "まで"
        ],
        "answer": 0,
        "id": "N5-08-044"
      },
      {
        "type": "文章の文法",
        "text": "わたしは まいにち 7じ（　）おきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "ごろ",
          "くらい",
          "ほど",
          "まで"
        ],
        "answer": 0,
        "id": "N5-08-045"
      },
      {
        "type": "読解（短文）",
        "text": "なつは　あついです。ふゆは　さむいです。わたしは　はるが　すきです。",
        "question": "わたしは　どの　きせつが　すきですか。",
        "options": [
          "はる",
          "なつ",
          "あき",
          "ふゆ"
        ],
        "answer": 1,
        "id": "N5-08-046"
      },
      {
        "type": "読解（短文）",
        "text": "きのうは　さむかったです。きょうは　あたたかいです。",
        "question": "きょうは　どうですか。",
        "options": [
          "さむい",
          "あつい",
          "あたたかい",
          "すずしい"
        ],
        "answer": 1,
        "id": "N5-08-047"
      },
      {
        "type": "読解（短文）",
        "text": "あめの　ひは　かさを　もっていきます。きょうは　はれなので　かさは　いりません。",
        "question": "きょうは　かさが　いりますか。",
        "options": [
          "はい、いります",
          "いいえ、いりません",
          "たぶん　いります",
          "わかりません"
        ],
        "answer": 1,
        "id": "N5-08-048"
      },
      {
        "type": "読解（中文）",
        "text": "どようびに　ともだちの　うちに　いきます。ともだちの　うちは　えきから　あるいて　10ぷんです。3じに　あう　やくそくです。",
        "question": "なんじに　あいますか。",
        "options": [
          "1じ",
          "2じ",
          "3じ",
          "4じ"
        ],
        "answer": 1,
        "id": "N5-08-049"
      },
      {
        "type": "読解（中文）",
        "text": "このレストランは　イタリアりょうりの　みせです。パスタと　ピザが　ゆうめいです。ランチは　800えんからです。やすくて　おいしいです。",
        "question": "なにりょうりの　みせですか。",
        "options": [
          "にほんりょうり",
          "ちゅうかりょうり",
          "イタリアりょうり",
          "フランスりょうり"
        ],
        "answer": 1,
        "id": "N5-08-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-08-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "800円",
          "850円",
          "900円",
          "950円"
        ],
        "answer": 0,
        "id": "N5-08-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "750円",
          "800円",
          "850円",
          "900円"
        ],
        "answer": 2,
        "id": "N5-08-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "700円",
          "750円",
          "800円",
          "850円"
        ],
        "answer": 0,
        "id": "N5-08-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "650円",
          "700円",
          "750円",
          "800円"
        ],
        "answer": 2,
        "id": "N5-08-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "600円",
          "650円",
          "700円",
          "750円"
        ],
        "answer": 0,
        "id": "N5-08-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "550円",
          "600円",
          "650円",
          "700円"
        ],
        "answer": 2,
        "id": "N5-08-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "7:15",
          "7:30",
          "7:45",
          "8:00"
        ],
        "answer": 0,
        "id": "N5-08-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "8:15",
          "8:30",
          "8:45",
          "9:00"
        ],
        "answer": 1,
        "id": "N5-08-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "9:15",
          "9:30",
          "9:45",
          "10:00"
        ],
        "answer": 0,
        "id": "N5-08-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "10:15",
          "10:30",
          "10:45",
          "11:00"
        ],
        "answer": 1,
        "id": "N5-08-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "11:15",
          "11:30",
          "11:45",
          "12:00"
        ],
        "answer": 0,
        "id": "N5-08-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "12:15",
          "12:30",
          "12:45",
          "1:00"
        ],
        "answer": 1,
        "id": "N5-08-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "覚えています",
          "覚えていません",
          "忘れました",
          "思い出せません"
        ],
        "answer": 0,
        "id": "N5-08-064",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "知っています",
          "知りません",
          "聞いたことがあります",
          "初めて聞きました"
        ],
        "answer": 0,
        "id": "N5-08-065",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "できます",
          "できません",
          "難しいです",
          "やってみます"
        ],
        "answer": 0,
        "id": "N5-08-066",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "ありです",
          "なしです",
          "微妙です",
          "考え中です"
        ],
        "answer": 0,
        "id": "N5-08-067",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "行きます",
          "行けません",
          "行きたいです",
          "行けたら行きます"
        ],
        "answer": 0,
        "id": "N5-08-068",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "教師です",
          "医者です",
          "看護師です",
          "弁護士です"
        ],
        "answer": 0,
        "id": "N5-08-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "医者です",
          "看護師です",
          "弁護士です",
          "公務員です"
        ],
        "answer": 0,
        "id": "N5-08-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "看護師です",
          "弁護士です",
          "公務員です",
          "銀行員です"
        ],
        "answer": 0,
        "id": "N5-08-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "弁護士です",
          "公務員です",
          "銀行員です",
          "店員です"
        ],
        "answer": 0,
        "id": "N5-08-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "公務員です",
          "銀行員です",
          "店員です",
          "主婦です"
        ],
        "answer": 0,
        "id": "N5-08-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "銀行員です",
          "店員です",
          "主婦です",
          "エンジニアです"
        ],
        "answer": 0,
        "id": "N5-08-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "店員です",
          "主婦です",
          "エンジニアです",
          "デザイナーです"
        ],
        "answer": 0,
        "id": "N5-08-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "主婦です",
          "エンジニアです",
          "デザイナーです",
          "料理人です"
        ],
        "answer": 0,
        "id": "N5-08-076",
        "script": "なんさいですか。"
      }
    ],
    "9": [
      {
        "type": "漢字読み",
        "text": "<u>中</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "あう",
          "おんがく",
          "みず",
          "なか"
        ],
        "answer": 3,
        "id": "N5-09-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>会社</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "なまえ",
          "かいしゃ",
          "はな",
          "おちゃ"
        ],
        "answer": 1,
        "id": "N5-09-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>行く</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "でんき",
          "いく",
          "ふたつ",
          "おと"
        ],
        "answer": 1,
        "id": "N5-09-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>月曜日</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "たりる",
          "うしろ",
          "げつようび",
          "した"
        ],
        "answer": 2,
        "id": "N5-09-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>聞こえる</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "じょうず",
          "きこえる",
          "いろ",
          "すいようび"
        ],
        "answer": 1,
        "id": "N5-09-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>電車</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "でんしゃ",
          "へた",
          "きょうしつ",
          "おと"
        ],
        "answer": 0,
        "id": "N5-09-006"
      },
      {
        "type": "表記",
        "text": "<u>あかるい</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "上手",
          "明るい",
          "知る",
          "生まれる"
        ],
        "answer": 1,
        "id": "N5-09-007"
      },
      {
        "type": "表記",
        "text": "<u>かく</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "書く",
          "鳥",
          "名前",
          "上手"
        ],
        "answer": 0,
        "id": "N5-09-008"
      },
      {
        "type": "表記",
        "text": "<u>いま</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "今",
          "先生",
          "毎日",
          "鳥"
        ],
        "answer": 0,
        "id": "N5-09-009"
      },
      {
        "type": "表記",
        "text": "<u>にゅうりょく</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "足",
          "後ろ",
          "学生",
          "入力"
        ],
        "answer": 3,
        "id": "N5-09-010"
      },
      {
        "type": "表記",
        "text": "<u>えき</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "形",
          "駅",
          "先週",
          "知る"
        ],
        "answer": 1,
        "id": "N5-09-011"
      },
      {
        "type": "表記",
        "text": "<u>らいねん</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "前",
          "電気",
          "生まれる",
          "来年"
        ],
        "answer": 3,
        "id": "N5-09-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "バス",
          "つくえ",
          "えき",
          "ごはん"
        ],
        "answer": 3,
        "id": "N5-09-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ノート",
          "ほん",
          "ズボン",
          "おちゃ"
        ],
        "answer": 3,
        "id": "N5-09-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "そば",
          "びょういん",
          "えき",
          "でんわ"
        ],
        "answer": 0,
        "id": "N5-09-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "バス",
          "かばん",
          "テレビ",
          "やさい"
        ],
        "answer": 3,
        "id": "N5-09-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "デパート",
          "えんぴつ",
          "おちゃ",
          "テレビ"
        ],
        "answer": 0,
        "id": "N5-09-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みず",
          "いえ",
          "みかん",
          "りんご"
        ],
        "answer": 1,
        "id": "N5-09-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ほん",
          "コンビニ",
          "テレビ",
          "みかん"
        ],
        "answer": 1,
        "id": "N5-09-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ほん",
          "うみ",
          "テレビ",
          "えんぴつ"
        ],
        "answer": 1,
        "id": "N5-09-020"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>話す<rt>はなす</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "above",
          "to speak",
          "sound",
          "classroom"
        ],
        "answer": 1,
        "id": "N5-09-021"
      },
      {
        "type": "言い換え",
        "text": "「よく」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "often",
          "eye",
          "thousand",
          "bright"
        ],
        "answer": 0,
        "id": "N5-09-022"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>生きる<rt>いきる</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "to live",
          "weather",
          "to give",
          "Tuesday"
        ],
        "answer": 0,
        "id": "N5-09-023"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>人<rt>ひと</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "person",
          "mother (polite)",
          "tomorrow",
          "that (far)"
        ],
        "answer": 0,
        "id": "N5-09-024"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>元気<rt>げんき</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "bread",
          "notebook",
          "immediately",
          "healthy/energetic"
        ],
        "answer": 3,
        "id": "N5-09-025"
      },
      {
        "type": "文法形式",
        "text": "きょうは　しごとに　（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "いきません",
          "いく",
          "いかない",
          "いきます"
        ],
        "answer": 0,
        "id": "N5-09-026"
      },
      {
        "type": "文法形式",
        "text": "おさけは　（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "のみません",
          "のみます",
          "のまない",
          "のむ"
        ],
        "answer": 0,
        "id": "N5-09-027"
      },
      {
        "type": "文法形式",
        "text": "でんしゃに　のっ（　）いきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "て",
          "から",
          "で",
          "に"
        ],
        "answer": 0,
        "id": "N5-09-028"
      },
      {
        "type": "文法形式",
        "text": "ここに　すわっ（　）ください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "て",
          "から",
          "で",
          "に"
        ],
        "answer": 0,
        "id": "N5-09-029"
      },
      {
        "type": "文法形式",
        "text": "まっすぐ　いっ（　）ください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "て",
          "に",
          "から",
          "で"
        ],
        "answer": 0,
        "id": "N5-09-030"
      },
      {
        "type": "文法形式",
        "text": "ゆっくり　はなし（　）ください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "て",
          "に",
          "から",
          "で"
        ],
        "answer": 0,
        "id": "N5-09-031"
      },
      {
        "type": "文法形式",
        "text": "ちょっと　まっ（　）ください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "て",
          "で",
          "から",
          "に"
        ],
        "answer": 0,
        "id": "N5-09-032"
      },
      {
        "type": "文法形式",
        "text": "わたしは　とうきょうに　（　）います。",
        "question": "（　）に　はいるのは？",
        "options": [
          "すんで",
          "すみ",
          "すんだ",
          "すむ"
        ],
        "answer": 0,
        "id": "N5-09-033"
      },
      {
        "type": "文法形式",
        "text": "やまださんを　（　）いますか。",
        "question": "（　）に　はいるのは？",
        "options": [
          "しって",
          "しる",
          "しり",
          "しった"
        ],
        "answer": 0,
        "id": "N5-09-034"
      },
      {
        "type": "文法形式",
        "text": "いま　ほんを　（　）います。",
        "question": "（　）に　はいるのは？",
        "options": [
          "よんで",
          "よんだ",
          "よむ",
          "よみ"
        ],
        "answer": 0,
        "id": "N5-09-035"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "コーヒーが",
          "すきです",
          "は",
          "とても"
        ],
        "answer": 1,
        "id": "N5-09-036"
      },
      {
        "type": "文の組み立て",
        "text": "かれは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "スポーツが",
          "すきです",
          "は",
          "とても"
        ],
        "answer": 1,
        "id": "N5-09-037"
      },
      {
        "type": "文の組み立て",
        "text": "かのじょは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "おんがくが",
          "すきです",
          "は",
          "とても"
        ],
        "answer": 1,
        "id": "N5-09-038"
      },
      {
        "type": "文の組み立て",
        "text": "こどもは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "ゲームが",
          "すきです",
          "は",
          "とても"
        ],
        "answer": 1,
        "id": "N5-09-039"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "やさいが",
          "きらいです",
          "は",
          "あまり"
        ],
        "answer": 1,
        "id": "N5-09-040"
      },
      {
        "type": "文章の文法",
        "text": "この へやに だれ（　）いません。",
        "question": "（　）に　はいるのは？",
        "options": [
          "も",
          "が",
          "は",
          "か"
        ],
        "answer": 0,
        "id": "N5-09-041"
      },
      {
        "type": "文章の文法",
        "text": "あした なに（　）しますか。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "は",
          "も"
        ],
        "answer": 0,
        "id": "N5-09-042"
      },
      {
        "type": "文章の文法",
        "text": "この えいがは（　）おもしろかったです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "とても",
          "あまり",
          "ぜんぜん",
          "すこし"
        ],
        "answer": 0,
        "id": "N5-09-043"
      },
      {
        "type": "文章の文法",
        "text": "きょうは てんきが（　）ですね。",
        "question": "（　）に　はいるのは？",
        "options": [
          "いい",
          "よく",
          "いいな",
          "いいに"
        ],
        "answer": 0,
        "id": "N5-09-044"
      },
      {
        "type": "文章の文法",
        "text": "わたしは にほんに 3ねん（　）すんでいます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "ぐらい",
          "しか",
          "だけ",
          "ほど"
        ],
        "answer": 0,
        "id": "N5-09-045"
      },
      {
        "type": "読解（短文）",
        "text": "1がつは　さむいです。7がつは　あついです。",
        "question": "7がつは　どうですか。",
        "options": [
          "さむい",
          "すずしい",
          "あたたかい",
          "あつい"
        ],
        "answer": 1,
        "id": "N5-09-046"
      },
      {
        "type": "読解（短文）",
        "text": "このほんは　500えんです。あのほんは　800えんです。",
        "question": "あのほんは　いくらですか。",
        "options": [
          "300えん",
          "500えん",
          "800えん",
          "1000えん"
        ],
        "answer": 1,
        "id": "N5-09-047"
      },
      {
        "type": "読解（短文）",
        "text": "りんごは　1こ　100えんです。3こ　かいました。",
        "question": "ぜんぶで　いくらですか。",
        "options": [
          "100えん",
          "200えん",
          "300えん",
          "400えん"
        ],
        "answer": 1,
        "id": "N5-09-048"
      },
      {
        "type": "読解（中文）",
        "text": "わたしは　やさいが　すきです。にくは　あまり　たべません。さかなは　ときどき　たべます。くだものも　だいすきです。",
        "question": "よく　たべるものは　なんですか。",
        "options": [
          "にく",
          "やさい",
          "さかな",
          "たまご"
        ],
        "answer": 1,
        "id": "N5-09-049"
      },
      {
        "type": "読解（中文）",
        "text": "きょうの　ひるごはんは　おべんとうです。おにぎりが　2こと　たまごやきが　はいっています。おかあさんが　つくりました。",
        "question": "おべんとうを　だれが　つくりましたか。",
        "options": [
          "わたし",
          "おとうさん",
          "おかあさん",
          "ともだち"
        ],
        "answer": 1,
        "id": "N5-09-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-09-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "500円",
          "550円",
          "600円",
          "650円"
        ],
        "answer": 0,
        "id": "N5-09-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "450円",
          "500円",
          "550円",
          "600円"
        ],
        "answer": 2,
        "id": "N5-09-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "400円",
          "450円",
          "500円",
          "550円"
        ],
        "answer": 0,
        "id": "N5-09-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "350円",
          "400円",
          "450円",
          "500円"
        ],
        "answer": 2,
        "id": "N5-09-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "300円",
          "350円",
          "400円",
          "450円"
        ],
        "answer": 0,
        "id": "N5-09-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "250円",
          "300円",
          "350円",
          "400円"
        ],
        "answer": 2,
        "id": "N5-09-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "1時ごろ",
          "1時すぎ",
          "1時前",
          "1時ちょうど"
        ],
        "answer": 0,
        "id": "N5-09-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "2時ごろ",
          "2時すぎ",
          "2時前",
          "2時ちょうど"
        ],
        "answer": 1,
        "id": "N5-09-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "3時ごろ",
          "3時すぎ",
          "3時前",
          "3時ちょうど"
        ],
        "answer": 0,
        "id": "N5-09-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "4時ごろ",
          "4時すぎ",
          "4時前",
          "4時ちょうど"
        ],
        "answer": 1,
        "id": "N5-09-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "5時ごろ",
          "5時すぎ",
          "5時前",
          "5時ちょうど"
        ],
        "answer": 0,
        "id": "N5-09-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "6時ごろ",
          "6時すぎ",
          "6時前",
          "6時ちょうど"
        ],
        "answer": 1,
        "id": "N5-09-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "食べます",
          "食べません",
          "いただきます",
          "遠慮します"
        ],
        "answer": 0,
        "id": "N5-09-064",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "飲みます",
          "飲みません",
          "いただきます",
          "結構です"
        ],
        "answer": 0,
        "id": "N5-09-065",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "買います",
          "買いません",
          "考えます",
          "見るだけです"
        ],
        "answer": 0,
        "id": "N5-09-066",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "やります",
          "やりません",
          "挑戦します",
          "今度にします"
        ],
        "answer": 0,
        "id": "N5-09-067",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "手伝います",
          "手伝えません",
          "やってみます",
          "後で"
        ],
        "answer": 0,
        "id": "N5-09-068",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "エンジニアです",
          "デザイナーです",
          "料理人です",
          "運転手です"
        ],
        "answer": 0,
        "id": "N5-09-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "デザイナーです",
          "料理人です",
          "運転手です",
          "警察官です"
        ],
        "answer": 0,
        "id": "N5-09-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "料理人です",
          "運転手です",
          "警察官です",
          "消防士です"
        ],
        "answer": 0,
        "id": "N5-09-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "運転手です",
          "警察官です",
          "消防士です",
          "記者です"
        ],
        "answer": 0,
        "id": "N5-09-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "警察官です",
          "消防士です",
          "記者です",
          "作家です"
        ],
        "answer": 0,
        "id": "N5-09-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "消防士です",
          "記者です",
          "作家です",
          "歌手です"
        ],
        "answer": 0,
        "id": "N5-09-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "記者です",
          "作家です",
          "歌手です",
          "俳優です"
        ],
        "answer": 0,
        "id": "N5-09-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "1時です",
          "1時半です",
          "2時です",
          "2時半です"
        ],
        "answer": 0,
        "id": "N5-09-076",
        "script": "なんさいですか。"
      }
    ],
    "10": [
      {
        "type": "漢字読み",
        "text": "<u>生きる</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "いきる",
          "もり",
          "つき",
          "ひだり"
        ],
        "answer": 0,
        "id": "N5-10-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>電話</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "しる",
          "ふん",
          "ときどき",
          "でんわ"
        ],
        "answer": 3,
        "id": "N5-10-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>土曜日</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "せん",
          "はんぶん",
          "でぐち",
          "どようび"
        ],
        "answer": 3,
        "id": "N5-10-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>見る</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "ゆき",
          "ときどき",
          "みる",
          "おかね"
        ],
        "answer": 2,
        "id": "N5-10-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>食べる</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "いろ",
          "ひだり",
          "えき",
          "たべる"
        ],
        "answer": 3,
        "id": "N5-10-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>時々</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "め",
          "しゃかい",
          "いきる",
          "ときどき"
        ],
        "answer": 3,
        "id": "N5-10-006"
      },
      {
        "type": "表記",
        "text": "<u>ちから</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "会計",
          "下がる",
          "三つ",
          "力"
        ],
        "answer": 3,
        "id": "N5-10-007"
      },
      {
        "type": "表記",
        "text": "<u>おとこのこ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "一人",
          "花",
          "男の子",
          "三つ"
        ],
        "answer": 2,
        "id": "N5-10-008"
      },
      {
        "type": "表記",
        "text": "<u>した</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "十分",
          "下",
          "山",
          "毎月"
        ],
        "answer": 1,
        "id": "N5-10-009"
      },
      {
        "type": "表記",
        "text": "<u>くち</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "口",
          "水",
          "見る",
          "出力"
        ],
        "answer": 0,
        "id": "N5-10-010"
      },
      {
        "type": "表記",
        "text": "<u>くろい</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "二人",
          "本",
          "天気",
          "黒い"
        ],
        "answer": 3,
        "id": "N5-10-011"
      },
      {
        "type": "表記",
        "text": "<u>ごぜん</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "お水",
          "先週",
          "買う",
          "午前"
        ],
        "answer": 3,
        "id": "N5-10-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "でんわ",
          "バス",
          "でんしゃ",
          "りんご"
        ],
        "answer": 3,
        "id": "N5-10-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ほん",
          "かぎ",
          "コーヒー",
          "くつ"
        ],
        "answer": 2,
        "id": "N5-10-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "でんわ",
          "カレー",
          "つくえ",
          "かさ"
        ],
        "answer": 1,
        "id": "N5-10-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "やさい",
          "テレビ",
          "つくえ",
          "びょういん"
        ],
        "answer": 0,
        "id": "N5-10-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "こうえん",
          "テレビ",
          "みず",
          "パン"
        ],
        "answer": 0,
        "id": "N5-10-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "としょかん",
          "ほん",
          "えんぴつ",
          "みず"
        ],
        "answer": 0,
        "id": "N5-10-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "りんご",
          "えんぴつ",
          "テレビ",
          "コンビニ"
        ],
        "answer": 3,
        "id": "N5-10-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "うみ",
          "テレビ",
          "ほん",
          "みかん"
        ],
        "answer": 0,
        "id": "N5-10-020"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>見る<rt>みる</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "this",
          "to see/watch",
          "train",
          "last month"
        ],
        "answer": 1,
        "id": "N5-10-021"
      },
      {
        "type": "言い換え",
        "text": "「やっぱり」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "power",
          "as expected",
          "to buy",
          "sofa"
        ],
        "answer": 1,
        "id": "N5-10-022"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>右<rt>みぎ</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "right",
          "two things",
          "what",
          "company president"
        ],
        "answer": 0,
        "id": "N5-10-023"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>前<rt>まえ</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "computer",
          "front",
          "output",
          "where"
        ],
        "answer": 1,
        "id": "N5-10-024"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>外<rt>そと</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "outside",
          "bird",
          "every week",
          "correct"
        ],
        "answer": 0,
        "id": "N5-10-025"
      },
      {
        "type": "文法形式",
        "text": "つめたい　みずが　のみ（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "たい",
          "ます",
          "て",
          "る"
        ],
        "answer": 0,
        "id": "N5-10-026"
      },
      {
        "type": "文法形式",
        "text": "あたらしい　くつを　かい（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "たい",
          "る",
          "ます",
          "て"
        ],
        "answer": 0,
        "id": "N5-10-027"
      },
      {
        "type": "文法形式",
        "text": "えいがを　み（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "たい",
          "る",
          "て",
          "ます"
        ],
        "answer": 0,
        "id": "N5-10-028"
      },
      {
        "type": "文法形式",
        "text": "この　りんごは　（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "おいしい",
          "おいしく",
          "おいしくて",
          "おいしかった"
        ],
        "answer": 0,
        "id": "N5-10-029"
      },
      {
        "type": "文法形式",
        "text": "あの　えいがは　（　）でした。",
        "question": "（　）に　はいるのは？",
        "options": [
          "おもしろかった",
          "おもしろく",
          "おもしろくて",
          "おもしろい"
        ],
        "answer": 0,
        "id": "N5-10-030"
      },
      {
        "type": "文法形式",
        "text": "さくらの　はなは　（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "きれい",
          "きれいに",
          "きれいな",
          "きれいで"
        ],
        "answer": 0,
        "id": "N5-10-031"
      },
      {
        "type": "文法形式",
        "text": "これは　（　）ですか。",
        "question": "（　）に　はいるのは？",
        "options": [
          "なん",
          "どこ",
          "だれ",
          "いつ"
        ],
        "answer": 0,
        "id": "N5-10-032"
      },
      {
        "type": "文法形式",
        "text": "あの　ひとは　（　）ですか。",
        "question": "（　）に　はいるのは？",
        "options": [
          "だれ",
          "なに",
          "いつ",
          "どこ"
        ],
        "answer": 0,
        "id": "N5-10-033"
      },
      {
        "type": "文法形式",
        "text": "みずを　（　）ください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "いっぱい",
          "ひっぱい",
          "ひとはい",
          "いちはい"
        ],
        "answer": 0,
        "id": "N5-10-034"
      },
      {
        "type": "文法形式",
        "text": "かみを　（　）ください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "いちまい",
          "ひとばい",
          "ひとまい",
          "いちばい"
        ],
        "answer": 0,
        "id": "N5-10-035"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "にほんで",
          "はたらきたい",
          "です",
          "と"
        ],
        "answer": 0,
        "id": "N5-10-036"
      },
      {
        "type": "文の組み立て",
        "text": "かれは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "いしゃに",
          "なりたい",
          "です",
          "と"
        ],
        "answer": 0,
        "id": "N5-10-037"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "せんせいに",
          "なりたい",
          "です",
          "と"
        ],
        "answer": 0,
        "id": "N5-10-038"
      },
      {
        "type": "文の組み立て",
        "text": "かのじょは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "かしゅに",
          "なりたい",
          "です",
          "と"
        ],
        "answer": 0,
        "id": "N5-10-039"
      },
      {
        "type": "文の組み立て",
        "text": "こどもは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "パイロットに",
          "なりたい",
          "です",
          "と"
        ],
        "answer": 0,
        "id": "N5-10-040"
      },
      {
        "type": "文章の文法",
        "text": "あの レストランは おいしい（　）、たかいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "けど",
          "から",
          "ので",
          "のに"
        ],
        "answer": 0,
        "id": "N5-10-041"
      },
      {
        "type": "文章の文法",
        "text": "わたしは ピアノ（　）ひけます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "は",
          "に"
        ],
        "answer": 0,
        "id": "N5-10-042"
      },
      {
        "type": "文章の文法",
        "text": "この ほんは わたし（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "の",
          "に",
          "が",
          "を"
        ],
        "answer": 0,
        "id": "N5-10-043"
      },
      {
        "type": "文章の文法",
        "text": "きょう がっこうで なに（　）しましたか。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "は",
          "も"
        ],
        "answer": 0,
        "id": "N5-10-044"
      },
      {
        "type": "文章の文法",
        "text": "わたしは まいしゅう にちようびに（　）います。",
        "question": "（　）に　はいるのは？",
        "options": [
          "やすんで",
          "やすみます",
          "やすんだ",
          "やすむ"
        ],
        "answer": 0,
        "id": "N5-10-045"
      },
      {
        "type": "読解（短文）",
        "text": "このシャツは　2000えんです。きのうより　500えん　やすいです。",
        "question": "きのうは　いくらでしたか。",
        "options": [
          "1500えん",
          "2000えん",
          "2500えん",
          "3000えん"
        ],
        "answer": 1,
        "id": "N5-10-046"
      },
      {
        "type": "読解（短文）",
        "text": "コーヒーは　300えんです。ケーキは　400えんです。",
        "question": "コーヒーと　ケーキで　いくらですか。",
        "options": [
          "500えん",
          "600えん",
          "700えん",
          "800えん"
        ],
        "answer": 1,
        "id": "N5-10-047"
      },
      {
        "type": "読解（短文）",
        "text": "このみせは　やすいです。シャツが　1000えんです。",
        "question": "シャツは　いくらですか。",
        "options": [
          "500えん",
          "1000えん",
          "1500えん",
          "2000えん"
        ],
        "answer": 1,
        "id": "N5-10-048"
      },
      {
        "type": "読解（中文）",
        "text": "このカフェは　しずかで　すきです。コーヒーが　おいしいです。よく　ここで　ほんを　よみます。",
        "question": "このカフェは　どうですか。",
        "options": [
          "うるさい",
          "しずか",
          "ひろい",
          "せまい"
        ],
        "answer": 1,
        "id": "N5-10-049"
      },
      {
        "type": "読解（中文）",
        "text": "あしたは　ともだちの　たんじょうびです。パーティーを　します。ケーキを　かいます。プレゼントも　かいました。",
        "question": "あしたは　なにを　しますか。",
        "options": [
          "りょこう",
          "パーティー",
          "かいもの",
          "えいが"
        ],
        "answer": 1,
        "id": "N5-10-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-10-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "200円",
          "250円",
          "300円",
          "350円"
        ],
        "answer": 0,
        "id": "N5-10-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "150円",
          "200円",
          "250円",
          "300円"
        ],
        "answer": 2,
        "id": "N5-10-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "100円",
          "150円",
          "200円",
          "250円"
        ],
        "answer": 0,
        "id": "N5-10-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "20本",
          "21本",
          "22本",
          "23本"
        ],
        "answer": 2,
        "id": "N5-10-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "19本",
          "20本",
          "21本",
          "22本"
        ],
        "answer": 0,
        "id": "N5-10-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "18本",
          "19本",
          "20本",
          "21本"
        ],
        "answer": 2,
        "id": "N5-10-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "7時ごろ",
          "7時すぎ",
          "7時前",
          "7時ちょうど"
        ],
        "answer": 0,
        "id": "N5-10-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "8時ごろ",
          "8時すぎ",
          "8時前",
          "8時ちょうど"
        ],
        "answer": 1,
        "id": "N5-10-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "9時ごろ",
          "9時すぎ",
          "9時前",
          "9時ちょうど"
        ],
        "answer": 0,
        "id": "N5-10-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "10時ごろ",
          "10時すぎ",
          "10時前",
          "10時ちょうど"
        ],
        "answer": 1,
        "id": "N5-10-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "11時ごろ",
          "11時すぎ",
          "11時前",
          "11時ちょうど"
        ],
        "answer": 0,
        "id": "N5-10-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "12時ごろ",
          "12時すぎ",
          "12時前",
          "12時ちょうど"
        ],
        "answer": 1,
        "id": "N5-10-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "お願いします",
          "お願いできますか",
          "していただけますか",
          "可能ですか"
        ],
        "answer": 0,
        "id": "N5-10-064",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "教えてください",
          "説明してください",
          "見せてください",
          "聞かせてください"
        ],
        "answer": 0,
        "id": "N5-10-065",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "待ってください",
          "止まってください",
          "座ってください",
          "立ってください"
        ],
        "answer": 0,
        "id": "N5-10-066",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "開けてください",
          "閉めてください",
          "つけてください",
          "消してください"
        ],
        "answer": 0,
        "id": "N5-10-067",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "書いてください",
          "読んでください",
          "話してください",
          "聞いてください"
        ],
        "answer": 0,
        "id": "N5-10-068",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "2時です",
          "2時半です",
          "3時です",
          "3時半です"
        ],
        "answer": 0,
        "id": "N5-10-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "3時です",
          "3時半です",
          "4時です",
          "4時半です"
        ],
        "answer": 0,
        "id": "N5-10-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "4時です",
          "4時半です",
          "5時です",
          "5時半です"
        ],
        "answer": 0,
        "id": "N5-10-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "5時です",
          "5時半です",
          "6時です",
          "6時半です"
        ],
        "answer": 0,
        "id": "N5-10-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "6時です",
          "6時半です",
          "7時です",
          "7時半です"
        ],
        "answer": 0,
        "id": "N5-10-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "7時です",
          "7時半です",
          "8時です",
          "8時半です"
        ],
        "answer": 0,
        "id": "N5-10-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "8時です",
          "8時半です",
          "9時です",
          "9時半です"
        ],
        "answer": 0,
        "id": "N5-10-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "9時です",
          "9時半です",
          "10時です",
          "10時半です"
        ],
        "answer": 0,
        "id": "N5-10-076",
        "script": "なんさいですか。"
      }
    ],
    "11": [
      {
        "type": "漢字読み",
        "text": "<u>今日</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "きょう",
          "おおきい",
          "せん",
          "まいにち"
        ],
        "answer": 0,
        "id": "N5-11-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>今週</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "こんしゅう",
          "でんわ",
          "でる",
          "そと"
        ],
        "answer": 0,
        "id": "N5-11-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>お母さん</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "ひゃく",
          "うまれる",
          "おかあさん",
          "あめ"
        ],
        "answer": 2,
        "id": "N5-11-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>名前</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "いろいろ",
          "なまえ",
          "め",
          "まん"
        ],
        "answer": 1,
        "id": "N5-11-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>休む</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "げつようび",
          "みみ",
          "しゃちょう",
          "やすむ"
        ],
        "answer": 3,
        "id": "N5-11-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>教える</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "あいだ",
          "みず",
          "おしえる",
          "なまえ"
        ],
        "answer": 2,
        "id": "N5-11-006"
      },
      {
        "type": "表記",
        "text": "<u>つくる</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "聞く",
          "先週",
          "作る",
          "元気"
        ],
        "answer": 2,
        "id": "N5-11-007"
      },
      {
        "type": "表記",
        "text": "<u>おとうさん</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "正しい",
          "男の子",
          "お父さん",
          "長い"
        ],
        "answer": 2,
        "id": "N5-11-008"
      },
      {
        "type": "表記",
        "text": "<u>あう</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "学校",
          "今",
          "会う",
          "食べる"
        ],
        "answer": 2,
        "id": "N5-11-009"
      },
      {
        "type": "表記",
        "text": "<u>せん</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "千",
          "学生",
          "先月",
          "音"
        ],
        "answer": 0,
        "id": "N5-11-010"
      },
      {
        "type": "表記",
        "text": "<u>はなす</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "話す",
          "日曜日",
          "大きい",
          "鳥"
        ],
        "answer": 0,
        "id": "N5-11-011"
      },
      {
        "type": "表記",
        "text": "<u>ひとつ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "学生",
          "一つ",
          "前",
          "四つ"
        ],
        "answer": 1,
        "id": "N5-11-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ごはん",
          "バス",
          "びょういん",
          "でんしゃ"
        ],
        "answer": 0,
        "id": "N5-11-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "シャツ",
          "ズボン",
          "かぎ",
          "おちゃ"
        ],
        "answer": 3,
        "id": "N5-11-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "テレビ",
          "でんわ",
          "びょういん",
          "うどん"
        ],
        "answer": 3,
        "id": "N5-11-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "びょういん",
          "くるま",
          "バス",
          "さかな"
        ],
        "answer": 3,
        "id": "N5-11-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "びょういん",
          "ほん",
          "おちゃ",
          "りんご"
        ],
        "answer": 0,
        "id": "N5-11-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "きょうしつ",
          "りんご",
          "みず",
          "テレビ"
        ],
        "answer": 0,
        "id": "N5-11-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ほん",
          "コンビニ",
          "おちゃ",
          "みかん"
        ],
        "answer": 1,
        "id": "N5-11-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "うみ",
          "えんぴつ",
          "りんご",
          "パン"
        ],
        "answer": 0,
        "id": "N5-11-020"
      },
      {
        "type": "言い換え",
        "text": "「そこ」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "size",
          "become empty",
          "there",
          "be audible"
        ],
        "answer": 2,
        "id": "N5-11-021"
      },
      {
        "type": "言い換え",
        "text": "「テレビ」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "television",
          "two people",
          "man",
          "bus"
        ],
        "answer": 0,
        "id": "N5-11-022"
      },
      {
        "type": "言い換え",
        "text": "「ここ」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "no",
          "here",
          "father",
          "can do"
        ],
        "answer": 1,
        "id": "N5-11-023"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>分かる<rt>わかる</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "to understand",
          "pants",
          "camera",
          "Wednesday"
        ],
        "answer": 0,
        "id": "N5-11-024"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>知る<rt>しる</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "where",
          "to know",
          "to read",
          "flower"
        ],
        "answer": 1,
        "id": "N5-11-025"
      },
      {
        "type": "文法形式",
        "text": "おいしい（　）、たかいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "けど",
          "から",
          "て",
          "ので"
        ],
        "answer": 0,
        "id": "N5-11-026"
      },
      {
        "type": "文法形式",
        "text": "しごとが　おわっ（　）、いきましょう。",
        "question": "（　）に　はいるのは？",
        "options": [
          "たら",
          "ら",
          "から",
          "と"
        ],
        "answer": 0,
        "id": "N5-11-027"
      },
      {
        "type": "文法形式",
        "text": "もっと　べんきょう（　）ほうがいいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "した",
          "して",
          "します",
          "する"
        ],
        "answer": 0,
        "id": "N5-11-028"
      },
      {
        "type": "文法形式",
        "text": "でんき（　）けしてください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "で",
          "が",
          "に"
        ],
        "answer": 0,
        "id": "N5-11-029"
      },
      {
        "type": "文法形式",
        "text": "まど（　）あけてください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "が",
          "で"
        ],
        "answer": 0,
        "id": "N5-11-030"
      },
      {
        "type": "文法形式",
        "text": "せんせい（　）きいてください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "が",
          "で",
          "を"
        ],
        "answer": 0,
        "id": "N5-11-031"
      },
      {
        "type": "文法形式",
        "text": "ははに　てがみ（　）かきました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "が",
          "で"
        ],
        "answer": 0,
        "id": "N5-11-032"
      },
      {
        "type": "文法形式",
        "text": "こども（　）おもちゃを　あげました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "が",
          "を",
          "で"
        ],
        "answer": 0,
        "id": "N5-11-033"
      },
      {
        "type": "文法形式",
        "text": "せんせい（　）ほんを　もらいました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "で",
          "が",
          "を"
        ],
        "answer": 0,
        "id": "N5-11-034"
      },
      {
        "type": "文法形式",
        "text": "この　へや（　）はいらないでください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "を",
          "で",
          "が"
        ],
        "answer": 0,
        "id": "N5-11-035"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "ともだちと",
          "あそびました",
          "きのう",
          "で"
        ],
        "answer": 1,
        "id": "N5-11-036"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "かぞくと",
          "たべました",
          "レストラン",
          "で"
        ],
        "answer": 1,
        "id": "N5-11-037"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "せんせいと",
          "はなしました",
          "がっこう",
          "で"
        ],
        "answer": 1,
        "id": "N5-11-038"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "どうりょうと",
          "のみました",
          "いざかや",
          "で"
        ],
        "answer": 1,
        "id": "N5-11-039"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "かのじょと",
          "みました",
          "えいがかん",
          "で"
        ],
        "answer": 1,
        "id": "N5-11-040"
      },
      {
        "type": "文章の文法",
        "text": "この ビルは 20かい（　）あります。",
        "question": "（　）に　はいるのは？",
        "options": [
          "まで",
          "から",
          "より",
          "ほど"
        ],
        "answer": 0,
        "id": "N5-11-041"
      },
      {
        "type": "文章の文法",
        "text": "あした パーティーに（　）ますか。",
        "question": "（　）に　はいるのは？",
        "options": [
          "き",
          "きて",
          "きた",
          "くる"
        ],
        "answer": 0,
        "id": "N5-11-042"
      },
      {
        "type": "文章の文法",
        "text": "わたしは にほんごが すこし（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "わかります",
          "わかって",
          "わかった",
          "わかる"
        ],
        "answer": 0,
        "id": "N5-11-043"
      },
      {
        "type": "文章の文法",
        "text": "この みせは 9じ（　）あいています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "から",
          "まで",
          "に",
          "で"
        ],
        "answer": 0,
        "id": "N5-11-044"
      },
      {
        "type": "文章の文法",
        "text": "きのう てがみを（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "かきました",
          "かいて",
          "かきます",
          "かく"
        ],
        "answer": 0,
        "id": "N5-11-045"
      },
      {
        "type": "読解（短文）",
        "text": "あさごはんに　パンを　たべました。ひるごはんに　ラーメンを　たべました。",
        "question": "ひるごはんに　なにを　たべましたか。",
        "options": [
          "パン",
          "ごはん",
          "ラーメン",
          "サンドイッチ"
        ],
        "answer": 1,
        "id": "N5-11-046"
      },
      {
        "type": "読解（短文）",
        "text": "わたしは　にくが　すきです。やさいは　あまり　たべません。",
        "question": "わたしは　なにが　すきですか。",
        "options": [
          "やさい",
          "さかな",
          "にく",
          "くだもの"
        ],
        "answer": 1,
        "id": "N5-11-047"
      },
      {
        "type": "読解（短文）",
        "text": "きょうの　ばんごはんは　カレーです。あしたは　すしを　たべます。",
        "question": "あしたは　なにを　たべますか。",
        "options": [
          "カレー",
          "すし",
          "ラーメン",
          "うどん"
        ],
        "answer": 1,
        "id": "N5-11-048"
      },
      {
        "type": "読解（中文）",
        "text": "にほんの　なつは　あついです。7がつと　8がつが　いちばん　あついです。エアコンが　ひつようです。",
        "question": "いつが　いちばん　あついですか。",
        "options": [
          "5がつ",
          "6がつ",
          "7がつと　8がつ",
          "9がつ"
        ],
        "answer": 1,
        "id": "N5-11-049"
      },
      {
        "type": "読解（中文）",
        "text": "はるは　さくらが　きれいです。4がつに　おはなみを　します。こうえんで　おべんとうを　たべます。",
        "question": "はるに　なにを　しますか。",
        "options": [
          "スキー",
          "およぐ",
          "おはなみ",
          "もみじがり"
        ],
        "answer": 1,
        "id": "N5-11-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-11-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "17本",
          "18本",
          "19本",
          "20本"
        ],
        "answer": 0,
        "id": "N5-11-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "16本",
          "17本",
          "18本",
          "19本"
        ],
        "answer": 2,
        "id": "N5-11-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "15本",
          "16本",
          "17本",
          "18本"
        ],
        "answer": 0,
        "id": "N5-11-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "14本",
          "15本",
          "16本",
          "17本"
        ],
        "answer": 2,
        "id": "N5-11-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "13本",
          "14本",
          "15本",
          "16本"
        ],
        "answer": 0,
        "id": "N5-11-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "12本",
          "13本",
          "14本",
          "15本"
        ],
        "answer": 2,
        "id": "N5-11-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "月曜日",
          "火曜日",
          "水曜日",
          "木曜日"
        ],
        "answer": 0,
        "id": "N5-11-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "火曜日",
          "水曜日",
          "木曜日",
          "金曜日"
        ],
        "answer": 1,
        "id": "N5-11-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "水曜日",
          "木曜日",
          "金曜日",
          "土曜日"
        ],
        "answer": 0,
        "id": "N5-11-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "木曜日",
          "金曜日",
          "土曜日",
          "日曜日"
        ],
        "answer": 1,
        "id": "N5-11-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "金曜日",
          "土曜日",
          "日曜日",
          "月曜日"
        ],
        "answer": 0,
        "id": "N5-11-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "土曜日",
          "日曜日",
          "月曜日",
          "火曜日"
        ],
        "answer": 1,
        "id": "N5-11-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "来てください",
          "行ってください",
          "帰ってください",
          "出てください"
        ],
        "answer": 0,
        "id": "N5-11-064",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "持ってきてください",
          "取ってきてください",
          "連れてきてください",
          "呼んできてください"
        ],
        "answer": 0,
        "id": "N5-11-065",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "使ってもいいですか",
          "借りてもいいですか",
          "見てもいいですか",
          "触ってもいいですか"
        ],
        "answer": 0,
        "id": "N5-11-066",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "入ってもいいですか",
          "出てもいいですか",
          "座ってもいいですか",
          "立ってもいいですか"
        ],
        "answer": 0,
        "id": "N5-11-067",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "写真を撮ってもいいですか",
          "録音してもいいですか",
          "メモしてもいいですか",
          "質問してもいいですか"
        ],
        "answer": 0,
        "id": "N5-11-068",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "10時です",
          "10時半です",
          "11時です",
          "11時半です"
        ],
        "answer": 0,
        "id": "N5-11-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "11時です",
          "11時半です",
          "12時です",
          "12時半です"
        ],
        "answer": 0,
        "id": "N5-11-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "12時です",
          "12時半です",
          "1時です",
          "1時半です"
        ],
        "answer": 0,
        "id": "N5-11-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "1時ごろです",
          "1時過ぎです",
          "1時前です",
          "1時ちょうどです"
        ],
        "answer": 0,
        "id": "N5-11-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "2時ごろです",
          "2時過ぎです",
          "2時前です",
          "2時ちょうどです"
        ],
        "answer": 0,
        "id": "N5-11-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "3時ごろです",
          "3時過ぎです",
          "3時前です",
          "3時ちょうどです"
        ],
        "answer": 0,
        "id": "N5-11-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "4時ごろです",
          "4時過ぎです",
          "4時前です",
          "4時ちょうどです"
        ],
        "answer": 0,
        "id": "N5-11-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "5時ごろです",
          "5時過ぎです",
          "5時前です",
          "5時ちょうどです"
        ],
        "answer": 0,
        "id": "N5-11-076",
        "script": "なんさいですか。"
      }
    ],
    "12": [
      {
        "type": "漢字読み",
        "text": "<u>一人</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "ひとり",
          "おみず",
          "じかん",
          "あした"
        ],
        "answer": 0,
        "id": "N5-12-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>間</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "あいだ",
          "くに",
          "じゅうぶん",
          "おかね"
        ],
        "answer": 0,
        "id": "N5-12-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>分ける</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "しる",
          "わける",
          "げつようび",
          "あし"
        ],
        "answer": 1,
        "id": "N5-12-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>金曜日</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "でんわ",
          "ときどき",
          "きんようび",
          "いま"
        ],
        "answer": 2,
        "id": "N5-12-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>知る</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "おとうさん",
          "あう",
          "しる",
          "よむ"
        ],
        "answer": 2,
        "id": "N5-12-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>外</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "にく",
          "おかあさん",
          "らいねん",
          "そと"
        ],
        "answer": 3,
        "id": "N5-12-006"
      },
      {
        "type": "表記",
        "text": "<u>がくせい</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "学生",
          "足",
          "十分",
          "お母さん"
        ],
        "answer": 0,
        "id": "N5-12-007"
      },
      {
        "type": "表記",
        "text": "<u>なに</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "午後",
          "何",
          "火曜日",
          "国"
        ],
        "answer": 1,
        "id": "N5-12-008"
      },
      {
        "type": "表記",
        "text": "<u>おしえる</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "見つける",
          "教える",
          "外国",
          "会計"
        ],
        "answer": 1,
        "id": "N5-12-009"
      },
      {
        "type": "表記",
        "text": "<u>まいつき</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "午後",
          "女の子",
          "分",
          "毎月"
        ],
        "answer": 3,
        "id": "N5-12-010"
      },
      {
        "type": "表記",
        "text": "<u>おんがく</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "社長",
          "高い",
          "音楽",
          "来年"
        ],
        "answer": 2,
        "id": "N5-12-011"
      },
      {
        "type": "表記",
        "text": "<u>ゆき</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "木",
          "駅",
          "左",
          "雪"
        ],
        "answer": 3,
        "id": "N5-12-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "パン",
          "いす",
          "くるま",
          "かさ"
        ],
        "answer": 0,
        "id": "N5-12-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みず",
          "ぼうし",
          "ほん",
          "ノート"
        ],
        "answer": 0,
        "id": "N5-12-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "でんわ",
          "いす",
          "かばん",
          "カレー"
        ],
        "answer": 3,
        "id": "N5-12-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "びょういん",
          "でんわ",
          "さかな",
          "つくえ"
        ],
        "answer": 2,
        "id": "N5-12-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "としょかん",
          "りんご",
          "おちゃ",
          "パン"
        ],
        "answer": 0,
        "id": "N5-12-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "へや",
          "えんぴつ",
          "テレビ",
          "りんご"
        ],
        "answer": 0,
        "id": "N5-12-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "おちゃ",
          "ほん",
          "みず",
          "デパート"
        ],
        "answer": 3,
        "id": "N5-12-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "えんぴつ",
          "えいがかん",
          "みず",
          "りんご"
        ],
        "answer": 1,
        "id": "N5-12-020"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>駅<rt>えき</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "bright",
          "station",
          "Sunday",
          "oneself"
        ],
        "answer": 1,
        "id": "N5-12-021"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>会計<rt>かいけい</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "left",
          "ten thousand",
          "bill",
          "to exit/leave"
        ],
        "answer": 2,
        "id": "N5-12-022"
      },
      {
        "type": "言い換え",
        "text": "「これ」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "this",
          "thanks for meal",
          "power",
          "what"
        ],
        "answer": 0,
        "id": "N5-12-023"
      },
      {
        "type": "言い換え",
        "text": "「それ」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "to go",
          "that",
          "half",
          "ear"
        ],
        "answer": 1,
        "id": "N5-12-024"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>土曜日<rt>どようび</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "good morning",
          "to know",
          "same",
          "Saturday"
        ],
        "answer": 3,
        "id": "N5-12-025"
      },
      {
        "type": "文法形式",
        "text": "きって（　）かいたいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "で",
          "が"
        ],
        "answer": 0,
        "id": "N5-12-026"
      },
      {
        "type": "文法形式",
        "text": "にほん（　）すみたいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "を",
          "が",
          "で"
        ],
        "answer": 0,
        "id": "N5-12-027"
      },
      {
        "type": "文法形式",
        "text": "かんじ（　）よめますか。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "で",
          "に"
        ],
        "answer": 0,
        "id": "N5-12-028"
      },
      {
        "type": "文法形式",
        "text": "くるま（　）うんてんできます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "に",
          "を",
          "で"
        ],
        "answer": 0,
        "id": "N5-12-029"
      },
      {
        "type": "文法形式",
        "text": "ピアノ（　）ひけますか。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "に",
          "で",
          "を"
        ],
        "answer": 0,
        "id": "N5-12-030"
      },
      {
        "type": "文法形式",
        "text": "なに（　）たべたいですか。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "に",
          "を",
          "で"
        ],
        "answer": 0,
        "id": "N5-12-031"
      },
      {
        "type": "文法形式",
        "text": "どこ（　）いきたいですか。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "が",
          "を",
          "で"
        ],
        "answer": 0,
        "id": "N5-12-032"
      },
      {
        "type": "文法形式",
        "text": "だれ（　）あいたいですか。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "が",
          "を",
          "で"
        ],
        "answer": 0,
        "id": "N5-12-033"
      },
      {
        "type": "文法形式",
        "text": "ここ（　）すわってください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "が",
          "で",
          "を"
        ],
        "answer": 0,
        "id": "N5-12-034"
      },
      {
        "type": "文法形式",
        "text": "くつ（　）ぬいでください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "で",
          "に"
        ],
        "answer": 0,
        "id": "N5-12-035"
      },
      {
        "type": "文の組み立て",
        "text": "にほんごを ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "べんきょうして",
          "います",
          "から",
          "いちねん"
        ],
        "answer": 0,
        "id": "N5-12-036"
      },
      {
        "type": "文の組み立て",
        "text": "ピアノを ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "ならって",
          "います",
          "から",
          "こども"
        ],
        "answer": 0,
        "id": "N5-12-037"
      },
      {
        "type": "文の組み立て",
        "text": "テニスを ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "して",
          "います",
          "から",
          "さんねん"
        ],
        "answer": 0,
        "id": "N5-12-038"
      },
      {
        "type": "文の組み立て",
        "text": "りょうりを ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "つくって",
          "います",
          "から",
          "いま"
        ],
        "answer": 0,
        "id": "N5-12-039"
      },
      {
        "type": "文の組み立て",
        "text": "おんがくを ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "きいて",
          "います",
          "から",
          "さっき"
        ],
        "answer": 0,
        "id": "N5-12-040"
      },
      {
        "type": "文章の文法",
        "text": "わたしは さかなが（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "きらい",
          "きらいな",
          "きらいに",
          "きらいで"
        ],
        "answer": 0,
        "id": "N5-12-041"
      },
      {
        "type": "文章の文法",
        "text": "この くるまは（　）はやいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "とても",
          "あまり",
          "ぜんぜん",
          "すこし"
        ],
        "answer": 0,
        "id": "N5-12-042"
      },
      {
        "type": "文章の文法",
        "text": "あしたは やすみ（　）、どこにもいきません。",
        "question": "（　）に　はいるのは？",
        "options": [
          "だから",
          "なので",
          "けど",
          "のに"
        ],
        "answer": 0,
        "id": "N5-12-043"
      },
      {
        "type": "文章の文法",
        "text": "わたしは コーヒーを（　）のみます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "よく",
          "とても",
          "あまり",
          "ぜんぜん"
        ],
        "answer": 0,
        "id": "N5-12-044"
      },
      {
        "type": "文章の文法",
        "text": "この りんごは（　）おいしいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "とても",
          "あまり",
          "ぜんぜん",
          "まだ"
        ],
        "answer": 0,
        "id": "N5-12-045"
      },
      {
        "type": "読解（短文）",
        "text": "このみせの　ラーメンは　600えんです。おおもりは　100えん　たかいです。",
        "question": "おおもりは　いくらですか。",
        "options": [
          "500えん",
          "600えん",
          "700えん",
          "800えん"
        ],
        "answer": 1,
        "id": "N5-12-046"
      },
      {
        "type": "読解（短文）",
        "text": "コーヒーは　ホットと　アイスが　あります。わたしは　ホットを　のみます。",
        "question": "わたしは　なにを　のみますか。",
        "options": [
          "アイスコーヒー",
          "ホットコーヒー",
          "ホットミルク",
          "アイスティー"
        ],
        "answer": 1,
        "id": "N5-12-047"
      },
      {
        "type": "読解（短文）",
        "text": "えきから　がっこうまで　あるいて　10ぷんです。",
        "question": "がっこうまで　どのくらい　かかりますか。",
        "options": [
          "5ふん",
          "10ぷん",
          "15ふん",
          "20ぷん"
        ],
        "answer": 1,
        "id": "N5-12-048"
      },
      {
        "type": "読解（中文）",
        "text": "きょうは　あめです。かさを　もっていきます。あしたは　はれの　よほうです。せんたくが　できます。",
        "question": "あしたの　てんきは　どうですか。",
        "options": [
          "あめ",
          "くもり",
          "はれ",
          "ゆき"
        ],
        "answer": 1,
        "id": "N5-12-049"
      },
      {
        "type": "読解（中文）",
        "text": "ふゆは　さむいです。セーターや　コートを　きます。ゆきの　ひは　とても　さむいです。",
        "question": "ふゆに　なにを　きますか。",
        "options": [
          "Tシャツ",
          "セーターや　コート",
          "みずぎ",
          "はんズボン"
        ],
        "answer": 1,
        "id": "N5-12-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-12-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "11本",
          "12本",
          "13本",
          "14本"
        ],
        "answer": 0,
        "id": "N5-12-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "10本",
          "11本",
          "12本",
          "13本"
        ],
        "answer": 2,
        "id": "N5-12-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "9本",
          "10本",
          "11本",
          "12本"
        ],
        "answer": 0,
        "id": "N5-12-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "8本",
          "9本",
          "10本",
          "11本"
        ],
        "answer": 2,
        "id": "N5-12-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "7本",
          "8本",
          "9本",
          "10本"
        ],
        "answer": 0,
        "id": "N5-12-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "6本",
          "7本",
          "8本",
          "9本"
        ],
        "answer": 2,
        "id": "N5-12-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "日曜日",
          "月曜日",
          "火曜日",
          "水曜日"
        ],
        "answer": 0,
        "id": "N5-12-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "1日",
          "2日",
          "3日",
          "4日"
        ],
        "answer": 1,
        "id": "N5-12-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "2日",
          "3日",
          "4日",
          "5日"
        ],
        "answer": 0,
        "id": "N5-12-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "3日",
          "4日",
          "5日",
          "6日"
        ],
        "answer": 1,
        "id": "N5-12-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "4日",
          "5日",
          "6日",
          "7日"
        ],
        "answer": 0,
        "id": "N5-12-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "5日",
          "6日",
          "7日",
          "8日"
        ],
        "answer": 1,
        "id": "N5-12-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "ちょっといいですか",
          "少しよろしいですか",
          "お時間ありますか",
          "話があります"
        ],
        "answer": 0,
        "id": "N5-12-064",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "もう一度お願いします",
          "ゆっくりお願いします",
          "大きい声でお願いします",
          "書いてください"
        ],
        "answer": 0,
        "id": "N5-12-065",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "すみませんが",
          "恐れ入りますが",
          "お手数ですが",
          "失礼ですが"
        ],
        "answer": 0,
        "id": "N5-12-066",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "どうぞ",
          "ご自由に",
          "お使いください",
          "お座りください"
        ],
        "answer": 0,
        "id": "N5-12-067",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "こちらへどうぞ",
          "お入りください",
          "お上がりください",
          "お座りください"
        ],
        "answer": 0,
        "id": "N5-12-068",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "6時ごろです",
          "6時過ぎです",
          "6時前です",
          "6時ちょうどです"
        ],
        "answer": 0,
        "id": "N5-12-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "7時ごろです",
          "7時過ぎです",
          "7時前です",
          "7時ちょうどです"
        ],
        "answer": 0,
        "id": "N5-12-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "8時ごろです",
          "8時過ぎです",
          "8時前です",
          "8時ちょうどです"
        ],
        "answer": 0,
        "id": "N5-12-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "9時ごろです",
          "9時過ぎです",
          "9時前です",
          "9時ちょうどです"
        ],
        "answer": 0,
        "id": "N5-12-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "10時ごろです",
          "10時過ぎです",
          "10時前です",
          "10時ちょうどです"
        ],
        "answer": 0,
        "id": "N5-12-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "11時ごろです",
          "11時過ぎです",
          "11時前です",
          "11時ちょうどです"
        ],
        "answer": 0,
        "id": "N5-12-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "12時ごろです",
          "12時過ぎです",
          "12時前です",
          "12時ちょうどです"
        ],
        "answer": 0,
        "id": "N5-12-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "午前1時です",
          "午後1時です",
          "1時です",
          "1時半です"
        ],
        "answer": 0,
        "id": "N5-12-076",
        "script": "なんさいですか。"
      }
    ],
    "13": [
      {
        "type": "漢字読み",
        "text": "<u>下手</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "おかね",
          "なか",
          "つくる",
          "へた"
        ],
        "answer": 3,
        "id": "N5-13-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>色</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "いろ",
          "かく",
          "しゃかい",
          "あう"
        ],
        "answer": 0,
        "id": "N5-13-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>耳</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "おと",
          "みみ",
          "にゅうがく",
          "て"
        ],
        "answer": 1,
        "id": "N5-13-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>右</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "でぐち",
          "にゅうりょく",
          "みぎ",
          "おとこ"
        ],
        "answer": 2,
        "id": "N5-13-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>鳥</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "おかね",
          "ひとつ",
          "せんせい",
          "とり"
        ],
        "answer": 3,
        "id": "N5-13-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>日曜日</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "のむ",
          "きょうしつ",
          "にちようび",
          "そと"
        ],
        "answer": 2,
        "id": "N5-13-006"
      },
      {
        "type": "表記",
        "text": "<u>うえ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "上",
          "知る",
          "千",
          "下手"
        ],
        "answer": 0,
        "id": "N5-13-007"
      },
      {
        "type": "表記",
        "text": "<u>うまれる</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "教室",
          "見える",
          "四つ",
          "生まれる"
        ],
        "answer": 3,
        "id": "N5-13-008"
      },
      {
        "type": "表記",
        "text": "<u>らいしゅう</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "百",
          "来週",
          "午後",
          "一本"
        ],
        "answer": 1,
        "id": "N5-13-009"
      },
      {
        "type": "表記",
        "text": "<u>みっつ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "三つ",
          "耳",
          "元気",
          "日曜日"
        ],
        "answer": 0,
        "id": "N5-13-010"
      },
      {
        "type": "表記",
        "text": "<u>とけい</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "今",
          "学生",
          "時計",
          "口"
        ],
        "answer": 2,
        "id": "N5-13-011"
      },
      {
        "type": "表記",
        "text": "<u>ひだり</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "会う",
          "雨",
          "川",
          "左"
        ],
        "answer": 3,
        "id": "N5-13-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "かさ",
          "いす",
          "やさい",
          "かばん"
        ],
        "answer": 2,
        "id": "N5-13-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ほん",
          "ズボン",
          "みず",
          "くつ"
        ],
        "answer": 2,
        "id": "N5-13-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "テレビ",
          "かばん",
          "でんわ",
          "サンドイッチ"
        ],
        "answer": 3,
        "id": "N5-13-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "かばん",
          "つくえ",
          "くるま",
          "すし"
        ],
        "answer": 3,
        "id": "N5-13-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "えき",
          "えんぴつ",
          "テレビ",
          "ほん"
        ],
        "answer": 0,
        "id": "N5-13-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "パン",
          "おちゃ",
          "テレビ",
          "いえ"
        ],
        "answer": 3,
        "id": "N5-13-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みかん",
          "えんぴつ",
          "テレビ",
          "みせ"
        ],
        "answer": 3,
        "id": "N5-13-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "びじゅつかん",
          "みかん",
          "ほん",
          "りんご"
        ],
        "answer": 0,
        "id": "N5-13-020"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>学生<rt>がくせい</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "student",
          "bed",
          "rain",
          "here"
        ],
        "answer": 0,
        "id": "N5-13-021"
      },
      {
        "type": "言い換え",
        "text": "「おはよう」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "if",
          "this week",
          "good morning",
          "supermarket"
        ],
        "answer": 2,
        "id": "N5-13-022"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>女<rt>おんな</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "woman",
          "university",
          "to rest",
          "one thing"
        ],
        "answer": 0,
        "id": "N5-13-023"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>三つ<rt>みっつ</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "electricity",
          "what",
          "three things",
          "inside"
        ],
        "answer": 2,
        "id": "N5-13-024"
      },
      {
        "type": "言い換え",
        "text": "「スーパー」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "fun",
          "necktie",
          "supermarket",
          "hand"
        ],
        "answer": 2,
        "id": "N5-13-025"
      },
      {
        "type": "文法形式",
        "text": "めがね（　）かけています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "が",
          "で"
        ],
        "answer": 0,
        "id": "N5-13-026"
      },
      {
        "type": "文法形式",
        "text": "ぼうし（　）かぶっています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "に",
          "で"
        ],
        "answer": 0,
        "id": "N5-13-027"
      },
      {
        "type": "文法形式",
        "text": "コート（　）きています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "で",
          "が"
        ],
        "answer": 0,
        "id": "N5-13-028"
      },
      {
        "type": "文法形式",
        "text": "でんわ（　）はなしています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "を",
          "に",
          "が"
        ],
        "answer": 0,
        "id": "N5-13-029"
      },
      {
        "type": "文法形式",
        "text": "にほんご（　）かきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "が",
          "を",
          "に"
        ],
        "answer": 0,
        "id": "N5-13-030"
      },
      {
        "type": "文法形式",
        "text": "えんぴつ（　）かきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "が",
          "を",
          "に"
        ],
        "answer": 0,
        "id": "N5-13-031"
      },
      {
        "type": "文法形式",
        "text": "はさみ（　）きります。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "に",
          "を",
          "が"
        ],
        "answer": 0,
        "id": "N5-13-032"
      },
      {
        "type": "文法形式",
        "text": "じてんしゃ（　）いきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "を",
          "に",
          "が"
        ],
        "answer": 0,
        "id": "N5-13-033"
      },
      {
        "type": "文法形式",
        "text": "バス（　）いきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "に",
          "が",
          "を"
        ],
        "answer": 0,
        "id": "N5-13-034"
      },
      {
        "type": "文法形式",
        "text": "ひこうき（　）いきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "が",
          "を",
          "に"
        ],
        "answer": 0,
        "id": "N5-13-035"
      },
      {
        "type": "文の組み立て",
        "text": "この ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "くだものは",
          "たかい",
          "です",
          "とても"
        ],
        "answer": 1,
        "id": "N5-13-036"
      },
      {
        "type": "文の組み立て",
        "text": "あの ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "やまは",
          "たかい",
          "です",
          "とても"
        ],
        "answer": 1,
        "id": "N5-13-037"
      },
      {
        "type": "文の組み立て",
        "text": "この ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "かわは",
          "ながい",
          "です",
          "とても"
        ],
        "answer": 1,
        "id": "N5-13-038"
      },
      {
        "type": "文の組み立て",
        "text": "あの ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "はしは",
          "ふるい",
          "です",
          "とても"
        ],
        "answer": 1,
        "id": "N5-13-039"
      },
      {
        "type": "文の組み立て",
        "text": "この ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "まちは",
          "にぎやか",
          "です",
          "とても"
        ],
        "answer": 1,
        "id": "N5-13-040"
      },
      {
        "type": "文章の文法",
        "text": "がっこうは うち（　）ちかいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "から",
          "より",
          "まで",
          "に"
        ],
        "answer": 0,
        "id": "N5-13-041"
      },
      {
        "type": "文章の文法",
        "text": "わたしは まいにち 6じかん（　）ねます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "ぐらい",
          "しか",
          "だけ",
          "ほど"
        ],
        "answer": 0,
        "id": "N5-13-042"
      },
      {
        "type": "文章の文法",
        "text": "この えいがは 2じかん（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "ぐらい",
          "しか",
          "だけ",
          "ほど"
        ],
        "answer": 0,
        "id": "N5-13-043"
      },
      {
        "type": "文章の文法",
        "text": "わたしは にほんご（　）えいごが できます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "と",
          "や",
          "も",
          "か"
        ],
        "answer": 0,
        "id": "N5-13-044"
      },
      {
        "type": "文章の文法",
        "text": "きょうは あつい（　）、エアコンをつけました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "ので",
          "から",
          "けど",
          "のに"
        ],
        "answer": 0,
        "id": "N5-13-045"
      },
      {
        "type": "読解（短文）",
        "text": "わたしは　まいにち　じてんしゃで　かいしゃに　いきます。",
        "question": "なにで　かいしゃに　いきますか。",
        "options": [
          "でんしゃ",
          "バス",
          "じてんしゃ",
          "くるま"
        ],
        "answer": 1,
        "id": "N5-13-046"
      },
      {
        "type": "読解（短文）",
        "text": "とうきょうから　おおさかまで　しんかんせんで　2じかん30ぷんです。",
        "question": "どのくらい　かかりますか。",
        "options": [
          "1じかん",
          "2じかん",
          "2じかん30ぷん",
          "3じかん"
        ],
        "answer": 1,
        "id": "N5-13-047"
      },
      {
        "type": "読解（短文）",
        "text": "バスは　15ふんおきに　きます。でんしゃは　5ふんおきに　きます。",
        "question": "でんしゃは　なんぷんおきに　きますか。",
        "options": [
          "5ふん",
          "10ぷん",
          "15ふん",
          "20ぷん"
        ],
        "answer": 1,
        "id": "N5-13-048"
      },
      {
        "type": "読解（中文）",
        "text": "きのうは　てんきが　よかったです。こうえんで　さんぽを　しました。きょうは　くもりです。すこし　さむいです。",
        "question": "きのうの　てんきは　どうでしたか。",
        "options": [
          "あめ",
          "くもり",
          "よかった",
          "ゆき"
        ],
        "answer": 1,
        "id": "N5-13-049"
      },
      {
        "type": "読解（中文）",
        "text": "わたしの　しゅみは　おんがくを　きくことです。いちばん　すきな　かしゅは　あいみょんです。コンサートに　いきたいです。",
        "question": "いちばん　すきな　かしゅは　だれですか。",
        "options": [
          "YOASOBI",
          "あいみょん",
          "ひげだん",
          "Mrs. GREEN APPLE"
        ],
        "answer": 1,
        "id": "N5-13-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-13-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "5本",
          "6本",
          "7本",
          "8本"
        ],
        "answer": 0,
        "id": "N5-13-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "4本",
          "5本",
          "6本",
          "7本"
        ],
        "answer": 2,
        "id": "N5-13-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "3本",
          "4本",
          "5本",
          "6本"
        ],
        "answer": 0,
        "id": "N5-13-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "2本",
          "3本",
          "4本",
          "5本"
        ],
        "answer": 2,
        "id": "N5-13-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "1本",
          "2本",
          "3本",
          "4本"
        ],
        "answer": 0,
        "id": "N5-13-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "20枚",
          "21枚",
          "22枚",
          "23枚"
        ],
        "answer": 2,
        "id": "N5-13-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "6日",
          "7日",
          "8日",
          "9日"
        ],
        "answer": 0,
        "id": "N5-13-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "7日",
          "8日",
          "9日",
          "10日"
        ],
        "answer": 1,
        "id": "N5-13-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "8日",
          "9日",
          "10日",
          "11日"
        ],
        "answer": 0,
        "id": "N5-13-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "9日",
          "10日",
          "11日",
          "12日"
        ],
        "answer": 1,
        "id": "N5-13-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "10日",
          "11日",
          "12日",
          "13日"
        ],
        "answer": 0,
        "id": "N5-13-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "11日",
          "12日",
          "13日",
          "14日"
        ],
        "answer": 1,
        "id": "N5-13-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "お先にどうぞ",
          "どうぞ先に",
          "お待ちください",
          "お並びください"
        ],
        "answer": 0,
        "id": "N5-13-064",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "ご案内します",
          "こちらです",
          "あちらです",
          "お連れします"
        ],
        "answer": 0,
        "id": "N5-13-065",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "確認させてください",
          "調べさせてください",
          "考えさせてください",
          "相談させてください"
        ],
        "answer": 0,
        "id": "N5-13-066",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "少々お待ちください",
          "しばらくお待ちください",
          "もう少しお待ちください",
          "お待たせしました"
        ],
        "answer": 0,
        "id": "N5-13-067",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "申し訳ございません",
          "大変申し訳ございません",
          "ご迷惑をおかけしました",
          "お詫び申し上げます"
        ],
        "answer": 0,
        "id": "N5-13-068",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "午前2時です",
          "午後2時です",
          "2時です",
          "2時半です"
        ],
        "answer": 0,
        "id": "N5-13-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "午前3時です",
          "午後3時です",
          "3時です",
          "3時半です"
        ],
        "answer": 0,
        "id": "N5-13-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "午前4時です",
          "午後4時です",
          "4時です",
          "4時半です"
        ],
        "answer": 0,
        "id": "N5-13-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "午前5時です",
          "午後5時です",
          "5時です",
          "5時半です"
        ],
        "answer": 0,
        "id": "N5-13-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "午前6時です",
          "午後6時です",
          "6時です",
          "6時半です"
        ],
        "answer": 0,
        "id": "N5-13-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "午前7時です",
          "午後7時です",
          "7時です",
          "7時半です"
        ],
        "answer": 0,
        "id": "N5-13-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "午前8時です",
          "午後8時です",
          "8時です",
          "8時半です"
        ],
        "answer": 0,
        "id": "N5-13-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "午前9時です",
          "午後9時です",
          "9時です",
          "9時半です"
        ],
        "answer": 0,
        "id": "N5-13-076",
        "script": "なんさいですか。"
      }
    ],
    "14": [
      {
        "type": "漢字読み",
        "text": "<u>会計</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "かいけい",
          "がっこう",
          "ちち",
          "くる"
        ],
        "answer": 0,
        "id": "N5-14-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>大学</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "どようび",
          "だいがく",
          "にちようび",
          "て"
        ],
        "answer": 1,
        "id": "N5-14-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>入る</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "うしろ",
          "いま",
          "はいる",
          "せん"
        ],
        "answer": 2,
        "id": "N5-14-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>読む</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "かく",
          "ふたつ",
          "あとで",
          "よむ"
        ],
        "answer": 3,
        "id": "N5-14-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>話す</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "せんげつ",
          "はなす",
          "にゅうがく",
          "かく"
        ],
        "answer": 1,
        "id": "N5-14-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>雪</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "ちいさい",
          "よっつ",
          "ゆき",
          "きく"
        ],
        "answer": 2,
        "id": "N5-14-006"
      },
      {
        "type": "表記",
        "text": "<u>かいけい</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "雪",
          "時間",
          "会計",
          "元気"
        ],
        "answer": 2,
        "id": "N5-14-007"
      },
      {
        "type": "表記",
        "text": "<u>きんようび</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "金曜日",
          "目",
          "楽しい",
          "車"
        ],
        "answer": 0,
        "id": "N5-14-008"
      },
      {
        "type": "表記",
        "text": "<u>いきる</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "学校",
          "色々",
          "毎日",
          "生きる"
        ],
        "answer": 3,
        "id": "N5-14-009"
      },
      {
        "type": "表記",
        "text": "<u>へた</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "下手",
          "見る",
          "時計",
          "教える"
        ],
        "answer": 0,
        "id": "N5-14-010"
      },
      {
        "type": "表記",
        "text": "<u>うしろ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "聞く",
          "後ろ",
          "大きい",
          "高い"
        ],
        "answer": 1,
        "id": "N5-14-011"
      },
      {
        "type": "表記",
        "text": "<u>たのしい</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "下",
          "時間",
          "楽しい",
          "十分"
        ],
        "answer": 2,
        "id": "N5-14-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "びょういん",
          "でんわ",
          "いす",
          "パン"
        ],
        "answer": 3,
        "id": "N5-14-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "えんぴつ",
          "ジュース",
          "めがね",
          "シャツ"
        ],
        "answer": 1,
        "id": "N5-14-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "いす",
          "えき",
          "でんしゃ",
          "カレー"
        ],
        "answer": 3,
        "id": "N5-14-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "さかな",
          "いす",
          "つくえ",
          "バス"
        ],
        "answer": 0,
        "id": "N5-14-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ぎんこう",
          "テレビ",
          "えんぴつ",
          "りんご"
        ],
        "answer": 0,
        "id": "N5-14-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "パン",
          "いえ",
          "りんご",
          "みかん"
        ],
        "answer": 1,
        "id": "N5-14-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ほん",
          "スーパー",
          "パン",
          "テレビ"
        ],
        "answer": 1,
        "id": "N5-14-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みず",
          "びじゅつかん",
          "みかん",
          "ほん"
        ],
        "answer": 1,
        "id": "N5-14-020"
      },
      {
        "type": "言い換え",
        "text": "「いただきます」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "fish",
          "sweater",
          "thanks for meal",
          "bon appetit"
        ],
        "answer": 3,
        "id": "N5-14-021"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>見える<rt>みえる</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "goodbye",
          "sweater",
          "toilet",
          "be visible"
        ],
        "answer": 3,
        "id": "N5-14-022"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>二人<rt>ふたり</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "bag",
          "two people",
          "table",
          "water"
        ],
        "answer": 1,
        "id": "N5-14-023"
      },
      {
        "type": "言い換え",
        "text": "「どう」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "above",
          "how",
          "why",
          "but"
        ],
        "answer": 1,
        "id": "N5-14-024"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>木曜日<rt>もくようび</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "rain",
          "Thursday",
          "many",
          "moon"
        ],
        "answer": 1,
        "id": "N5-14-025"
      },
      {
        "type": "文法形式",
        "text": "ふね（　）いきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "を",
          "に",
          "が"
        ],
        "answer": 0,
        "id": "N5-14-026"
      },
      {
        "type": "文法形式",
        "text": "タクシー（　）いきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "を",
          "に",
          "が"
        ],
        "answer": 0,
        "id": "N5-14-027"
      },
      {
        "type": "文法形式",
        "text": "3にん（　）いきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "を",
          "が",
          "に"
        ],
        "answer": 0,
        "id": "N5-14-028"
      },
      {
        "type": "文法形式",
        "text": "ひとり（　）すんでいます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "を",
          "が",
          "に"
        ],
        "answer": 0,
        "id": "N5-14-029"
      },
      {
        "type": "文法形式",
        "text": "かぞく（　）すんでいます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "と",
          "に",
          "が",
          "で"
        ],
        "answer": 0,
        "id": "N5-14-030"
      },
      {
        "type": "文法形式",
        "text": "みんな（　）たべましょう。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "を",
          "が",
          "に"
        ],
        "answer": 0,
        "id": "N5-14-031"
      },
      {
        "type": "文法形式",
        "text": "いっしょ（　）いきましょう。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "を",
          "で",
          "が"
        ],
        "answer": 0,
        "id": "N5-14-032"
      },
      {
        "type": "文法形式",
        "text": "しずか（　）してください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "と",
          "で",
          "を"
        ],
        "answer": 0,
        "id": "N5-14-033"
      },
      {
        "type": "文法形式",
        "text": "きれい（　）してください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "と",
          "を",
          "で"
        ],
        "answer": 0,
        "id": "N5-14-034"
      },
      {
        "type": "文法形式",
        "text": "げんき（　）なってください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "と",
          "を",
          "で"
        ],
        "answer": 0,
        "id": "N5-14-035"
      },
      {
        "type": "文の組み立て",
        "text": "わたしの ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "しゅみは",
          "どくしょ",
          "です",
          "と"
        ],
        "answer": 0,
        "id": "N5-14-036"
      },
      {
        "type": "文の組み立て",
        "text": "わたしの ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "しゅみは",
          "りょこう",
          "です",
          "と"
        ],
        "answer": 0,
        "id": "N5-14-037"
      },
      {
        "type": "文の組み立て",
        "text": "わたしの ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "しゅみは",
          "りょうり",
          "です",
          "と"
        ],
        "answer": 0,
        "id": "N5-14-038"
      },
      {
        "type": "文の組み立て",
        "text": "わたしの ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "しゅみは",
          "スポーツ",
          "です",
          "と"
        ],
        "answer": 0,
        "id": "N5-14-039"
      },
      {
        "type": "文の組み立て",
        "text": "わたしの ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "しゅみは",
          "えいが",
          "です",
          "と"
        ],
        "answer": 0,
        "id": "N5-14-040"
      },
      {
        "type": "文章の文法",
        "text": "この レストランは いつも（　）います。",
        "question": "（　）に　はいるのは？",
        "options": [
          "こんで",
          "こみます",
          "こんだ",
          "こむ"
        ],
        "answer": 0,
        "id": "N5-14-041"
      },
      {
        "type": "文章の文法",
        "text": "わたしは ラーメン（　）カレーが すきです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "と",
          "や",
          "も",
          "か"
        ],
        "answer": 0,
        "id": "N5-14-042"
      },
      {
        "type": "文章の文法",
        "text": "あした えき（　）あいましょう。",
        "question": "（　）に　はいるのは？",
        "options": [
          "で",
          "に",
          "を",
          "が"
        ],
        "answer": 0,
        "id": "N5-14-043"
      },
      {
        "type": "文章の文法",
        "text": "この ケーキは やすくて（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "おいしい",
          "おいしく",
          "おいしいな",
          "おいしくて"
        ],
        "answer": 0,
        "id": "N5-14-044"
      },
      {
        "type": "文章の文法",
        "text": "わたしは にほんに（　）ことがあります。",
        "question": "（　）に　はいるのは？",
        "options": [
          "いった",
          "いって",
          "いきます",
          "いく"
        ],
        "answer": 0,
        "id": "N5-14-045"
      },
      {
        "type": "読解（短文）",
        "text": "ひこうきは　9じに　しゅっぱつします。くうこうに　7じに　つきます。",
        "question": "ひこうきは　なんじに　しゅっぱつしますか。",
        "options": [
          "7じ",
          "8じ",
          "9じ",
          "10じ"
        ],
        "answer": 1,
        "id": "N5-14-046"
      },
      {
        "type": "読解（短文）",
        "text": "わたしは　まいあさ　シャワーを　あびます。かおを　あらって、はを　みがきます。",
        "question": "まいあさ　なにを　しますか。",
        "options": [
          "ごはんを　たべます",
          "シャワーを　あびます",
          "テレビを　みます",
          "さんぽを　します"
        ],
        "answer": 1,
        "id": "N5-14-047"
      },
      {
        "type": "読解（短文）",
        "text": "しゅくだいは　まいにち　30ぷんです。きょうは　1じかん　しました。",
        "question": "きょうは　どのくらい　しましたか。",
        "options": [
          "15ふん",
          "30ぷん",
          "45ふん",
          "1じかん"
        ],
        "answer": 1,
        "id": "N5-14-048"
      },
      {
        "type": "読解（中文）",
        "text": "にちようびに　サッカーを　しました。ともだちと　こうえんで　しました。2じかん　あそびました。つかれましたが、たのしかったです。",
        "question": "だれと　サッカーを　しましたか。",
        "options": [
          "かぞく",
          "ともだち",
          "せんせい",
          "ひとりで"
        ],
        "answer": 1,
        "id": "N5-14-049"
      },
      {
        "type": "読解（中文）",
        "text": "やまださんは　えを　かくのが　すきです。まいしゅう　どようびに　えを　かきます。いろいろな　はなの　えを　かきます。",
        "question": "やまださんは　なにを　かきますか。",
        "options": [
          "やまの　え",
          "うみの　え",
          "はなの　え",
          "どうぶつの　え"
        ],
        "answer": 1,
        "id": "N5-14-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-14-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "19枚",
          "20枚",
          "21枚",
          "22枚"
        ],
        "answer": 0,
        "id": "N5-14-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "18枚",
          "19枚",
          "20枚",
          "21枚"
        ],
        "answer": 2,
        "id": "N5-14-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "17枚",
          "18枚",
          "19枚",
          "20枚"
        ],
        "answer": 0,
        "id": "N5-14-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "16枚",
          "17枚",
          "18枚",
          "19枚"
        ],
        "answer": 2,
        "id": "N5-14-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "15枚",
          "16枚",
          "17枚",
          "18枚"
        ],
        "answer": 0,
        "id": "N5-14-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "14枚",
          "15枚",
          "16枚",
          "17枚"
        ],
        "answer": 2,
        "id": "N5-14-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "12日",
          "13日",
          "14日",
          "15日"
        ],
        "answer": 0,
        "id": "N5-14-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "13日",
          "14日",
          "15日",
          "16日"
        ],
        "answer": 1,
        "id": "N5-14-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "14日",
          "15日",
          "16日",
          "17日"
        ],
        "answer": 0,
        "id": "N5-14-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "15日",
          "16日",
          "17日",
          "18日"
        ],
        "answer": 1,
        "id": "N5-14-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "16日",
          "17日",
          "18日",
          "19日"
        ],
        "answer": 0,
        "id": "N5-14-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "17日",
          "18日",
          "19日",
          "20日"
        ],
        "answer": 1,
        "id": "N5-14-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "何ですか",
          "誰ですか",
          "どこですか",
          "いつですか"
        ],
        "answer": 0,
        "id": "N5-14-064",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "いくらですか",
          "いくつですか",
          "何人ですか",
          "何時ですか"
        ],
        "answer": 0,
        "id": "N5-14-065",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "どれですか",
          "どちらですか",
          "どのくらいですか",
          "どうですか"
        ],
        "answer": 0,
        "id": "N5-14-066",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "なぜですか",
          "どうしてですか",
          "何のためですか",
          "どういう意味ですか"
        ],
        "answer": 0,
        "id": "N5-14-067",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "どうやってですか",
          "どのようにですか",
          "やり方を教えて",
          "方法は"
        ],
        "answer": 0,
        "id": "N5-14-068",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "午前10時です",
          "午後10時です",
          "10時です",
          "10時半です"
        ],
        "answer": 0,
        "id": "N5-14-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "午前11時です",
          "午後11時です",
          "11時です",
          "11時半です"
        ],
        "answer": 0,
        "id": "N5-14-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "午前12時です",
          "午後12時です",
          "12時です",
          "12時半です"
        ],
        "answer": 0,
        "id": "N5-14-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "1日です",
          "2日です",
          "3日です",
          "4日です"
        ],
        "answer": 0,
        "id": "N5-14-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "2日です",
          "3日です",
          "4日です",
          "5日です"
        ],
        "answer": 0,
        "id": "N5-14-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "3日です",
          "4日です",
          "5日です",
          "6日です"
        ],
        "answer": 0,
        "id": "N5-14-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "4日です",
          "5日です",
          "6日です",
          "7日です"
        ],
        "answer": 0,
        "id": "N5-14-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "5日です",
          "6日です",
          "7日です",
          "8日です"
        ],
        "answer": 0,
        "id": "N5-14-076",
        "script": "なんさいですか。"
      }
    ],
    "15": [
      {
        "type": "漢字読み",
        "text": "<u>山</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "ふたり",
          "やま",
          "でんしゃ",
          "かう"
        ],
        "answer": 1,
        "id": "N5-15-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>口</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "まいとし",
          "ごご",
          "え",
          "くち"
        ],
        "answer": 3,
        "id": "N5-15-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>高い</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "らいねん",
          "わかる",
          "こんしゅう",
          "たかい"
        ],
        "answer": 3,
        "id": "N5-15-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>生まれる</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "うまれる",
          "でぐち",
          "いろいろ",
          "あし"
        ],
        "answer": 0,
        "id": "N5-15-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>出力</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "まいにち",
          "しゅつりょく",
          "おみず",
          "ふん"
        ],
        "answer": 1,
        "id": "N5-15-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>雨</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "みぎ",
          "あめ",
          "ひとり",
          "みっつ"
        ],
        "answer": 1,
        "id": "N5-15-006"
      },
      {
        "type": "表記",
        "text": "<u>ちいさい</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "来年",
          "二人",
          "小さい",
          "入る"
        ],
        "answer": 2,
        "id": "N5-15-007"
      },
      {
        "type": "表記",
        "text": "<u>せんしゅう</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "先週",
          "足",
          "間",
          "飲む"
        ],
        "answer": 0,
        "id": "N5-15-008"
      },
      {
        "type": "表記",
        "text": "<u>いく</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "四つ",
          "行く",
          "飲む",
          "お母さん"
        ],
        "answer": 1,
        "id": "N5-15-009"
      },
      {
        "type": "表記",
        "text": "<u>しゃかい</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "間",
          "山",
          "左",
          "社会"
        ],
        "answer": 3,
        "id": "N5-15-010"
      },
      {
        "type": "表記",
        "text": "<u>いつつ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "明日",
          "教室",
          "五つ",
          "色々"
        ],
        "answer": 2,
        "id": "N5-15-011"
      },
      {
        "type": "表記",
        "text": "<u>おなじ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "同じ",
          "三つ",
          "色々",
          "人"
        ],
        "answer": 0,
        "id": "N5-15-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "バス",
          "つくえ",
          "かさ",
          "ごはん"
        ],
        "answer": 3,
        "id": "N5-15-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "くつ",
          "ノート",
          "ぎゅうにゅう",
          "ズボン"
        ],
        "answer": 2,
        "id": "N5-15-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "そば",
          "かさ",
          "つくえ",
          "くるま"
        ],
        "answer": 0,
        "id": "N5-15-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "つくえ",
          "にく",
          "でんしゃ",
          "テレビ"
        ],
        "answer": 1,
        "id": "N5-15-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "こうえん",
          "みかん",
          "パン",
          "テレビ"
        ],
        "answer": 0,
        "id": "N5-15-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "へや",
          "おちゃ",
          "テレビ",
          "みかん"
        ],
        "answer": 0,
        "id": "N5-15-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みず",
          "ほん",
          "おちゃ",
          "コンビニ"
        ],
        "answer": 3,
        "id": "N5-15-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "パン",
          "ほん",
          "うみ",
          "えんぴつ"
        ],
        "answer": 2,
        "id": "N5-15-020"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>女の子<rt>おんなのこ</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "coffee",
          "bright",
          "girl",
          "output"
        ],
        "answer": 2,
        "id": "N5-15-021"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>出る<rt>でる</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "to exit/leave",
          "thanks for meal",
          "can do",
          "society"
        ],
        "answer": 0,
        "id": "N5-15-022"
      },
      {
        "type": "言い換え",
        "text": "「コート」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "time",
          "exit",
          "coat",
          "tea"
        ],
        "answer": 2,
        "id": "N5-15-023"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>大きい<rt>おおきい</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "big",
          "electricity",
          "that (far)",
          "where"
        ],
        "answer": 0,
        "id": "N5-15-024"
      },
      {
        "type": "言い換え",
        "text": "「ベッド」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "last week",
          "inside",
          "bed",
          "water"
        ],
        "answer": 2,
        "id": "N5-15-025"
      },
      {
        "type": "文法形式",
        "text": "ペン（　）かしてください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "で",
          "に"
        ],
        "answer": 0,
        "id": "N5-15-026"
      },
      {
        "type": "文法形式",
        "text": "かさ（　）わすれました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "が",
          "で"
        ],
        "answer": 0,
        "id": "N5-15-027"
      },
      {
        "type": "文法形式",
        "text": "さいふ（　）おとしました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "で",
          "が"
        ],
        "answer": 0,
        "id": "N5-15-028"
      },
      {
        "type": "文法形式",
        "text": "かぎ（　）なくしました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "が",
          "で"
        ],
        "answer": 0,
        "id": "N5-15-029"
      },
      {
        "type": "文法形式",
        "text": "やくそく（　）わすれないでください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "に",
          "で"
        ],
        "answer": 0,
        "id": "N5-15-030"
      },
      {
        "type": "文法形式",
        "text": "おみやげ（　）かってきました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "で",
          "が",
          "に"
        ],
        "answer": 0,
        "id": "N5-15-031"
      },
      {
        "type": "文法形式",
        "text": "おべんとう（　）つくりました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "で",
          "に",
          "が"
        ],
        "answer": 0,
        "id": "N5-15-032"
      },
      {
        "type": "文法形式",
        "text": "しゃしん（　）とりました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "が",
          "で"
        ],
        "answer": 0,
        "id": "N5-15-033"
      },
      {
        "type": "文法形式",
        "text": "てがみ（　）だしました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "で",
          "に",
          "が"
        ],
        "answer": 0,
        "id": "N5-15-034"
      },
      {
        "type": "文法形式",
        "text": "メール（　）おくりました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "で",
          "が"
        ],
        "answer": 0,
        "id": "N5-15-035"
      },
      {
        "type": "文の組み立て",
        "text": "コーヒーを ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "のみ",
          "ながら",
          "よみました",
          "ほんを"
        ],
        "answer": 0,
        "id": "N5-15-036"
      },
      {
        "type": "文の組み立て",
        "text": "おんがくを ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "きき",
          "ながら",
          "べんきょうしました",
          "を"
        ],
        "answer": 0,
        "id": "N5-15-037"
      },
      {
        "type": "文の組み立て",
        "text": "テレビを ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "み",
          "ながら",
          "たべました",
          "ごはんを"
        ],
        "answer": 0,
        "id": "N5-15-038"
      },
      {
        "type": "文の組み立て",
        "text": "でんわを ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "し",
          "ながら",
          "あるきました",
          "を"
        ],
        "answer": 0,
        "id": "N5-15-039"
      },
      {
        "type": "文の組み立て",
        "text": "うたを ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "うたい",
          "ながら",
          "そうじしました",
          "を"
        ],
        "answer": 0,
        "id": "N5-15-040"
      },
      {
        "type": "文章の文法",
        "text": "この へやは あかるくて（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "きれい",
          "きれいな",
          "きれいに",
          "きれいで"
        ],
        "answer": 0,
        "id": "N5-15-041"
      },
      {
        "type": "文章の文法",
        "text": "きのう ともだちに（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "あいました",
          "あって",
          "あいます",
          "あう"
        ],
        "answer": 0,
        "id": "N5-15-042"
      },
      {
        "type": "文章の文法",
        "text": "わたしは まいにち しんぶんを（　）います。",
        "question": "（　）に　はいるのは？",
        "options": [
          "よんで",
          "よみます",
          "よんだ",
          "よむ"
        ],
        "answer": 0,
        "id": "N5-15-043"
      },
      {
        "type": "文章の文法",
        "text": "この かばんは おもくて（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "ふべん",
          "ふべんな",
          "ふべんに",
          "ふべんで"
        ],
        "answer": 0,
        "id": "N5-15-044"
      },
      {
        "type": "文章の文法",
        "text": "きょうは かぜが（　）ですね。",
        "question": "（　）に　はいるのは？",
        "options": [
          "つよい",
          "つよく",
          "つよいな",
          "つよいに"
        ],
        "answer": 0,
        "id": "N5-15-045"
      },
      {
        "type": "読解（短文）",
        "text": "わたしの　へやは　きれいです。まいにち　そうじを　します。",
        "question": "なぜ　へやは　きれいですか。",
        "options": [
          "そうじを　しないから",
          "まいにち　そうじを　するから",
          "ひろいから",
          "あたらしいから"
        ],
        "answer": 1,
        "id": "N5-15-046"
      },
      {
        "type": "読解（短文）",
        "text": "わたしは　げつようびから　きんようびまで　しごとを　します。",
        "question": "いつ　しごとを　しますか。",
        "options": [
          "まいにち",
          "げつようびだけ",
          "げつようびから　きんようびまで",
          "どようびと　にちようび"
        ],
        "answer": 1,
        "id": "N5-15-047"
      },
      {
        "type": "読解（短文）",
        "text": "にちようびは　やすみです。かぞくと　こうえんに　いきます。",
        "question": "にちようびに　なにを　しますか。",
        "options": [
          "しごとを　します",
          "がっこうに　いきます",
          "こうえんに　いきます",
          "べんきょうを　します"
        ],
        "answer": 1,
        "id": "N5-15-048"
      },
      {
        "type": "読解（中文）",
        "text": "わたしは　まいあさ　30ぷん　はしります。けんこうに　いいです。あめの　ひは　いえで　ストレッチを　します。",
        "question": "まいあさ　なにを　しますか。",
        "options": [
          "およぐ",
          "はしる",
          "ストレッチ",
          "ヨガ"
        ],
        "answer": 1,
        "id": "N5-15-049"
      },
      {
        "type": "読解（中文）",
        "text": "ともだちと　カラオケに　いきました。3じかん　うたいました。のどが　いたくなりました。でも　たのしかったです。",
        "question": "どのくらい　うたいましたか。",
        "options": [
          "1じかん",
          "2じかん",
          "3じかん",
          "4じかん"
        ],
        "answer": 1,
        "id": "N5-15-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-15-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "13枚",
          "14枚",
          "15枚",
          "16枚"
        ],
        "answer": 0,
        "id": "N5-15-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "12枚",
          "13枚",
          "14枚",
          "15枚"
        ],
        "answer": 2,
        "id": "N5-15-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "11枚",
          "12枚",
          "13枚",
          "14枚"
        ],
        "answer": 0,
        "id": "N5-15-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "10枚",
          "11枚",
          "12枚",
          "13枚"
        ],
        "answer": 2,
        "id": "N5-15-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "9枚",
          "10枚",
          "11枚",
          "12枚"
        ],
        "answer": 0,
        "id": "N5-15-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "8枚",
          "9枚",
          "10枚",
          "11枚"
        ],
        "answer": 2,
        "id": "N5-15-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "18日",
          "19日",
          "20日",
          "21日"
        ],
        "answer": 0,
        "id": "N5-15-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "19日",
          "20日",
          "21日",
          "22日"
        ],
        "answer": 1,
        "id": "N5-15-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "20日",
          "21日",
          "22日",
          "23日"
        ],
        "answer": 0,
        "id": "N5-15-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "21日",
          "22日",
          "23日",
          "24日"
        ],
        "answer": 1,
        "id": "N5-15-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "22日",
          "23日",
          "24日",
          "25日"
        ],
        "answer": 0,
        "id": "N5-15-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "23日",
          "24日",
          "25日",
          "26日"
        ],
        "answer": 1,
        "id": "N5-15-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "何時からですか",
          "何時までですか",
          "何時間ですか",
          "いつまでですか"
        ],
        "answer": 0,
        "id": "N5-15-064",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "何曜日ですか",
          "何日ですか",
          "何月ですか",
          "何年ですか"
        ],
        "answer": 0,
        "id": "N5-15-065",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "どこにありますか",
          "どこで買えますか",
          "どこで食べられますか",
          "どこへ行きますか"
        ],
        "answer": 0,
        "id": "N5-15-066",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "誰が来ますか",
          "誰と行きますか",
          "誰に会いますか",
          "誰が担当ですか"
        ],
        "answer": 0,
        "id": "N5-15-067",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "何を食べますか",
          "何を飲みますか",
          "何を注文しますか",
          "何がおすすめですか"
        ],
        "answer": 0,
        "id": "N5-15-068",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "6日です",
          "7日です",
          "8日です",
          "9日です"
        ],
        "answer": 0,
        "id": "N5-15-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "7日です",
          "8日です",
          "9日です",
          "10日です"
        ],
        "answer": 0,
        "id": "N5-15-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "1つです",
          "2つです",
          "3つです",
          "4つです"
        ],
        "answer": 0,
        "id": "N5-15-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "2つです",
          "3つです",
          "4つです",
          "5つです"
        ],
        "answer": 0,
        "id": "N5-15-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "3つです",
          "4つです",
          "5つです",
          "6つです"
        ],
        "answer": 0,
        "id": "N5-15-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "4つです",
          "5つです",
          "6つです",
          "7つです"
        ],
        "answer": 0,
        "id": "N5-15-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "5つです",
          "6つです",
          "7つです",
          "8つです"
        ],
        "answer": 0,
        "id": "N5-15-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "6つです",
          "7つです",
          "8つです",
          "9つです"
        ],
        "answer": 0,
        "id": "N5-15-076",
        "script": "なんさいですか。"
      }
    ],
    "16": [
      {
        "type": "漢字読み",
        "text": "<u>午後</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "じょうず",
          "ごご",
          "おんがく",
          "とけい"
        ],
        "answer": 1,
        "id": "N5-16-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>母</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "たりる",
          "はは",
          "きょう",
          "ひとつ"
        ],
        "answer": 1,
        "id": "N5-16-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>お茶</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "おちゃ",
          "さかな",
          "おと",
          "いま"
        ],
        "answer": 0,
        "id": "N5-16-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>万</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "まん",
          "じゅうぶん",
          "じかん",
          "げんき"
        ],
        "answer": 0,
        "id": "N5-16-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>来週</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "せんしゅう",
          "なか",
          "らいしゅう",
          "いま"
        ],
        "answer": 2,
        "id": "N5-16-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>後ろ</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "よむ",
          "もくようび",
          "しゃちょう",
          "うしろ"
        ],
        "answer": 3,
        "id": "N5-16-006"
      },
      {
        "type": "表記",
        "text": "<u>こんげつ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "会計",
          "十分",
          "今月",
          "木曜日"
        ],
        "answer": 2,
        "id": "N5-16-007"
      },
      {
        "type": "表記",
        "text": "<u>おみず</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "先週",
          "分",
          "お水",
          "買う"
        ],
        "answer": 2,
        "id": "N5-16-008"
      },
      {
        "type": "表記",
        "text": "<u>やすみ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "休み",
          "万",
          "午後",
          "聞く"
        ],
        "answer": 0,
        "id": "N5-16-009"
      },
      {
        "type": "表記",
        "text": "<u>そと</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "入力",
          "外",
          "絵",
          "電気"
        ],
        "answer": 1,
        "id": "N5-16-010"
      },
      {
        "type": "表記",
        "text": "<u>おおきい</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "半分",
          "電車",
          "大きい",
          "電話"
        ],
        "answer": 2,
        "id": "N5-16-011"
      },
      {
        "type": "表記",
        "text": "<u>よっつ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "形",
          "四つ",
          "食べる",
          "会社"
        ],
        "answer": 1,
        "id": "N5-16-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "でんわ",
          "パン",
          "バス",
          "くるま"
        ],
        "answer": 1,
        "id": "N5-16-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "かぎ",
          "コーヒー",
          "ぼうし",
          "えんぴつ"
        ],
        "answer": 1,
        "id": "N5-16-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "かばん",
          "びょういん",
          "おにぎり",
          "かさ"
        ],
        "answer": 2,
        "id": "N5-16-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "かばん",
          "かさ",
          "バス",
          "やさい"
        ],
        "answer": 3,
        "id": "N5-16-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みず",
          "りんご",
          "テレビ",
          "デパート"
        ],
        "answer": 3,
        "id": "N5-16-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ほん",
          "えんぴつ",
          "りんご",
          "きょうしつ"
        ],
        "answer": 3,
        "id": "N5-16-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みせ",
          "ほん",
          "パン",
          "テレビ"
        ],
        "answer": 0,
        "id": "N5-16-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ほん",
          "りんご",
          "おちゃ",
          "びじゅつかん"
        ],
        "answer": 3,
        "id": "N5-16-020"
      },
      {
        "type": "言い換え",
        "text": "「サイズ」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "to exist (animate)",
          "black",
          "size",
          "to be enough"
        ],
        "answer": 2,
        "id": "N5-16-021"
      },
      {
        "type": "言い換え",
        "text": "「そして」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "society",
          "long",
          "to teach",
          "and then"
        ],
        "answer": 3,
        "id": "N5-16-022"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>火曜日<rt>かようび</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "Tuesday",
          "today",
          "to be enough",
          "last month"
        ],
        "answer": 0,
        "id": "N5-16-023"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>後ろ<rt>うしろ</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "a little",
          "behind",
          "fun",
          "same"
        ],
        "answer": 1,
        "id": "N5-16-024"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>正しい<rt>ただしい</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "yes",
          "that",
          "correct",
          "above"
        ],
        "answer": 2,
        "id": "N5-16-025"
      },
      {
        "type": "文法形式",
        "text": "でんわ（　）かけました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "で",
          "に"
        ],
        "answer": 0,
        "id": "N5-16-026"
      },
      {
        "type": "文法形式",
        "text": "やくそく（　）しました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "で",
          "が",
          "に"
        ],
        "answer": 0,
        "id": "N5-16-027"
      },
      {
        "type": "文法形式",
        "text": "けいかく（　）たてました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "で",
          "に"
        ],
        "answer": 0,
        "id": "N5-16-028"
      },
      {
        "type": "文法形式",
        "text": "じゅんび（　）しました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "で",
          "が"
        ],
        "answer": 0,
        "id": "N5-16-029"
      },
      {
        "type": "文法形式",
        "text": "そうじ（　）しました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "で",
          "が"
        ],
        "answer": 0,
        "id": "N5-16-030"
      },
      {
        "type": "文法形式",
        "text": "せんたく（　）しました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "で",
          "が",
          "に"
        ],
        "answer": 0,
        "id": "N5-16-031"
      },
      {
        "type": "文法形式",
        "text": "りょうり（　）しました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "が",
          "で"
        ],
        "answer": 0,
        "id": "N5-16-032"
      },
      {
        "type": "文法形式",
        "text": "かいもの（　）しました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "に",
          "で"
        ],
        "answer": 0,
        "id": "N5-16-033"
      },
      {
        "type": "文法形式",
        "text": "さんぽ（　）しました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "に",
          "で"
        ],
        "answer": 0,
        "id": "N5-16-034"
      },
      {
        "type": "文法形式",
        "text": "うんどう（　）しました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "で",
          "が"
        ],
        "answer": 0,
        "id": "N5-16-035"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "あめが",
          "ふったら",
          "いきません",
          "で"
        ],
        "answer": 1,
        "id": "N5-16-036"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "じかんが",
          "あったら",
          "いきたい",
          "と"
        ],
        "answer": 1,
        "id": "N5-16-037"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "おかねが",
          "あったら",
          "かいたい",
          "と"
        ],
        "answer": 1,
        "id": "N5-16-038"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "やすみが",
          "あったら",
          "ねたい",
          "と"
        ],
        "answer": 1,
        "id": "N5-16-039"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "ひまが",
          "あったら",
          "よみたい",
          "を"
        ],
        "answer": 1,
        "id": "N5-16-040"
      },
      {
        "type": "文章の文法",
        "text": "わたしは りょうりが（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "にがて",
          "にがてな",
          "にがてに",
          "にがてで"
        ],
        "answer": 0,
        "id": "N5-16-041"
      },
      {
        "type": "文章の文法",
        "text": "この もんだいは かんたん（　）むずかしいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "そうで",
          "そうに",
          "そうな",
          "そう"
        ],
        "answer": 0,
        "id": "N5-16-042"
      },
      {
        "type": "文章の文法",
        "text": "あした 9じに がっこうに（　）ください。",
        "question": "（　）に　はいるのは？",
        "options": [
          "きて",
          "きます",
          "きた",
          "くる"
        ],
        "answer": 0,
        "id": "N5-16-043"
      },
      {
        "type": "文章の文法",
        "text": "わたしは とうきょうに（　）たいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "いき",
          "いって",
          "いきます",
          "いく"
        ],
        "answer": 0,
        "id": "N5-16-044"
      },
      {
        "type": "文章の文法",
        "text": "この みせは やす（　）、おいしくないです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "いけど",
          "いから",
          "いので",
          "くて"
        ],
        "answer": 0,
        "id": "N5-16-045"
      },
      {
        "type": "読解（短文）",
        "text": "にほんごの　じゅぎょうは　10じからです。12じに　おわります。",
        "question": "じゅぎょうは　なんじに　おわりますか。",
        "options": [
          "10じ",
          "11じ",
          "12じ",
          "1じ"
        ],
        "answer": 1,
        "id": "N5-16-046"
      },
      {
        "type": "読解（短文）",
        "text": "わたしは　まいにち　2じかん　にほんごを　べんきょうします。",
        "question": "なんじかん　べんきょうしますか。",
        "options": [
          "1じかん",
          "2じかん",
          "3じかん",
          "4じかん"
        ],
        "answer": 1,
        "id": "N5-16-047"
      },
      {
        "type": "読解（短文）",
        "text": "しけんは　らいしゅうの　すいようびです。",
        "question": "しけんは　いつですか。",
        "options": [
          "こんしゅうの　すいようび",
          "らいしゅうの　げつようび",
          "らいしゅうの　すいようび",
          "らいげつ"
        ],
        "answer": 1,
        "id": "N5-16-048"
      },
      {
        "type": "読解（中文）",
        "text": "わたしの　へやは　6じょうです。ベッドと　つくえが　あります。まどから　やまが　みえます。",
        "question": "まどから　なにが　みえますか。",
        "options": [
          "うみ",
          "やま",
          "まち",
          "こうえん"
        ],
        "answer": 1,
        "id": "N5-16-049"
      },
      {
        "type": "読解（中文）",
        "text": "あたらしい　アパートに　ひっこしました。えきから　ちかくて　べんりです。へやは　せまいですが、きれいです。",
        "question": "あたらしい　アパートは　どうですか。",
        "options": [
          "えきから　とおい",
          "えきから　ちかい",
          "ひろい",
          "ふるい"
        ],
        "answer": 1,
        "id": "N5-16-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-16-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "7枚",
          "8枚",
          "9枚",
          "10枚"
        ],
        "answer": 0,
        "id": "N5-16-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "6枚",
          "7枚",
          "8枚",
          "9枚"
        ],
        "answer": 2,
        "id": "N5-16-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "5枚",
          "6枚",
          "7枚",
          "8枚"
        ],
        "answer": 0,
        "id": "N5-16-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "4枚",
          "5枚",
          "6枚",
          "7枚"
        ],
        "answer": 2,
        "id": "N5-16-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "3枚",
          "4枚",
          "5枚",
          "6枚"
        ],
        "answer": 0,
        "id": "N5-16-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "2枚",
          "3枚",
          "4枚",
          "5枚"
        ],
        "answer": 2,
        "id": "N5-16-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "24日",
          "25日",
          "26日",
          "27日"
        ],
        "answer": 0,
        "id": "N5-16-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "25日",
          "26日",
          "27日",
          "28日"
        ],
        "answer": 1,
        "id": "N5-16-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "26日",
          "27日",
          "28日",
          "29日"
        ],
        "answer": 0,
        "id": "N5-16-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "27日",
          "28日",
          "29日",
          "30日"
        ],
        "answer": 1,
        "id": "N5-16-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "28日",
          "29日",
          "30日",
          "31日"
        ],
        "answer": 0,
        "id": "N5-16-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "1月",
          "2月",
          "3月",
          "4月"
        ],
        "answer": 1,
        "id": "N5-16-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "空いていますか",
          "予約できますか",
          "席はありますか",
          "部屋はありますか"
        ],
        "answer": 0,
        "id": "N5-16-064",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "大丈夫ですか",
          "平気ですか",
          "問題ないですか",
          "よろしいですか"
        ],
        "answer": 0,
        "id": "N5-16-065",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "本当ですか",
          "確かですか",
          "間違いないですか",
          "正しいですか"
        ],
        "answer": 0,
        "id": "N5-16-066",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "分かりましたか",
          "聞こえましたか",
          "見えましたか",
          "届きましたか"
        ],
        "answer": 0,
        "id": "N5-16-067",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "もう終わりましたか",
          "もう始まりましたか",
          "まだですか",
          "準備できましたか"
        ],
        "answer": 0,
        "id": "N5-16-068",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "7つです",
          "8つです",
          "9つです",
          "10つです"
        ],
        "answer": 0,
        "id": "N5-16-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "8つです",
          "9つです",
          "10つです",
          "11つです"
        ],
        "answer": 0,
        "id": "N5-16-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "9つです",
          "10つです",
          "11つです",
          "12つです"
        ],
        "answer": 0,
        "id": "N5-16-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "10つです",
          "11つです",
          "12つです",
          "13つです"
        ],
        "answer": 0,
        "id": "N5-16-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "1個です",
          "2個です",
          "3個です",
          "4個です"
        ],
        "answer": 0,
        "id": "N5-16-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "2個です",
          "3個です",
          "4個です",
          "5個です"
        ],
        "answer": 0,
        "id": "N5-16-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "3個です",
          "4個です",
          "5個です",
          "6個です"
        ],
        "answer": 0,
        "id": "N5-16-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "4個です",
          "5個です",
          "6個です",
          "7個です"
        ],
        "answer": 0,
        "id": "N5-16-076",
        "script": "なんさいですか。"
      }
    ],
    "17": [
      {
        "type": "漢字読み",
        "text": "<u>分</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "ふん",
          "おとこのこ",
          "わける",
          "しる"
        ],
        "answer": 0,
        "id": "N5-17-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>来る</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "はなす",
          "ときどき",
          "くる",
          "うえ"
        ],
        "answer": 2,
        "id": "N5-17-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>空</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "いろ",
          "おちゃ",
          "みえる",
          "そら"
        ],
        "answer": 3,
        "id": "N5-17-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>父</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "ちち",
          "まいにち",
          "ちいさい",
          "こんしゅう"
        ],
        "answer": 0,
        "id": "N5-17-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>半分</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "らいげつ",
          "おちゃ",
          "やすむ",
          "はんぶん"
        ],
        "answer": 3,
        "id": "N5-17-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>学校</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "じゅうぶん",
          "みず",
          "がっこう",
          "たかい"
        ],
        "answer": 2,
        "id": "N5-17-006"
      },
      {
        "type": "表記",
        "text": "<u>あした</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "半分",
          "会う",
          "行く",
          "明日"
        ],
        "answer": 3,
        "id": "N5-17-007"
      },
      {
        "type": "表記",
        "text": "<u>くに</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "買う",
          "会計",
          "国",
          "同じ"
        ],
        "answer": 2,
        "id": "N5-17-008"
      },
      {
        "type": "表記",
        "text": "<u>ふたつ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "行く",
          "火曜日",
          "出口",
          "二つ"
        ],
        "answer": 3,
        "id": "N5-17-009"
      },
      {
        "type": "表記",
        "text": "<u>きょう</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "百",
          "今日",
          "女の子",
          "読む"
        ],
        "answer": 1,
        "id": "N5-17-010"
      },
      {
        "type": "表記",
        "text": "<u>かう</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "知る",
          "買う",
          "お水",
          "今日"
        ],
        "answer": 1,
        "id": "N5-17-011"
      },
      {
        "type": "表記",
        "text": "<u>みず</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "女の子",
          "電話",
          "入力",
          "水"
        ],
        "answer": 3,
        "id": "N5-17-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "でんわ",
          "かさ",
          "りんご",
          "びょういん"
        ],
        "answer": 2,
        "id": "N5-17-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "シャツ",
          "みず",
          "くつ",
          "かぎ"
        ],
        "answer": 1,
        "id": "N5-17-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "くるま",
          "おにぎり",
          "テレビ",
          "びょういん"
        ],
        "answer": 1,
        "id": "N5-17-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "いす",
          "えき",
          "でんわ",
          "すし"
        ],
        "answer": 3,
        "id": "N5-17-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "びょういん",
          "パン",
          "ほん",
          "おちゃ"
        ],
        "answer": 0,
        "id": "N5-17-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "えんぴつ",
          "りんご",
          "みかん",
          "としょかん"
        ],
        "answer": 3,
        "id": "N5-17-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ほん",
          "デパート",
          "えんぴつ",
          "みかん"
        ],
        "answer": 1,
        "id": "N5-17-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "えいがかん",
          "テレビ",
          "ほん",
          "パン"
        ],
        "answer": 0,
        "id": "N5-17-020"
      },
      {
        "type": "言い換え",
        "text": "「エアコン」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "air conditioner",
          "picture",
          "below",
          "as expected"
        ],
        "answer": 0,
        "id": "N5-17-021"
      },
      {
        "type": "言い換え",
        "text": "「ネクタイ」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "this",
          "high/expensive",
          "one thing",
          "necktie"
        ],
        "answer": 3,
        "id": "N5-17-022"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>高い<rt>たかい</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "woman",
          "two things",
          "to rest",
          "high/expensive"
        ],
        "answer": 3,
        "id": "N5-17-023"
      },
      {
        "type": "言い換え",
        "text": "「カフェ」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "cafe",
          "convenience store",
          "you",
          "already"
        ],
        "answer": 0,
        "id": "N5-17-024"
      },
      {
        "type": "言い換え",
        "text": "「こんにちは」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "mother",
          "hello",
          "rain",
          "every week"
        ],
        "answer": 1,
        "id": "N5-17-025"
      },
      {
        "type": "文法形式",
        "text": "べんきょう（　）しました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "に",
          "で"
        ],
        "answer": 0,
        "id": "N5-17-026"
      },
      {
        "type": "文法形式",
        "text": "しごと（　）おわりました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "で",
          "に"
        ],
        "answer": 0,
        "id": "N5-17-027"
      },
      {
        "type": "文法形式",
        "text": "じゅぎょう（　）はじまりました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "で",
          "に"
        ],
        "answer": 0,
        "id": "N5-17-028"
      },
      {
        "type": "文法形式",
        "text": "かいぎ（　）おわりました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "で",
          "に"
        ],
        "answer": 0,
        "id": "N5-17-029"
      },
      {
        "type": "文法形式",
        "text": "でんしゃ（　）きました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "で",
          "に"
        ],
        "answer": 0,
        "id": "N5-17-030"
      },
      {
        "type": "文法形式",
        "text": "バス（　）きました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "に",
          "で",
          "を"
        ],
        "answer": 0,
        "id": "N5-17-031"
      },
      {
        "type": "文法形式",
        "text": "あめ（　）やみました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "に",
          "を",
          "で"
        ],
        "answer": 0,
        "id": "N5-17-032"
      },
      {
        "type": "文法形式",
        "text": "かぜ（　）ふいています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "に",
          "で",
          "を"
        ],
        "answer": 0,
        "id": "N5-17-033"
      },
      {
        "type": "文法形式",
        "text": "ゆき（　）ふっています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "に",
          "で",
          "を"
        ],
        "answer": 0,
        "id": "N5-17-034"
      },
      {
        "type": "文法形式",
        "text": "はな（　）さいています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "で",
          "に",
          "を"
        ],
        "answer": 0,
        "id": "N5-17-035"
      },
      {
        "type": "文の組み立て",
        "text": "えきに ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "ついたら",
          "でんわして",
          "ください",
          "に"
        ],
        "answer": 0,
        "id": "N5-17-036"
      },
      {
        "type": "文の組み立て",
        "text": "かいしゃに ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "ついたら",
          "れんらくして",
          "ください",
          "に"
        ],
        "answer": 0,
        "id": "N5-17-037"
      },
      {
        "type": "文の組み立て",
        "text": "うちに ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "かえったら",
          "やすんで",
          "ください",
          "と"
        ],
        "answer": 0,
        "id": "N5-17-038"
      },
      {
        "type": "文の組み立て",
        "text": "しごとが ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "おわったら",
          "のみに",
          "いきましょう",
          "へ"
        ],
        "answer": 0,
        "id": "N5-17-039"
      },
      {
        "type": "文の組み立て",
        "text": "べんきょうが ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "おわったら",
          "あそびに",
          "いきましょう",
          "へ"
        ],
        "answer": 0,
        "id": "N5-17-040"
      },
      {
        "type": "文章の文法",
        "text": "きのう てんきが（　）ので、さんぽしました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "よかった",
          "よくて",
          "いい",
          "いいで"
        ],
        "answer": 0,
        "id": "N5-17-041"
      },
      {
        "type": "文章の文法",
        "text": "わたしは あした（　）つもりです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "やすむ",
          "やすんで",
          "やすみます",
          "やすんだ"
        ],
        "answer": 0,
        "id": "N5-17-042"
      },
      {
        "type": "文章の文法",
        "text": "この くつは おおき（　）、はきにくいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "すぎて",
          "すぎる",
          "すぎます",
          "すぎた"
        ],
        "answer": 0,
        "id": "N5-17-043"
      },
      {
        "type": "文章の文法",
        "text": "わたしは にほんごを（　）ことができます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "はなす",
          "はなして",
          "はなした",
          "はなします"
        ],
        "answer": 0,
        "id": "N5-17-044"
      },
      {
        "type": "文章の文法",
        "text": "きょうは しごとが（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "いそがしい",
          "いそがしく",
          "いそがしいな",
          "いそがしくて"
        ],
        "answer": 0,
        "id": "N5-17-045"
      },
      {
        "type": "読解（短文）",
        "text": "わたしの　クラスには　20にんの　がくせいが　います。",
        "question": "クラスに　なんにん　いますか。",
        "options": [
          "10にん",
          "15にん",
          "20にん",
          "25にん"
        ],
        "answer": 1,
        "id": "N5-17-046"
      },
      {
        "type": "読解（短文）",
        "text": "せんせいは　えいごと　にほんごを　おしえています。",
        "question": "せんせいは　なにを　おしえていますか。",
        "options": [
          "すうがく",
          "かがく",
          "えいごと　にほんご",
          "れきし"
        ],
        "answer": 1,
        "id": "N5-17-047"
      },
      {
        "type": "読解（短文）",
        "text": "このほんは　わたしのです。あのほんは　ともだちのです。",
        "question": "あのほんは　だれのですか。",
        "options": [
          "わたしの",
          "せんせいの",
          "ともだちの",
          "かぞくの"
        ],
        "answer": 1,
        "id": "N5-17-048"
      },
      {
        "type": "読解（中文）",
        "text": "わたしの　いえには　にわが　あります。はなや　やさいを　そだてています。トマトが　たくさん　なりました。",
        "question": "なにを　そだてていますか。",
        "options": [
          "くだもの",
          "はなや　やさい",
          "どうぶつ",
          "さかな"
        ],
        "answer": 1,
        "id": "N5-17-049"
      },
      {
        "type": "読解（中文）",
        "text": "リビングに　テレビが　あります。ソファも　あります。かぞくで　よく　ここで　テレビを　みます。",
        "question": "どこで　テレビを　みますか。",
        "options": [
          "だいどころ",
          "リビング",
          "へや",
          "げんかん"
        ],
        "answer": 1,
        "id": "N5-17-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-17-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "1枚",
          "2枚",
          "3枚",
          "4枚"
        ],
        "answer": 0,
        "id": "N5-17-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "20人",
          "21人",
          "22人",
          "23人"
        ],
        "answer": 2,
        "id": "N5-17-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "19人",
          "20人",
          "21人",
          "22人"
        ],
        "answer": 0,
        "id": "N5-17-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "18人",
          "19人",
          "20人",
          "21人"
        ],
        "answer": 2,
        "id": "N5-17-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "17人",
          "18人",
          "19人",
          "20人"
        ],
        "answer": 0,
        "id": "N5-17-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "16人",
          "17人",
          "18人",
          "19人"
        ],
        "answer": 2,
        "id": "N5-17-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "2月",
          "3月",
          "4月",
          "5月"
        ],
        "answer": 0,
        "id": "N5-17-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "3月",
          "4月",
          "5月",
          "6月"
        ],
        "answer": 1,
        "id": "N5-17-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "4月",
          "5月",
          "6月",
          "7月"
        ],
        "answer": 0,
        "id": "N5-17-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "5月",
          "6月",
          "7月",
          "8月"
        ],
        "answer": 1,
        "id": "N5-17-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "6月",
          "7月",
          "8月",
          "9月"
        ],
        "answer": 0,
        "id": "N5-17-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "7月",
          "8月",
          "9月",
          "10月"
        ],
        "answer": 1,
        "id": "N5-17-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "好きですか",
          "嫌いですか",
          "得意ですか",
          "苦手ですか"
        ],
        "answer": 0,
        "id": "N5-17-064",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "行ったことありますか",
          "食べたことありますか",
          "見たことありますか",
          "聞いたことありますか"
        ],
        "answer": 0,
        "id": "N5-17-065",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "何がいいですか",
          "どれがいいですか",
          "どちらがいいですか",
          "いつがいいですか"
        ],
        "answer": 0,
        "id": "N5-17-066",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "どう思いますか",
          "何か意見ありますか",
          "どう感じましたか",
          "印象はどうですか"
        ],
        "answer": 0,
        "id": "N5-17-067",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "他に何かありますか",
          "何か質問ありますか",
          "何か必要ですか",
          "他にご注文は"
        ],
        "answer": 0,
        "id": "N5-17-068",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "5個です",
          "6個です",
          "7個です",
          "8個です"
        ],
        "answer": 0,
        "id": "N5-17-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "6個です",
          "7個です",
          "8個です",
          "9個です"
        ],
        "answer": 0,
        "id": "N5-17-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "7個です",
          "8個です",
          "9個です",
          "10個です"
        ],
        "answer": 0,
        "id": "N5-17-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "8個です",
          "9個です",
          "10個です",
          "11個です"
        ],
        "answer": 0,
        "id": "N5-17-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "9個です",
          "10個です",
          "11個です",
          "12個です"
        ],
        "answer": 0,
        "id": "N5-17-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "10個です",
          "11個です",
          "12個です",
          "13個です"
        ],
        "answer": 0,
        "id": "N5-17-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "1人です",
          "2人です",
          "3人です",
          "4人です"
        ],
        "answer": 0,
        "id": "N5-17-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "2人です",
          "3人です",
          "4人です",
          "5人です"
        ],
        "answer": 0,
        "id": "N5-17-076",
        "script": "なんさいですか。"
      }
    ],
    "18": [
      {
        "type": "漢字読み",
        "text": "<u>女</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "じかん",
          "ふたり",
          "はな",
          "おんな"
        ],
        "answer": 3,
        "id": "N5-18-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>小さい</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "ちいさい",
          "きょうしつ",
          "みず",
          "て"
        ],
        "answer": 0,
        "id": "N5-18-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>出口</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "でぐち",
          "くろい",
          "なに",
          "しゅつりょく"
        ],
        "answer": 0,
        "id": "N5-18-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>入力</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "さかな",
          "はいる",
          "そと",
          "にゅうりょく"
        ],
        "answer": 3,
        "id": "N5-18-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>同じ</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "おなじ",
          "にゅうりょく",
          "なまえ",
          "せんしゅう"
        ],
        "answer": 0,
        "id": "N5-18-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>三つ</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "うえ",
          "もり",
          "みっつ",
          "のむ"
        ],
        "answer": 2,
        "id": "N5-18-006"
      },
      {
        "type": "表記",
        "text": "<u>でんわ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "手",
          "車",
          "木",
          "電話"
        ],
        "answer": 3,
        "id": "N5-18-007"
      },
      {
        "type": "表記",
        "text": "<u>みち</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "道",
          "先週",
          "本",
          "見える"
        ],
        "answer": 0,
        "id": "N5-18-008"
      },
      {
        "type": "表記",
        "text": "<u>とり</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "万",
          "一人",
          "鳥",
          "黒い"
        ],
        "answer": 2,
        "id": "N5-18-009"
      },
      {
        "type": "表記",
        "text": "<u>ふたり</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "二人",
          "電車",
          "出力",
          "下がる"
        ],
        "answer": 0,
        "id": "N5-18-010"
      },
      {
        "type": "表記",
        "text": "<u>はは</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "母",
          "お水",
          "分ける",
          "月曜日"
        ],
        "answer": 0,
        "id": "N5-18-011"
      },
      {
        "type": "表記",
        "text": "<u>かようび</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "入力",
          "母",
          "百",
          "火曜日"
        ],
        "answer": 3,
        "id": "N5-18-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "バス",
          "やさい",
          "くるま",
          "かさ"
        ],
        "answer": 1,
        "id": "N5-18-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "めがね",
          "ジュース",
          "えんぴつ",
          "ズボン"
        ],
        "answer": 1,
        "id": "N5-18-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "でんわ",
          "くるま",
          "いす",
          "おにぎり"
        ],
        "answer": 3,
        "id": "N5-18-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "かさ",
          "てんぷら",
          "でんわ",
          "バス"
        ],
        "answer": 1,
        "id": "N5-18-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "えんぴつ",
          "パン",
          "としょかん",
          "ほん"
        ],
        "answer": 2,
        "id": "N5-18-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "テレビ",
          "パン",
          "えんぴつ",
          "がっこう"
        ],
        "answer": 3,
        "id": "N5-18-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "デパート",
          "おちゃ",
          "パン",
          "テレビ"
        ],
        "answer": 0,
        "id": "N5-18-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "テレビ",
          "ほん",
          "えいがかん",
          "えんぴつ"
        ],
        "answer": 2,
        "id": "N5-18-020"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>足りる<rt>たりる</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "hundred",
          "to be enough",
          "thanks for meal",
          "often"
        ],
        "answer": 1,
        "id": "N5-18-021"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>今週<rt>こんしゅう</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "road",
          "good morning",
          "sound",
          "this week"
        ],
        "answer": 3,
        "id": "N5-18-022"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>長い<rt>ながい</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "long",
          "as expected",
          "girl",
          "news"
        ],
        "answer": 0,
        "id": "N5-18-023"
      },
      {
        "type": "言い換え",
        "text": "「エレベーター」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "today",
          "elevator",
          "to find",
          "computer"
        ],
        "answer": 1,
        "id": "N5-18-024"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>今年<rt>ことし</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "to eat",
          "to drink",
          "this year",
          "how"
        ],
        "answer": 2,
        "id": "N5-18-025"
      },
      {
        "type": "文法形式",
        "text": "とり（　）ないています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "に",
          "で"
        ],
        "answer": 0,
        "id": "N5-18-026"
      },
      {
        "type": "文法形式",
        "text": "あかちゃん（　）ないています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "に",
          "で"
        ],
        "answer": 0,
        "id": "N5-18-027"
      },
      {
        "type": "文法形式",
        "text": "こども（　）あそんでいます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "で",
          "を",
          "に"
        ],
        "answer": 0,
        "id": "N5-18-028"
      },
      {
        "type": "文法形式",
        "text": "いぬ（　）はしっています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "で",
          "に"
        ],
        "answer": 0,
        "id": "N5-18-029"
      },
      {
        "type": "文法形式",
        "text": "ねこ（　）ねています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "で",
          "を",
          "に"
        ],
        "answer": 0,
        "id": "N5-18-030"
      },
      {
        "type": "文法形式",
        "text": "でんわ（　）なっています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "で",
          "に"
        ],
        "answer": 0,
        "id": "N5-18-031"
      },
      {
        "type": "文法形式",
        "text": "ドア（　）あいています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "に",
          "を",
          "で"
        ],
        "answer": 0,
        "id": "N5-18-032"
      },
      {
        "type": "文法形式",
        "text": "まど（　）しまっています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "で",
          "を",
          "に"
        ],
        "answer": 0,
        "id": "N5-18-033"
      },
      {
        "type": "文法形式",
        "text": "でんき（　）ついています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "に",
          "を",
          "で"
        ],
        "answer": 0,
        "id": "N5-18-034"
      },
      {
        "type": "文法形式",
        "text": "エアコン（　）ついています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "に",
          "を",
          "で"
        ],
        "answer": 0,
        "id": "N5-18-035"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "にほんに",
          "きて",
          "から",
          "いちねん"
        ],
        "answer": 1,
        "id": "N5-18-036"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "けっこん",
          "して",
          "から",
          "さんねん"
        ],
        "answer": 1,
        "id": "N5-18-037"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "そつぎょう",
          "して",
          "から",
          "ごねん"
        ],
        "answer": 1,
        "id": "N5-18-038"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "にゅうしゃ",
          "して",
          "から",
          "にねん"
        ],
        "answer": 1,
        "id": "N5-18-039"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "ひっこし",
          "して",
          "から",
          "いっかげつ"
        ],
        "answer": 1,
        "id": "N5-18-040"
      },
      {
        "type": "文章の文法",
        "text": "この えいがは（　）みたいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "おもしろ",
          "おもしろい",
          "おもしろく",
          "おもしろそう"
        ],
        "answer": 0,
        "id": "N5-18-041"
      },
      {
        "type": "文章の文法",
        "text": "わたしは あした ともだちに（　）つもりです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "あう",
          "あって",
          "あいます",
          "あった"
        ],
        "answer": 0,
        "id": "N5-18-042"
      },
      {
        "type": "文章の文法",
        "text": "この りんごは あか（　）、おいしそうです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "くて",
          "いで",
          "いと",
          "くない"
        ],
        "answer": 0,
        "id": "N5-18-043"
      },
      {
        "type": "文章の文法",
        "text": "きのう 10じまで（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "おきていました",
          "おきています",
          "おきて",
          "おきる"
        ],
        "answer": 0,
        "id": "N5-18-044"
      },
      {
        "type": "文章の文法",
        "text": "わたしは にほんに 2かい（　）ことがあります。",
        "question": "（　）に　はいるのは？",
        "options": [
          "いった",
          "いって",
          "いきます",
          "いく"
        ],
        "answer": 0,
        "id": "N5-18-045"
      },
      {
        "type": "読解（短文）",
        "text": "わたしは　コーヒーが　すきです。さとうを　いれません。ミルクだけ　いれます。",
        "question": "コーヒーに　なにを　いれますか。",
        "options": [
          "さとう",
          "ミルク",
          "さとうと　ミルク",
          "なにも　いれません"
        ],
        "answer": 1,
        "id": "N5-18-046"
      },
      {
        "type": "読解（短文）",
        "text": "きょうは　もくようびです。あしたは　きんようびです。",
        "question": "あしたは　なんようびですか。",
        "options": [
          "すいようび",
          "もくようび",
          "きんようび",
          "どようび"
        ],
        "answer": 1,
        "id": "N5-18-047"
      },
      {
        "type": "読解（短文）",
        "text": "わたしは　8がつ　15にちに　うまれました。",
        "question": "たんじょうびは　いつですか。",
        "options": [
          "7がつ15にち",
          "8がつ15にち",
          "9がつ15にち",
          "10がつ15にち"
        ],
        "answer": 1,
        "id": "N5-18-048"
      },
      {
        "type": "読解（中文）",
        "text": "わたしの　へやには　ほんだなが　あります。ほんが　100さつ　くらい　あります。まんがも　あります。",
        "question": "ほんは　なんさつ　くらい　ありますか。",
        "options": [
          "50さつ",
          "100さつ",
          "150さつ",
          "200さつ"
        ],
        "answer": 1,
        "id": "N5-18-049"
      },
      {
        "type": "読解（中文）",
        "text": "わたしは　スーパーで　はたらいています。まいにち　10じから　6じまでです。どようびも　はたらきます。にちようびは　やすみです。",
        "question": "いつ　やすみですか。",
        "options": [
          "どようび",
          "にちようび",
          "げつようび",
          "きんようび"
        ],
        "answer": 1,
        "id": "N5-18-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-18-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "15人",
          "16人",
          "17人",
          "18人"
        ],
        "answer": 0,
        "id": "N5-18-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "14人",
          "15人",
          "16人",
          "17人"
        ],
        "answer": 2,
        "id": "N5-18-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "13人",
          "14人",
          "15人",
          "16人"
        ],
        "answer": 0,
        "id": "N5-18-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "12人",
          "13人",
          "14人",
          "15人"
        ],
        "answer": 2,
        "id": "N5-18-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "11人",
          "12人",
          "13人",
          "14人"
        ],
        "answer": 0,
        "id": "N5-18-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "10人",
          "11人",
          "12人",
          "13人"
        ],
        "answer": 2,
        "id": "N5-18-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "8月",
          "9月",
          "10月",
          "11月"
        ],
        "answer": 0,
        "id": "N5-18-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "9月",
          "10月",
          "11月",
          "12月"
        ],
        "answer": 1,
        "id": "N5-18-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "1個",
          "2個",
          "3個",
          "4個"
        ],
        "answer": 0,
        "id": "N5-18-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "2個",
          "3個",
          "4個",
          "5個"
        ],
        "answer": 1,
        "id": "N5-18-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "3個",
          "4個",
          "5個",
          "6個"
        ],
        "answer": 0,
        "id": "N5-18-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "4個",
          "5個",
          "6個",
          "7個"
        ],
        "answer": 1,
        "id": "N5-18-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "いらっしゃいませ",
          "ようこそ",
          "お待ちしていました",
          "ご来店ありがとうございます"
        ],
        "answer": 0,
        "id": "N5-18-064",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "何名様ですか",
          "ご予約はございますか",
          "禁煙席と喫煙席",
          "カウンターとテーブル"
        ],
        "answer": 0,
        "id": "N5-18-065",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "ご注文は",
          "お決まりですか",
          "メニューをどうぞ",
          "おすすめは"
        ],
        "answer": 0,
        "id": "N5-18-066",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "こちらになります",
          "お待たせしました",
          "どうぞお召し上がりください",
          "ごゆっくりどうぞ"
        ],
        "answer": 0,
        "id": "N5-18-067",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "お会計は",
          "レジはあちらです",
          "カードでよろしいですか",
          "現金のみです"
        ],
        "answer": 0,
        "id": "N5-18-068",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "3人です",
          "4人です",
          "5人です",
          "6人です"
        ],
        "answer": 0,
        "id": "N5-18-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "4人です",
          "5人です",
          "6人です",
          "7人です"
        ],
        "answer": 0,
        "id": "N5-18-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "5人です",
          "6人です",
          "7人です",
          "8人です"
        ],
        "answer": 0,
        "id": "N5-18-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "6人です",
          "7人です",
          "8人です",
          "9人です"
        ],
        "answer": 0,
        "id": "N5-18-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "7人です",
          "8人です",
          "9人です",
          "10人です"
        ],
        "answer": 0,
        "id": "N5-18-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "8人です",
          "9人です",
          "10人です",
          "11人です"
        ],
        "answer": 0,
        "id": "N5-18-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "9人です",
          "10人です",
          "11人です",
          "12人です"
        ],
        "answer": 0,
        "id": "N5-18-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "10人です",
          "11人です",
          "12人です",
          "13人です"
        ],
        "answer": 0,
        "id": "N5-18-076",
        "script": "なんさいですか。"
      }
    ],
    "19": [
      {
        "type": "漢字読み",
        "text": "<u>上</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "うまれる",
          "うえ",
          "げつようび",
          "もり"
        ],
        "answer": 1,
        "id": "N5-19-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>書く</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "ごご",
          "やま",
          "かく",
          "おとこ"
        ],
        "answer": 2,
        "id": "N5-19-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>自分</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "きこえる",
          "じぶん",
          "おおきい",
          "あいだ"
        ],
        "answer": 1,
        "id": "N5-19-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>毎週</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "まいしゅう",
          "なか",
          "ごぜん",
          "らいしゅう"
        ],
        "answer": 0,
        "id": "N5-19-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>明るい</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "みち",
          "あかるい",
          "なに",
          "すく"
        ],
        "answer": 1,
        "id": "N5-19-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>一本</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "でんわ",
          "らいげつ",
          "はは",
          "いっぽん"
        ],
        "answer": 3,
        "id": "N5-19-006"
      },
      {
        "type": "表記",
        "text": "<u>て</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "長い",
          "手",
          "水",
          "休む"
        ],
        "answer": 1,
        "id": "N5-19-007"
      },
      {
        "type": "表記",
        "text": "<u>きこえる</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "三つ",
          "一人",
          "聞こえる",
          "出力"
        ],
        "answer": 2,
        "id": "N5-19-008"
      },
      {
        "type": "表記",
        "text": "<u>あいだ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "学校",
          "間",
          "国",
          "魚"
        ],
        "answer": 1,
        "id": "N5-19-009"
      },
      {
        "type": "表記",
        "text": "<u>おちゃ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "お茶",
          "食べる",
          "楽しい",
          "先生"
        ],
        "answer": 0,
        "id": "N5-19-010"
      },
      {
        "type": "表記",
        "text": "<u>まえ</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "元気",
          "左",
          "前",
          "長い"
        ],
        "answer": 2,
        "id": "N5-19-011"
      },
      {
        "type": "表記",
        "text": "<u>ひと</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "同じ",
          "力",
          "十分",
          "人"
        ],
        "answer": 3,
        "id": "N5-19-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "いす",
          "パン",
          "びょういん",
          "えき"
        ],
        "answer": 1,
        "id": "N5-19-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "えんぴつ",
          "ほん",
          "ジュース",
          "かぎ"
        ],
        "answer": 2,
        "id": "N5-19-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "でんわ",
          "ラーメン",
          "びょういん",
          "かばん"
        ],
        "answer": 1,
        "id": "N5-19-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "びょういん",
          "かさ",
          "でんわ",
          "さかな"
        ],
        "answer": 3,
        "id": "N5-19-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "おちゃ",
          "パン",
          "みず",
          "こうえん"
        ],
        "answer": 3,
        "id": "N5-19-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "きょうしつ",
          "おちゃ",
          "みかん",
          "えんぴつ"
        ],
        "answer": 0,
        "id": "N5-19-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "みかん",
          "ほん",
          "パン",
          "デパート"
        ],
        "answer": 3,
        "id": "N5-19-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "ほん",
          "パン",
          "テレビ",
          "うみ"
        ],
        "answer": 3,
        "id": "N5-19-020"
      },
      {
        "type": "言い換え",
        "text": "「できる」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "can do",
          "teacher",
          "mother (polite)",
          "to eat"
        ],
        "answer": 0,
        "id": "N5-19-021"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>お父さん<rt>おとうさん</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "juice",
          "father (polite)",
          "tomorrow",
          "bon appetit"
        ],
        "answer": 1,
        "id": "N5-19-022"
      },
      {
        "type": "言い換え",
        "text": "「ドア」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "door",
          "notebook",
          "water",
          "correct"
        ],
        "answer": 0,
        "id": "N5-19-023"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>道<rt>どう</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "excuse me",
          "road",
          "power",
          "door"
        ],
        "answer": 1,
        "id": "N5-19-024"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>花<rt>はな</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "man",
          "can do",
          "flower",
          "this month"
        ],
        "answer": 2,
        "id": "N5-19-025"
      },
      {
        "type": "文法形式",
        "text": "テレビ（　）ついています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "で",
          "を",
          "に"
        ],
        "answer": 0,
        "id": "N5-19-026"
      },
      {
        "type": "文法形式",
        "text": "がっこう（ 　）いきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "が",
          "を",
          "は"
        ],
        "answer": 0,
        "id": "N5-19-027"
      },
      {
        "type": "文法形式",
        "text": "ともだち（ 　）あいました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "を",
          "が",
          "は"
        ],
        "answer": 0,
        "id": "N5-19-028"
      },
      {
        "type": "文法形式",
        "text": "にほんご（　）べんきょうしています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "に",
          "が",
          "で"
        ],
        "answer": 0,
        "id": "N5-19-029"
      },
      {
        "type": "文法形式",
        "text": "でんしゃ（　）きます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "に",
          "は"
        ],
        "answer": 0,
        "id": "N5-19-030"
      },
      {
        "type": "文法形式",
        "text": "コーヒー（　）のみたいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "は",
          "に",
          "を"
        ],
        "answer": 0,
        "id": "N5-19-031"
      },
      {
        "type": "文法形式",
        "text": "えいが（　）みました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "に",
          "は"
        ],
        "answer": 0,
        "id": "N5-19-032"
      },
      {
        "type": "文法形式",
        "text": "へや（　）きれいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "は",
          "に",
          "が",
          "を"
        ],
        "answer": 0,
        "id": "N5-19-033"
      },
      {
        "type": "文法形式",
        "text": "たべ（　）から　ねます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "て",
          "る",
          "た",
          "ない"
        ],
        "answer": 0,
        "id": "N5-19-034"
      },
      {
        "type": "文法形式",
        "text": "にほんに　いき（　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "たい",
          "た",
          "る",
          "ます"
        ],
        "answer": 0,
        "id": "N5-19-035"
      },
      {
        "type": "文の組み立て",
        "text": "この ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "かばんは",
          "かるくて",
          "べんり",
          "です"
        ],
        "answer": 1,
        "id": "N5-19-036"
      },
      {
        "type": "文の組み立て",
        "text": "この ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "パソコンは",
          "うすくて",
          "かるい",
          "です"
        ],
        "answer": 1,
        "id": "N5-19-037"
      },
      {
        "type": "文の組み立て",
        "text": "この ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "スマホは",
          "ちいさくて",
          "べんり",
          "です"
        ],
        "answer": 1,
        "id": "N5-19-038"
      },
      {
        "type": "文の組み立て",
        "text": "この ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "レストランは",
          "やすくて",
          "おいしい",
          "です"
        ],
        "answer": 1,
        "id": "N5-19-039"
      },
      {
        "type": "文の組み立て",
        "text": "この ＿＿ ＿＿ ★ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "ホテルは",
          "きれいで",
          "しずか",
          "です"
        ],
        "answer": 1,
        "id": "N5-19-040"
      },
      {
        "type": "文章の文法",
        "text": "この ほんを（　）もいいですか。",
        "question": "（　）に　はいるのは？",
        "options": [
          "かりて",
          "かります",
          "かりた",
          "かりる"
        ],
        "answer": 0,
        "id": "N5-19-041"
      },
      {
        "type": "文章の文法",
        "text": "きょうは あまり さむ（　）ないです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "く",
          "い",
          "くて",
          "いで"
        ],
        "answer": 0,
        "id": "N5-19-042"
      },
      {
        "type": "文章の文法",
        "text": "わたしは まいにち うんどう（　）います。",
        "question": "（　）に　はいるのは？",
        "options": [
          "して",
          "します",
          "した",
          "する"
        ],
        "answer": 0,
        "id": "N5-19-043"
      },
      {
        "type": "文章の文法",
        "text": "この へやは（　）すぎます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "せまく",
          "せまい",
          "せま",
          "せまくて"
        ],
        "answer": 0,
        "id": "N5-19-044"
      },
      {
        "type": "文章の文法",
        "text": "あした てんきが（　）、ピクニックにいきます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "よかったら",
          "よくて",
          "いいから",
          "いいので"
        ],
        "answer": 0,
        "id": "N5-19-045"
      },
      {
        "type": "読解（短文）",
        "text": "としょかんは　9じから　5じまで　あいています。",
        "question": "としょかんは　なんじまで　あいていますか。",
        "options": [
          "4じ",
          "5じ",
          "6じ",
          "7じ"
        ],
        "answer": 1,
        "id": "N5-19-046"
      },
      {
        "type": "読解（短文）",
        "text": "わたしは　いぬが　すきです。ねこも　すきです。",
        "question": "なにが　すきですか。",
        "options": [
          "いぬだけ",
          "ねこだけ",
          "いぬと　ねこ",
          "とり"
        ],
        "answer": 1,
        "id": "N5-19-047"
      },
      {
        "type": "読解（短文）",
        "text": "すずきさんは　めがねを　かけています。かみが　ながいです。",
        "question": "すずきさんは　どんな　ひとですか。",
        "options": [
          "かみが　みじかい",
          "めがねを　かけている",
          "せが　ひくい",
          "ふとっている"
        ],
        "answer": 1,
        "id": "N5-19-048"
      },
      {
        "type": "読解（中文）",
        "text": "たなかさんは　びょういんで　はたらいています。かんごしです。よるも　しごとが　あります。たいへんですが、やりがいが　あります。",
        "question": "たなかさんは　なにを　していますか。",
        "options": [
          "いしゃ",
          "かんごし",
          "やくざいし",
          "じむいん"
        ],
        "answer": 1,
        "id": "N5-19-049"
      },
      {
        "type": "読解（中文）",
        "text": "あさ　6じに　おきて、7じに　いえを　でます。かいしゃには　8じに　つきます。しごとは　9じから　はじまります。",
        "question": "しごとは　なんじに　はじまりますか。",
        "options": [
          "7じ",
          "8じ",
          "9じ",
          "10じ"
        ],
        "answer": 1,
        "id": "N5-19-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-19-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "9人",
          "10人",
          "11人",
          "12人"
        ],
        "answer": 0,
        "id": "N5-19-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "8人",
          "9人",
          "10人",
          "11人"
        ],
        "answer": 2,
        "id": "N5-19-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "7人",
          "8人",
          "9人",
          "10人"
        ],
        "answer": 0,
        "id": "N5-19-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "6人",
          "7人",
          "8人",
          "9人"
        ],
        "answer": 2,
        "id": "N5-19-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "5人",
          "6人",
          "7人",
          "8人"
        ],
        "answer": 0,
        "id": "N5-19-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "4人",
          "5人",
          "6人",
          "7人"
        ],
        "answer": 2,
        "id": "N5-19-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "5個",
          "6個",
          "7個",
          "8個"
        ],
        "answer": 0,
        "id": "N5-19-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "6個",
          "7個",
          "8個",
          "9個"
        ],
        "answer": 1,
        "id": "N5-19-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "7個",
          "8個",
          "9個",
          "10個"
        ],
        "answer": 0,
        "id": "N5-19-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "8個",
          "9個",
          "10個",
          "11個"
        ],
        "answer": 1,
        "id": "N5-19-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "9個",
          "10個",
          "11個",
          "12個"
        ],
        "answer": 0,
        "id": "N5-19-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "10個",
          "11個",
          "12個",
          "13個"
        ],
        "answer": 1,
        "id": "N5-19-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "レシートはご入用ですか",
          "袋はご入用ですか",
          "ポイントカードは",
          "領収書は"
        ],
        "answer": 0,
        "id": "N5-19-064",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "お釣りです",
          "ちょうどいただきます",
          "〇〇円のお返しです",
          "お確かめください"
        ],
        "answer": 0,
        "id": "N5-19-065",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "ありがとうございました",
          "またお越しください",
          "お気をつけて",
          "よい一日を"
        ],
        "answer": 0,
        "id": "N5-19-066",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "申し訳ございません",
          "売り切れです",
          "品切れです",
          "在庫がございません"
        ],
        "answer": 0,
        "id": "N5-19-067",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "少々お待ちください",
          "確認してまいります",
          "お調べします",
          "お取り寄せできます"
        ],
        "answer": 0,
        "id": "N5-19-068",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "1回です",
          "2回です",
          "3回です",
          "4回です"
        ],
        "answer": 0,
        "id": "N5-19-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "2回です",
          "3回です",
          "4回です",
          "5回です"
        ],
        "answer": 0,
        "id": "N5-19-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "3回です",
          "4回です",
          "5回です",
          "6回です"
        ],
        "answer": 0,
        "id": "N5-19-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "4回です",
          "5回です",
          "6回です",
          "7回です"
        ],
        "answer": 0,
        "id": "N5-19-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "5回です",
          "6回です",
          "7回です",
          "8回です"
        ],
        "answer": 0,
        "id": "N5-19-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "6回です",
          "7回です",
          "8回です",
          "9回です"
        ],
        "answer": 0,
        "id": "N5-19-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "7回です",
          "8回です",
          "9回です",
          "10回です"
        ],
        "answer": 0,
        "id": "N5-19-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "8回です",
          "9回です",
          "10回です",
          "11回です"
        ],
        "answer": 0,
        "id": "N5-19-076",
        "script": "なんさいですか。"
      }
    ],
    "20": [
      {
        "type": "漢字読み",
        "text": "<u>社会</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "のむ",
          "せんげつ",
          "しゃかい",
          "はな"
        ],
        "answer": 2,
        "id": "N5-20-001"
      },
      {
        "type": "漢字読み",
        "text": "<u>駅</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "えき",
          "つくる",
          "ひだり",
          "まいにち"
        ],
        "answer": 0,
        "id": "N5-20-002"
      },
      {
        "type": "漢字読み",
        "text": "<u>花</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "なか",
          "はな",
          "ひゃく",
          "だいがく"
        ],
        "answer": 1,
        "id": "N5-20-003"
      },
      {
        "type": "漢字読み",
        "text": "<u>大きい</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "おおきい",
          "ごぜん",
          "せんしゅう",
          "でんき"
        ],
        "answer": 0,
        "id": "N5-20-004"
      },
      {
        "type": "漢字読み",
        "text": "<u>お父さん</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "おとうさん",
          "らいげつ",
          "みる",
          "なか"
        ],
        "answer": 0,
        "id": "N5-20-005"
      },
      {
        "type": "漢字読み",
        "text": "<u>後で</u>の　よみかたは？",
        "question": "よみかた",
        "options": [
          "あとで",
          "みっつ",
          "きょうしつ",
          "した"
        ],
        "answer": 0,
        "id": "N5-20-006"
      },
      {
        "type": "表記",
        "text": "<u>しゃちょう</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "右",
          "一つ",
          "社長",
          "耳"
        ],
        "answer": 2,
        "id": "N5-20-007"
      },
      {
        "type": "表記",
        "text": "<u>おんな</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "一つ",
          "女",
          "千",
          "耳"
        ],
        "answer": 1,
        "id": "N5-20-008"
      },
      {
        "type": "表記",
        "text": "<u>ちち</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "父",
          "時々",
          "来月",
          "外"
        ],
        "answer": 0,
        "id": "N5-20-009"
      },
      {
        "type": "表記",
        "text": "<u>いっぽん</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "来週",
          "本",
          "前",
          "一本"
        ],
        "answer": 3,
        "id": "N5-20-010"
      },
      {
        "type": "表記",
        "text": "<u>ときどき</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "時々",
          "お母さん",
          "名前",
          "外国"
        ],
        "answer": 0,
        "id": "N5-20-011"
      },
      {
        "type": "表記",
        "text": "<u>ごご</u>の　かんじは？",
        "question": "かきかた",
        "options": [
          "食べる",
          "空く",
          "社長",
          "午後"
        ],
        "answer": 3,
        "id": "N5-20-012"
      },
      {
        "type": "文脈規定",
        "text": "あさ（　）を　たべます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "くるま",
          "つくえ",
          "ごはん",
          "かさ"
        ],
        "answer": 2,
        "id": "N5-20-013"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）を　のみます。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "めがね",
          "ぎゅうにゅう",
          "かぎ",
          "ほん"
        ],
        "answer": 1,
        "id": "N5-20-014"
      },
      {
        "type": "文脈規定",
        "text": "ひるごはんに（　）を　たべました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "でんわ",
          "びょういん",
          "そば",
          "かばん"
        ],
        "answer": 2,
        "id": "N5-20-015"
      },
      {
        "type": "文脈規定",
        "text": "ばんごはんは（　）が　すきです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "えき",
          "くるま",
          "いす",
          "てんぷら"
        ],
        "answer": 3,
        "id": "N5-20-016"
      },
      {
        "type": "文脈規定",
        "text": "きのう（　）に　いきました。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "りんご",
          "みず",
          "としょかん",
          "えんぴつ"
        ],
        "answer": 2,
        "id": "N5-20-017"
      },
      {
        "type": "文脈規定",
        "text": "まいにち（　）で　べんきょうします。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "りんご",
          "パン",
          "みず",
          "きょうしつ"
        ],
        "answer": 3,
        "id": "N5-20-018"
      },
      {
        "type": "文脈規定",
        "text": "（　）で　かいものを　します。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "えんぴつ",
          "おちゃ",
          "スーパー",
          "ほん"
        ],
        "answer": 2,
        "id": "N5-20-019"
      },
      {
        "type": "文脈規定",
        "text": "あした（　）に　いきたいです。",
        "question": "（　）に入れるのに最もよいものを選んでください。",
        "options": [
          "えいがかん",
          "ほん",
          "パン",
          "みず"
        ],
        "answer": 0,
        "id": "N5-20-020"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>時計<rt>とけい</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "watch/clock",
          "thanks for meal",
          "flower",
          "road"
        ],
        "answer": 0,
        "id": "N5-20-021"
      },
      {
        "type": "言い換え",
        "text": "「でも」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "sound",
          "thousand",
          "but",
          "train"
        ],
        "answer": 2,
        "id": "N5-20-022"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>来る<rt>くる</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "to write",
          "station",
          "to come",
          "here"
        ],
        "answer": 2,
        "id": "N5-20-023"
      },
      {
        "type": "言い換え",
        "text": "「タクシー」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "department store",
          "forest",
          "taxi",
          "when"
        ],
        "answer": 2,
        "id": "N5-20-024"
      },
      {
        "type": "言い換え",
        "text": "「<ruby>雨<rt>あめ</rt></ruby>」の　いみは？",
        "question": "ちかい　いみ",
        "options": [
          "inside",
          "rain",
          "color",
          "weather"
        ],
        "answer": 1,
        "id": "N5-20-025"
      },
      {
        "type": "文法形式",
        "text": "わたしは　がくせい（ 　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "です",
          "ます",
          "だ",
          "ある"
        ],
        "answer": 0,
        "id": "N5-20-026"
      },
      {
        "type": "文法形式",
        "text": "がっこう（　）いきます。  ",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "は",
          "が",
          "を"
        ],
        "answer": 0,
        "id": "N5-20-027"
      },
      {
        "type": "文法形式",
        "text": "ともだち（　）あいました。  ",
        "question": "（　）に　はいるのは？",
        "options": [
          "に",
          "を",
          "は",
          "が"
        ],
        "answer": 0,
        "id": "N5-20-028"
      },
      {
        "type": "文法形式",
        "text": "にほんご（ 　）べんきょうしています。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "で",
          "に"
        ],
        "answer": 0,
        "id": "N5-20-029"
      },
      {
        "type": "文法形式",
        "text": "でんしゃ（ 　）きます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "は",
          "に"
        ],
        "answer": 0,
        "id": "N5-20-030"
      },
      {
        "type": "文法形式",
        "text": "コーヒー（ 　）のみたいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "が",
          "を",
          "に",
          "は"
        ],
        "answer": 0,
        "id": "N5-20-031"
      },
      {
        "type": "文法形式",
        "text": "えいが（ 　）みました。",
        "question": "（　）に　はいるのは？",
        "options": [
          "を",
          "が",
          "は",
          "に"
        ],
        "answer": 0,
        "id": "N5-20-032"
      },
      {
        "type": "文法形式",
        "text": "へや（ 　）きれいです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "は",
          "を",
          "が",
          "に"
        ],
        "answer": 0,
        "id": "N5-20-033"
      },
      {
        "type": "文法形式",
        "text": "たべ（ 　）から　ねます。",
        "question": "（　）に　はいるのは？",
        "options": [
          "て",
          "た",
          "る",
          "ない"
        ],
        "answer": 0,
        "id": "N5-20-034"
      },
      {
        "type": "文法形式",
        "text": "にほんに　いき（ 　）です。",
        "question": "（　）に　はいるのは？",
        "options": [
          "たい",
          "る",
          "ます",
          "た"
        ],
        "answer": 0,
        "id": "N5-20-035"
      },
      {
        "type": "文の組み立て",
        "text": "わたしは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "にほんご",
          "だけでなく",
          "えいごも",
          "はなせます"
        ],
        "answer": 0,
        "id": "N5-20-036"
      },
      {
        "type": "文の組み立て",
        "text": "かれは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "うたが",
          "じょうず",
          "だけでなく",
          "ダンスも"
        ],
        "answer": 0,
        "id": "N5-20-037"
      },
      {
        "type": "文の組み立て",
        "text": "この ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "みせは",
          "やすい",
          "だけでなく",
          "おいしい"
        ],
        "answer": 0,
        "id": "N5-20-038"
      },
      {
        "type": "文の組み立て",
        "text": "かのじょは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "きれい",
          "だけでなく",
          "あたまも",
          "いい"
        ],
        "answer": 0,
        "id": "N5-20-039"
      },
      {
        "type": "文の組み立て",
        "text": "このまちは ＿＿ ★ ＿＿ ＿＿。",
        "question": "★に入るものを選んでください",
        "options": [
          "べんり",
          "だけでなく",
          "しぜんも",
          "おおい"
        ],
        "answer": 0,
        "id": "N5-20-040"
      },
      {
        "type": "文章の文法",
        "text": "わたしは えいがを（　）のが すきです。",
        "question": "（　）に　はいるのは？",
        "options": [
          "みる",
          "みて",
          "みます",
          "みた"
        ],
        "answer": 0,
        "id": "N5-20-041"
      },
      {
        "type": "文章の文法",
        "text": "この レストランは（　）そうですね。",
        "question": "（　）に　はいるのは？",
        "options": [
          "おいしい",
          "おいし",
          "おいしく",
          "おいしくて"
        ],
        "answer": 0,
        "id": "N5-20-042"
      },
      {
        "type": "文章の文法",
        "text": "きのう ともだちと でんわで（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "はなしました",
          "はなして",
          "はなします",
          "はなす"
        ],
        "answer": 0,
        "id": "N5-20-043"
      },
      {
        "type": "文章の文法",
        "text": "このみせは　やすくて、（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "だから",
          "でも",
          "しかし",
          "それで"
        ],
        "answer": 0,
        "id": "N5-20-044"
      },
      {
        "type": "文章の文法",
        "text": "わたしは　まいあさ　6じに　おきます。かおを　あらって、あさごはんを（　）。",
        "question": "（　）に　はいるのは？",
        "options": [
          "たべます",
          "たべて",
          "たべた",
          "たべる"
        ],
        "answer": 0,
        "id": "N5-20-045"
      },
      {
        "type": "読解（短文）",
        "text": "このへやは　しずかです。べんきょうに　いいです。",
        "question": "このへやは　どうですか。",
        "options": [
          "うるさい",
          "しずか",
          "くらい",
          "せまい"
        ],
        "answer": 1,
        "id": "N5-20-046"
      },
      {
        "type": "読解（短文）",
        "text": "わたしは　まいにち　8じに　がっこうに　いきます。",
        "question": "なんじに　がっこうに　いきますか。",
        "options": [
          "7じ",
          "8じ",
          "9じ",
          "10じ"
        ],
        "answer": 1,
        "id": "N5-20-047"
      },
      {
        "type": "読解（短文）",
        "text": "えきから　がっこうまで　バスで　10ぷんです。",
        "question": "バスで　なんぷんですか。",
        "options": [
          "5ふん",
          "10ぷん",
          "15ふん",
          "20ぷん"
        ],
        "answer": 1,
        "id": "N5-20-048"
      },
      {
        "type": "読解（中文）",
        "text": "きょうは　しごとが　いそがしかったです。ざんぎょうを　して、9じに　いえに　かえりました。つかれました。",
        "question": "なんじに　いえに　かえりましたか。",
        "options": [
          "6じ",
          "7じ",
          "8じ",
          "9じ"
        ],
        "answer": 1,
        "id": "N5-20-049"
      },
      {
        "type": "読解（中文）",
        "text": "わたしの　かいしゃは　おおきいです。500にん　くらい　はたらいています。ビルの　10かいに　あります。\n\nかいしゃには　なんにん　くらい　いますか。",
        "question": "かぞくは　なんにんですか。",
        "options": [
          "100にん",
          "300にん",
          "500にん",
          "700にん"
        ],
        "answer": 1,
        "id": "N5-20-050"
      },
      {
        "type": "情報検索",
        "text": "【としょかん】あいているじかん: 9:00-17:00　やすみ: げつようび",
        "question": "なんじまで　あいていますか。",
        "options": [
          "15じ",
          "16じ",
          "17じ",
          "18じ"
        ],
        "answer": 2,
        "id": "N5-20-051"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "3人",
          "4人",
          "5人",
          "6人"
        ],
        "answer": 0,
        "id": "N5-20-052",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "2人",
          "3人",
          "4人",
          "5人"
        ],
        "answer": 2,
        "id": "N5-20-053",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "1人",
          "2人",
          "3人",
          "4人"
        ],
        "answer": 0,
        "id": "N5-20-054",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "20個",
          "21個",
          "22個",
          "23個"
        ],
        "answer": 2,
        "id": "N5-20-055",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "19個",
          "20個",
          "21個",
          "22個"
        ],
        "answer": 0,
        "id": "N5-20-056",
        "script": "おとこのひとと　おんなのひとが　はなしています。おとこのひとは　なにを　かいますか。"
      },
      {
        "type": "聴解（課題理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "18個",
          "19個",
          "20個",
          "21個"
        ],
        "answer": 2,
        "id": "N5-20-057",
        "script": "がくせいと　せんせいが　はなしています。しゅくだいは　いつまでですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "11個",
          "12個",
          "13個",
          "14個"
        ],
        "answer": 0,
        "id": "N5-20-058",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "12個",
          "13個",
          "14個",
          "15個"
        ],
        "answer": 1,
        "id": "N5-20-059",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "13個",
          "14個",
          "15個",
          "16個"
        ],
        "answer": 0,
        "id": "N5-20-060",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "14個",
          "15個",
          "16個",
          "17個"
        ],
        "answer": 1,
        "id": "N5-20-061",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "15個",
          "16個",
          "17個",
          "18個"
        ],
        "answer": 0,
        "id": "N5-20-062",
        "script": "おんなのひとが　はなしています。なにが　すきですか。"
      },
      {
        "type": "聴解（ポイント理解）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "しつもん",
        "options": [
          "16個",
          "17個",
          "18個",
          "19個"
        ],
        "answer": 1,
        "id": "N5-20-063",
        "script": "おとこのひとが　はなしています。どこに　すんでいますか。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "試着できますか",
          "サイズはございますか",
          "色違いはありますか",
          "他の商品は"
        ],
        "answer": 0,
        "id": "N5-20-064",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "返品できますか",
          "交換できますか",
          "保証はありますか",
          "修理できますか"
        ],
        "answer": 0,
        "id": "N5-20-065",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "配送できますか",
          "お届けはいつですか",
          "送料はいくらですか",
          "届け先は"
        ],
        "answer": 0,
        "id": "N5-20-066",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "ポイントが貯まります",
          "割引があります",
          "セール中です",
          "お得ですよ"
        ],
        "answer": 0,
        "id": "N5-20-067",
        "script": "「ありがとう」と　いわれました。"
      },
      {
        "type": "聴解（発話表現）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　いいますか。",
        "options": [
          "ラッピングできますか",
          "プレゼント用ですか",
          "のしは",
          "メッセージカードは"
        ],
        "answer": 0,
        "id": "N5-20-068",
        "script": "あさ、せんせいに　あいました。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "9回です",
          "10回です",
          "11回です",
          "12回です"
        ],
        "answer": 0,
        "id": "N5-20-069",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "10回です",
          "11回です",
          "12回です",
          "13回です"
        ],
        "answer": 0,
        "id": "N5-20-070",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "はい",
          "いいえ",
          "はい、そうです",
          "いいえ、違います"
        ],
        "answer": 0,
        "id": "N5-20-071",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "そうです",
          "違います",
          "そうかもしれません",
          "分かりません"
        ],
        "answer": 0,
        "id": "N5-20-072",
        "script": "なんさいですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "分かりました",
          "分かりません",
          "了解です",
          "承知しました"
        ],
        "answer": 0,
        "id": "N5-20-073",
        "script": "おなまえは？"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "いいです",
          "だめです",
          "大丈夫です",
          "無理です"
        ],
        "answer": 0,
        "id": "N5-20-074",
        "script": "これは　いくらですか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "あります",
          "ありません",
          "あるかもしれません",
          "分かりません"
        ],
        "answer": 0,
        "id": "N5-20-075",
        "script": "どこに　すんでいますか。"
      },
      {
        "type": "聴解（即時応答）",
        "text": {
          "ja": "🔊 音声を聞いて答えてください",
          "en": "🔊 Listen and answer",
          "zh": "🔊 请听录音并回答",
          "ko": "🔊 음성을 듣고 답하세요",
          "vi": "🔊 Nghe và trả lời",
          "id": "🔊 Dengarkan dan jawab",
          "th": "🔊 ฟังและตอบ",
          "es": "🔊 Escucha y responde",
          "pt": "🔊 Ouça e responda",
          "fr": "🔊 Écoutez et répondez"
        },
        "question": "なんと　こたえますか。",
        "options": [
          "います",
          "いません",
          "いるかもしれません",
          "分かりません"
        ],
        "answer": 0,
        "id": "N5-20-076",
        "script": "なんさいですか。"
      }
    ]
  }
};
